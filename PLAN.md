...
### MOVE-02 · Goal/path caching to stop per-tick A* recomputation
Identified during the SIM-08 `--map` investigation. `astar_advance`/`astar_next_step` run a complete from-scratch A* over the full passable-cell graph (~3,700 cells on San Marcos) **every move tick**, just to take one step's worth of cells — no path or goal memoization. Measured cost: **2,752 ms/round with a map vs 354 ms/round on the 3-zone fallback (~8×)**, the bulk of the "significantly longer with a map" slowdown. Cache the chosen goal and its A* path per player; re-step along the cached path each move tick and only recompute when (a) the goal changes, (b) the path is blocked, or (c) the player is knocked off-path (downed/respawned). Becomes more important once MOVE-01 lands (all roles moving ⇒ more move ticks). Goal-recompute throttling (recompute goal every N ticks rather than every move) is an acceptable sub-option. Must preserve the SIM-07/SIM-08 determinism contract (identical games for a given master seed, serial == parallel) — any caching must be a pure performance optimisation with no behavioural change.
