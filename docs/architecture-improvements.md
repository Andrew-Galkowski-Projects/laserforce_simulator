...
## 5. Collapse `PlayerRoundState` counters into a derived view — **REJECTED — see [ADR-0018](adr/0018-playerroundstate-counters-deliberate.md)**

The 18 counter fields on `PlayerRoundState` are not a denormalised cache of `GameEvent` aggregates — they are tick-loop state. `BatchSimulator` is pure in-memory during the round; counters on the `PlayerState` dataclass and events in the `EventLog` are co-products of the same tick step, both flushed atomically at round close, so there is no "events first, counters derived" write order to denormalise from. The counters are also load-bearing mid-loop (MVP calculation, scoring-burst Highlights, event-metadata SP snapshots all read `points_scored` / `tags_made` / etc. every tick), and the aggregate-vs-directional split between `PlayerRoundState` (per-round aggregate, O(1) read) and `GameEvent` (directional A→B / per-pair `GROUP BY`) is the documented pattern from CONTEXT.md's **Player head-to-head record** entry, not an invariant-by-convention. See ADR-0018 for the full argument and the rejected alternatives (derive at read time; round-close JSON snapshot; pass-through view interface).
