...
class QuadtreeNode:
    ...
    def _try_subdivide(self):
        ...
        self.children = [
            QuadtreeNode(self.r_min, r_mid, self.c_min, c_mid, self.max_depth, self.depth + 1),
            QuadtreeNode(self.r_min, r_mid, c_mid + 1, self.c_max, self.max_depth, self.depth + 1),
            QuadtreeNode(r_mid + 1, self.r_max, self.c_min, c_mid, self.max_depth, self.depth + 1),
            QuadtreeNode(r_mid + 1, self.r_max, c_mid + 1, self.c_max, self.max_depth, self.depth + 1),
        ]
        # Redistribute cells
...
def detect_zones(image_path, cell_size):
    ...
    blocked_edges_dict, blocked_edges_grid = _compute_blocked_edges(processed_bw, rows, cols, cell_size)
...
def create_processed_image(image_path):
    ...
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(features, connectivity=8)
...
def _compute_blocked_edges(processed_bw, rows, cols, cell_size):
    ...
    """
    blocked_dict = {}
    blocked_grid = [[{"e": False, "s": False} for _ in range(cols)] for _ in range(rows)]
        ...
        for c in range(cols):
            ...
            if c < cols - 1:
                ...
                if x1 < processed_bw.shape[1]:
                    ...
                    wall_ratio = np.sum(edge_pixels < 128) / len(edge_pixels) if len(edge_pixels) > 0 else 0
                    if wall_ratio > 0.3:
            ...
            if moved_x and prev_cy in range(len(blocked_edges_grid)):
                if prev_cx in range(len(blocked_edges_grid[0])):
                    if sx > 0 and blocked_edges_grid[prev_cy][prev_cx].get("e", False):
                        return False
                    elif sx < 0 and prev_cx > 0 and blocked_edges_grid[prev_cy][prev_cx - 1].get("e", False):
                        return False
            ...
            if moved_y and cy in range(len(blocked_edges_grid)):
                if prev_cx in range(len(blocked_edges_grid[0])):
                    if sy > 0 and blocked_edges_grid[prev_cy][prev_cx].get("s", False):
                        return False
                    elif sy < 0 and prev_cy > 0 and blocked_edges_grid[prev_cy - 1][prev_cx].get("s", False):
                        return False
...
def compute_sight_lines(zone_data, blocked_edges_grid=None, use_quadtree=True):
    ...
    if isinstance(zone_data, dict):
        blocked_edges_grid = zone_data.get("blocked_edges_grid", zone_data.get("blocked_edges", {}))
    ...
    passable = [(r, c) for r in range(rows) for c in range(cols) if zone_grid[r][c] != 0]
...
def compute_single_cell_visibility(r1, c1, zone_data, blocked_edges_grid=None):
    ...
    for r2, c2 in passable:
        if (r1, c1) != (r2, c2) and _has_los(zone_grid, r1, c1, r2, c2, blocked_edges_grid):
