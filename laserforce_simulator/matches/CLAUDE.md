...
**Worker concurrency model.** Each task is **serial inside its body** (`workers=1` baked in). Horizontal throughput comes from running multiple Celery workers — `celery -A laserforce_simulator worker --concurrency 4` — one concurrency knob (`--concurrency`), not two stacked. The SIM-11 `_workers_for(n)` in-process helper is **retired** as part of this swap; the threshold/cap heuristic was tied to a single-process `ProcessPoolExecutor` and does not apply to broker-distributed tasks. **GEN-01 (2026-06-26) partially revives intra-task parallelism for `simulate_batch_task` ONLY:** the SIM-11 `_workers_for(n)` heuristic (`1` for `n < 50`, else `min(os.cpu_count() or 1, n)` — **all cores**, bounded only by the game count; the old SIM-11 4-cap is dropped since the `n < 50` gate already keeps every test/CI batch serial) is restored in `matches/tasks.py` and threaded as `workers=_batch_workers(n)` into `run_incremental` — so a **single** large batch run uses multiple cores (API-03's `--concurrency` only parallelises *across* tasks, never *within* one run). `_batch_workers(n)` **daemon-guards** it: `multiprocessing.current_process().daemon` ⇒ `1`, because a Celery **prefork** worker runs each task in a daemonic child that **may not spawn subprocesses** (`ProcessPoolExecutor` would raise `AssertionError: daemonic processes are not allowed to have children`). So under prefork the task stays serial (unchanged from API-03, scaling via `--concurrency`); under **EAGER** (task runs in the Django process) or a **`solo`/`threads`** pool (non-daemonic) the run goes multi-core. `n < 50` stays serial regardless (every batch test uses `n ≤ 10`, so all stay serial — no subprocess spawn in CI). Determinism is unchanged — the SIM-10 serial==parallel-at-every-boundary contract holds for any `workers`, so **no Score Calibration re-baseline**. `save_games_task` is **unchanged** (it replays specific avg/outlier seeds serially, small N). See [ADR-0013](../../docs/adr/0013-async-batch-execution-via-celery-redis.md) (GEN-01 addendum).
...
**Round-playback map overlay.** `game_round_events` (view) additionally calls `_build_playback_map(game_round) -> dict | None` (`matches/views.py`) and passes `playback_map_json` + `processed_image_url` into the context. The overlay re-lays-out the SIM-05 Playback card into **map-above / scoreboard-below / live-feed-beside** and animates player markers + comet **Movement trails** on a `<canvas>` drawn over the arena's processed map PNG (the same `processed_image` URL the RES-04 heatmap uses; cell→pixel mapping via `zone_size`), driven by the existing `pbCur` clock (single scrubber — no second engine). `_build_playback_map` returns `None` for a map-less round (3-zone fallback) — the canvas is hidden, a `pb-no-map-notice` renders, and the scoreboard + live feed still work. Per player it ships `spawn` + `moves = [[tick, polyline], …]` where the polyline is the **exact cells the player walked that Advance**: it prefers the persisted `GameEvent.metadata["route"]` (MOVE-01, see the **`GameEvent`** paragraph above) and falls back to a fresh `astar_path(start, end)` reconstruction only for pre-route rounds. The route is captured during simulation on the transient `PlayerState.movement_routes` (in lockstep with `movement_trail`; see [`sim_helpers/CLAUDE.md`](sim_helpers/CLAUDE.md)) and written by `_flush_to_db`. The client interpolates marker/trail position along the polyline with **Catmull-Rom** smoothing (rounds the 4-connected grid staircase into a flowing curve, so even old A*-reconstructed rounds read smoothly). The events page also gains a compact play-synced **live feed** pane (`pb-live-feed`) listing non-movement events up to the cursor. DOM ids: `pb-map-stage` / `pb-map-bg` / `pb-map-canvas` / `pb-map-data` (the `json_script` payload) / `pb-no-map-notice` / `pb-live-feed`. Tested by `matches/tests/test_playback_map.py`. **No model change / no migration** — `movement_routes` is transient and `metadata["route"]` rides on the existing `GameEvent.metadata` JSON; the MOVE-01 "no route list / flat scalars" assertion in `test_map.py` / `test_batch_sim.py` is deliberately superseded (the canonical `route` key is now allowed, alternative encodings still forbidden).
...
## CAR-01 manager team assignment

Lets the manager **name their own team at create-League time**; that named team becomes one of the N generated teams **and** the League's `current_team` (the manager's career team). Single-player `league` mode **IS** career mode — there is **NO new mode value** and **NO `Manager` / `User` model** (the manager is the implicit local user; both deferred to UX-01). **Reuses the existing `League.current_team` FK** — no new model field, no migration. Seam contract: [`.claude/worktrees/car-01-manager-team-assignment-seam-contract.md`](../../.claude/worktrees/car-01-manager-team-assignment-seam-contract.md).

**Form.** ONE new optional field on `matches.forms.CreateLeagueForm` (the LG-01b form): `manager_team_name = forms.CharField(max_length=100, required=False, label="Your team name")`, inserted **after `league_name` / before `season_name`** in the pinned field order. **No `clean()` change, no uniqueness validation** (team names are not globally unique). Blank is valid — the LG-01b 7-field validation surface is otherwise untouched.

**View wiring** (`matches.league_views.league_create`, inside its existing `@transaction.atomic`). When `cleaned["manager_team_name"]` is **non-blank** (stripped), the **alphabetical-first** generated team is renamed to it and set as `league.current_team`; **blank → the verbatim LG-01g `sorted(created_teams, key=name)[0]` alphabetical auto-pick** (byte-identical, backward-compatible — pre-CAR-01 blank-name behaviour is preserved exactly). The named team **stays one of the N** (league size unchanged = `num_teams`). The rename + `current_team` assignment run at the current `current_team` insertion point, **before** `Season.objects.create` / `season.teams.add(*created_teams)` / the phase loop / `_write_baseline_ratings`.

**Template.** `templates/leagues/create.html` gains one field row with DOM id `league-create-manager-team-name`, placed **between the league-name and season-name rows** (mirrors the LG-01b field-by-field rendering for deterministic ids). The existing LG-01g 'your team' framing already surfaces `current_team`, so no other template change.

**Scope-out (locked).** The **per-team scouting budget is DEFERRED** (out of CAR-01 scope — LG-05's FIXED `DEFAULT_SCOUTING_BUDGET = 50` stands). **No migration, no model field** (reuses `League.current_team`), **no new mode value**, **no `Manager` / `User` model**, **no simulator change**, **no Score Calibration re-baseline**, **no ADR**.

**Tests** live in `matches/tests/test_league_create.py::TestCar01ManagerTeamName` (the LG-01b create-flow test file).
...
> **RETIRED by NAV-01.** These LG-01d advancement DOM ids (`{season,league}-dashboard-play-dropdown` / `-play-start-season` / `-play-one-week` / `-play-two-months` / `-play-until-end` / `-play-progress`) and the `-action-button` wrapper are **DELETED** from both dashboards — the advancement controls relocate to the league-branch `Play ▾` topnav (`topbar-play-*` ids; see **NAV-01** above). The `{season,league}-dashboard-play-error` banner is **KEPT** (sync errors still land on the dashboard). Post-NAV-01 dashboard tests assert these advancement ids are ABSENT.

**Inline polling JS** — both dashboard templates carry an inline `<script>` block (no external JS file, no htmx, no framework). The One Week form submits normally (sync — server-side redirect); the Two Months and Until End forms intercept submit via `addEventListener("submit", e => { e.preventDefault(); fetch(form.action, {method: "POST", body: new FormData(form), …}).then(r => r.json()).then(json => startPolling(json.job_id, json.season_id)); })`. Polling cycle is 1000 ms via `setInterval`, hitting `{% url 'play_status' season_id=season.id job_id='JOB' %}?season_id={{ season.id }}` with `'JOB'` substituted client-side. On `data.status === "complete"` ⇒ `clearInterval` + `window.location.reload()`. On `data.status === "error"` ⇒ render `data.error` into the `play-error` element and re-enable the dropdown. **The JS is duplicated across the two templates** (no Django `{% include %}` for the JS block) — keeping it inline and per-template is the LG-01d simplicity choice over factoring out a shared partial. *(RETIRED by NAV-01: both inline poll blocks are deleted from the dashboards and relocated to ONE copy in the league branch of `base.html`, re-targeted at the `topbar-play-*` hooks — see **NAV-01** above.)*
...
> **RETIRED by NAV-01.** The `{league,season}-dashboard-next-season-form` ids (and, for career leagues, the `{season,league}-dashboard-owner-evaluation-link`) are **DELETED** from the dashboards — the Start-Next-Season form / owner-evaluation entry relocate to the `Play ▾` topnav (`topbar-play-next-season` / `topbar-play-owner-evaluation`; see **NAV-01** above). Post-NAV-01 `TestLg01eDashboardWiring` asserts these dashboard ids ABSENT.
...
## NAV-01 `Play ▾` top-nav dropdown (single advancement surface)

**Relocate-not-duplicate** — the league-mode `Play ▾` top-nav dropdown becomes the **SOLE** league-advancement surface. All advancement controls (Start Season / One Week / Two Months / Until End of Season / One Week Live / Start Next Season / owner-evaluation entry / Play Single Round / Play Playoffs) move OUT of both LG-01c/d/e/i/Part2c-1 dashboards into the league branch of `templates/base.html`; the dashboards keep only their read-only panels (standings snippet / leaders / next-round / round-count / map-config label, LG-01c/j), the playoff **View bracket** link (→ `tournament_detail`, Part2c-1), the CAR-02 **View past evaluations** link, and the LG-01d `play_error` banner. **League-mode only** — rendered in the `{% if app_mode == "league" %}` branch of `base.html` (the LG-01k path-prefix rule: `request.path` under `/leagues/` or `/seasons/`). The nav advances the league's **resolved active/displayed Season** via the existing `core.context_processors.league_nav` resolution chain (session `last_league_id` → single-League → list-page fallback for the League; `league.active_season` → most-recent completed → `None` for the displayed Season) — **NOT** the `season_id` in the URL (the topnav has no `season`/`league` template var; it is league-scoped, not page-scoped). **No model change, no migration, no new URL routes, no new view functions** — all 10 play endpoints reused verbatim; the only code edit beyond templates is the `_build_play_controls_context` factor-out + the `league_nav` extension. **Pure view-context + template relocation → no simulator touch, no RNG, no `_flush_to_db` touch, no SIM-07/08 interaction, no Score Calibration re-baseline, no CONTEXT.md edit** (NAV-01 adds no domain term — it is a UI-surface relocation). Decision: [ADR-0030](../../docs/adr/0030-play-controls-relocated-to-topnav.md). Seam contract: [`.claude/worktrees/nav-01-seam-contract.md`](../../.claude/worktrees/nav-01-seam-contract.md).

**Shared helper (factored OUT of `_build_dashboard_context`).** A new module-level helper `matches.league_views._build_play_controls_context(league, displayed_season) -> dict` (RV-01 / HX-03 / LG-01c `_`-prefixed flat-helper precedent) computes the play-control state for the **resolved** league + displayed Season and returns **9 play keys**: `action_button_label` (`str`) / `action_button_state` (`str` — `"none"` / `"start_season"` / `"play_next"` / `"start_next_season"`) / `playoff_phase_active` (`bool`) / `playoff_tournament_id` (`int | None`) / `playoff_completed` (`bool`) / `has_following_tournament_phase` (`bool`) / `following_tournament_is_final` (`bool`) / `live_preview_available` (`bool`) / `is_career_mode` (`bool`). The playoff keys derive from `_playoff_cursor_keys(displayed_season)`, `live_preview_available` from `_resolve_live_cursor(...)` (LG-01i) when `season_mode == "active"`, `is_career_mode` from `_is_career_league(league)`; `season_mode` is derived inside the helper from `displayed_season.state` / `None` (the helper takes the **already-resolved** displayed Season — it does NOT re-implement the resolution chain). After the factor-out **`_build_dashboard_context` STOPS emitting those 9 play keys**; it KEEPS `playoff_tournament_id` (the read-only dashboard View-bracket link still needs it) plus the read-only body keys (`displayed_season` / `season_mode` / `standings_snippet` / `next_fixture` / `round_count_completed` / `round_count_total` / `leaders_points` / `leaders_tags` / `leaders_ratio` / `map_config_label`). The LG-01d `play_error` / `play_job_id` context keys STAY on the dashboard view (the banner is dashboard-rendered on a sync-error re-render).

**Processor extension — `core.context_processors.league_nav`.** The LG-01k 2-key processor (`top_bar_links` / `top_bar_dashboard_url`) is EXTENDED: after the existing 3-step League + displayed-Season resolution, **GATED on the league path-prefix** (same `app_mode == "league"` test — skip the work on sandbox/start pages), it calls `_build_play_controls_context(league, displayed_season)` via a **lazy local import inside the function body** (the LG-01f `core ↔ matches` apps-loading-cycle guard) and merges its 9 keys into the return dict alongside the LG-01k keys, **plus two reverse-helper ids** the topnav forms need to build their `{% url %}` reverses: `play_displayed_season_id` (`int | None`, `displayed_season.id`) + `play_league_id` (`int | None`, `league.id`). The play keys are **ABSENT** off-league (sandbox/start path) AND on the `_fallback()` path (no resolvable league) — the `Play ▾` renders only in the league branch, which only renders when `app_mode == "league"`, so absent keys are never read. `top_bar_links` / `top_bar_dashboard_url` and the fallback shape are unchanged.

**Base.html — added league-branch markup + relocated poll JS.** Inside the `{% if app_mode == "league" %}` branch of the `<div class="navbar-nav ms-auto">`, a `Play ▾` dropdown is added immediately AFTER the LG-01k `dashboard-nav-link` ⌂ icon and BEFORE the `League ▾` section dropdown — final order `⌂ | Play ▾ | League ▾ | Team ▾ | Players ▾ | Stats ▾ | Tools ▾ | Help ▾`. The dropdown body branches on `action_button_state` and uses `play_displayed_season_id` / `play_league_id` for its `{% url %}` reverses; the `"none"` state renders a disabled "Play" affordance. The **terminal relabel** of the Until-End button text is unchanged behaviour (`Play Until Playoffs` / `Play Until Tournament` / `Play Until End of Season` via `has_following_tournament_phase` / `following_tournament_is_final`, mirroring Part2c-3c). The navbar `topbar-play-progress` / `topbar-play-error` elements live alongside the dropdown (always present in the league branch, hidden by default). The **inline poll `<script>` is relocated to ONE copy** in the league branch (collapsed from the two per-dashboard LG-01d/Part2c-1 blocks), preserving the LG-01d DOM contract — function names `interceptAsync(form)` / `startPolling(jobId)` / `showProgress(completed, total)` / `clearPolling()` / `setDropdownDisabled(disabled)` / `ensureErrorEl()` — re-targeted at the `topbar-play-progress` (inner `.play-progress-spinner` / `.play-progress-label` / `.play-progress-bar`) / `topbar-play-error` hooks, disabling `topbar-play-dropdown` while a run polls. (Optional, Code-agent discretion: factor the dropdown body into `templates/_partials/topnav_play.html` and the poll JS into `templates/_partials/topnav_play_script.html`, ONE include each in the league branch — only the DOM ids are pinned, not the inclusion structure.)

**Endpoints reused verbatim — NO new routes, NO new views.** All 10 play endpoints are submitted from the topnav unchanged: `start_season` / `play_week` / `play_single_round` / `next_season` (sync → `redirect("season_dashboard", …)` ⇒ 302), `play_two_months` / `play_until_end` / `play_playoffs` (async → `JsonResponse(..., status=202)`), `play_week_live` (sync, gated `live_preview_available`), `play_status` (the 5-key poll JSON), `owner_evaluation` (career GET link). Their 302 / 202-JSON targets do not depend on the request origin, so a topnav submit works with no view tweak. Sync errors still land on the dashboard `play_error` banner (`_render_season_dashboard_error(...)` → 400 dashboard re-render, the LG-01d/decision-#8 path unchanged); the async trio's 409/400 guard JSON surfaces via `topbar-play-error`.

**PLAY-01 deferral.** NAV-01 ships **progress-display only** — the async trio reuse `play_status` + `_build_play_status_response` + `_celery_state_to_job_status` verbatim, per-page poll, no cross-navigation resume. The **Play→Stop swap, cancel/revoke (`AsyncResult.revoke`), live incremental standings/leaders during a run, and cross-page resumable progress are DEFERRED to PLAY-01.**

**Locked names (quick index).** Shared helper `matches.league_views._build_play_controls_context(league, displayed_season) -> dict` (9 play keys: `action_button_label` / `action_button_state` / `playoff_phase_active` / `playoff_tournament_id` / `playoff_completed` / `has_following_tournament_phase` / `following_tournament_is_final` / `live_preview_available` / `is_career_mode`); `_build_dashboard_context` DROPS those 9 keys, KEEPS `playoff_tournament_id` + the read-only body keys. Processor `core.context_processors.league_nav` adds the 9 play keys + `play_displayed_season_id` (`int | None`) + `play_league_id` (`int | None`) on the league-prefix path; ABSENT off-league / on fallback. NEW nav DOM ids — toggle `play-nav-link`; wrapper `topbar-play-dropdown`; items `topbar-play-start-season` (→ `start_season`) / `topbar-play-one-week` (→ `play_week`) / `topbar-play-two-months` (→ `play_two_months`) / `topbar-play-until-end` (→ `play_until_end`) / `topbar-play-one-week-live` (→ `play_week_live`, gated) / `topbar-play-owner-evaluation` (→ `owner_evaluation`, career GET link) / `topbar-play-next-season` (→ `next_season`, non-career) / `topbar-play-play-single-round` (→ `play_single_round`) / `topbar-play-play-playoffs` (→ `play_playoffs`); progress `topbar-play-progress` (+ inner `.play-progress-spinner` / `.play-progress-label` / `.play-progress-bar`); error `topbar-play-error`; each nav `<form>` carries `{% csrf_token %}` + `data-action-state="{{ action_button_state }}"` (test-hook parity with the dashboards). RETIRED dashboard ids (DELETED) — the full `{season,league}-dashboard-play-*` advancement set (`-play-dropdown` / `-play-start-season` / `-play-one-week` / `-play-two-months` / `-play-until-end` / `-play-one-week-live` / `-play-single-round-form` / `-submit` / `-play-playoffs-form` / `-submit` / `-progress` / `-play-progress`) + `-owner-evaluation-link` + `-next-season-form` + `-action-button` wrapper `<span>` + both inline poll `<script>` blocks. KEPT read-only dashboard ids — `{season,league}-dashboard-state-badge` / `-view-bracket-link` / `-past-evaluations-link` / `-play-error` + all standings / next-round / round-count / leaders / map-config ids (LG-01c/j). Reused-verbatim URL names — `start_season` / `play_week` / `play_two_months` / `play_until_end` / `play_week_live` / `play_single_round` / `play_playoffs` / `play_status` / `next_season` / `owner_evaluation`. Files modified — `laserforce_simulator/matches/league_views.py` (helper factor-out) + `laserforce_simulator/core/context_processors.py` (processor extension) + `laserforce_simulator/templates/base.html` + `templates/seasons/dashboard.html` + `templates/leagues/dashboard.html` (relocation). Tests — NEW `matches/tests/test_nav_play_dropdown.py` (league-branch DOM-id state matrix), EXTENDED `matches/tests/test_league_nav_context_processor.py` (the 9 play keys + the two ids present on a league-prefix request, absent off-league / on fallback); the LG-01d/01e/01i/Part2c-1 dashboard-play assertions rewritten to assert the RETIRED ids ABSENT + the KEPT read-only ids PRESENT. ADR: [ADR-0030](../../docs/adr/0030-play-controls-relocated-to-topnav.md). Seam contract: [`.claude/worktrees/nav-01-seam-contract.md`](../../.claude/worktrees/nav-01-seam-contract.md).

## PLAY-01 live incremental stats + Stop/Cancel

Layers onto the NAV-01 single `Play ▾` advancement surface (`templates/_partials/topnav_play.html` + `templates/_partials/topnav_play_script.html`). Adds (1) a **cooperative between-fixture cancel** + a **Stop** control that swaps in for the async Play forms while a run polls, and (2) **live incremental** standings/leaders that recompute view-side from committed rows each poll and patch the dashboard panels by id. **Scope: ASYNC runs only** — `play_two_months` / `play_until_end` / `play_playoffs`; the sync paths (`start_season` / `play_week` / `play_single_round` / `play_week_live` / `next_season`) are untouched. **NO `AsyncResult.revoke`** (a non-terminating revoke can't stop a running task; a terminating revoke would leave the half-committed Round ADR-0016 forbids — cooperative DB-flag cancel only). **No simulator/RNG touch → no Score Calibration re-baseline.** Transport is the EXISTING NAV-01 polling rail (the `play_status` poll); **WebSockets/Channels are deferred** (no cancel-latency win — the safe-stop granularity is the fixture boundary, not the network round-trip — and they'd need an ASGI/Channels/deploy migration). Decision: [ADR-0031](../../docs/adr/0031-cooperative-cancel-and-live-polling-stats.md) (reverses the ADR-0013 / ADR-0016 cancel scope-out; the `revoke` rejection stands). Seam contract: [`.claude/worktrees/play-01-seam-contract.md`](../../.claude/worktrees/play-01-seam-contract.md).

**Model fields (2 NEW on `Season`) + migration.** `Season.active_play_job_id` (`models.CharField(max_length=255, null=True, blank=True, default=None)`) — the Celery job id of the in-flight async run; an async enqueue view SETS it, the task CLEARS it (`= None`) in its `finally`. `Season.play_cancel_requested` (`models.BooleanField(default=False)`) — the cooperative cancel flag; the async enqueue views CLEAR it (`= False`), `play_cancel` SETS it (`= True`), the task READS it (top + between fixtures) and never writes it. Both appended after `starting_map_rotation_ids_json` (the SUB-01 last field), before `__str__`. Migration `matches/migrations/0056_season_play_job_cancel.py` (dep `0055_gameround_fidelity_roster_snapshot`), exactly **2× `AddField`** (`active_play_job_id` then `play_cancel_requested`), **NO `RunPython` / `RunSQL` / backfill** (ADR-0004 — existing Seasons take `null` / `False`). Render/poll treats a **terminal** `AsyncResult` (status not `running`) as "no active run" regardless of the column — defends against a crashed worker that never cleared `active_play_job_id`.

**Cancel view + URL (NEW).** `matches.league_views.play_cancel(request, season_id) -> JsonResponse` — POST-only (`if request.method != "POST": return HttpResponseNotAllowed(["POST"])`, the LG-01d idiom), then `get_object_or_404(Season, pk=season_id)`, write `request.session["last_league_id"] = season.league_id`, set `play_cancel_requested = True` + `save(update_fields=["play_cancel_requested"])`, return `JsonResponse({"cancelled": True, "season_id": season.id})` (HTTP **200**). GET → 405; missing Season → 404 (the 405 guard precedes any ORM hit). No active-run guard — setting the flag on an idle Season is harmless (the next run clears it at enqueue; nothing reads a stale `True`). URL name `play_cancel`, path `/seasons/<int:season_id>/play-cancel/`, added to `matches/season_urls.py` among the play routes (after `play_status`, before `season_standings` / `season_schedule`).

**Extended `play_status` JSON — `_build_play_status_response` (CHANGED).** `matches.league_views._build_play_status_response(async_result, *, season_id) -> dict` keeps its existing **5 keys unchanged** (`status` / `completed` / `total` / `error` / `season_id`) and `_celery_state_to_job_status` is reused verbatim (no new status string). ADDS 3 keys: `standings` (the live partial standings as a **server-rendered HTML fragment**), `leaders` (a **dict of 3 server-rendered HTML fragments** keyed `{"points": <html>, "tags": <html>, "ratio": <html>}`), `cancelled` (`bool` — `True` when the task RETURNED a result dict carrying `cancelled: true`, read off `async_result.result["cancelled"]` on SUCCESS; `False` otherwise — the status stays `complete`). The recompute runs **VIEW-SIDE from committed rows on EVERY poll** (running OR complete, so the final poll patches the finished tables too) via the same helpers the dashboards use: `matches.standings.compute_standings(completed_matches, enrolled_teams, season_rounds=None) -> list[StandingsRow]` over `Match.objects.filter(season=season, is_completed=True)`, and `matches.season_dashboard.compute_leaders(player_rounds, stat, limit=3) -> list[LeaderRow]` over `PlayerRoundState.objects.filter(game_round__match__season=season)` for each of `"points_per_game"` / `"tags_per_game"` / `"tag_ratio"` — **NOT** read from Celery task meta. The recommended single-source mechanism (only the JSON keys + the patched DOM ids are LOCKED): extract the standings + the three leaders snippet markup into shared partials `templates/_partials/dashboard_standings_snippet.html` + `dashboard_leaders_snippet.html`, `{% include %}`d by both dashboards AND rendered by `_build_play_status_response` via `django.template.loader.render_to_string(...)` fed the same `standings_snippet` / `leaders_points` / `leaders_tags` / `leaders_ratio` context, so the dashboard and the live patch render byte-identical HTML.

**Enqueue-time edits to the 3 async views (CHANGED).** `play_two_months` / `play_until_end` / `play_playoffs` keep their existing 405 guard / `last_league_id` write / active-state (or built-tournament) guard VERBATIM; the only change — between the guard and the existing `.delay(...)`, then on the returned result — sets `active_play_job_id = result.id` + `play_cancel_requested = False` + `save(update_fields=["active_play_job_id", "play_cancel_requested"])` before the UNCHANGED `JsonResponse({"job_id": result.id, "season_id": season.id}, status=202)`. (Clear cancel + record the job id at enqueue; the 202 shape is unchanged. `play_playoffs` uses `play_playoffs_task.delay(season.id)` per its own body — only the two-field assignment + save is inserted.)

**Task control-flow (`matches/tasks.py`).** NEW module-level helper `matches.tasks._play_cancel_requested(season_id: int) -> bool` — `Season.objects.filter(id=season_id, play_cancel_requested=True).exists()` (single-column exists query, no instance materialised, re-read each call so a mid-run `play_cancel` POST is observed; deferred `from matches.models import Season` per the file's deferred-import precedent). Checked at **two points** in BOTH `play_season_task` (`name="matches.play_season"`) and `play_playoffs_task` (`name="matches.play_playoffs"`): (1) **task top** (after loading the Season, BEFORE the fixture/stage loop) — early-return `{"completed": 0, "total": <n or 0>, "cancelled": True}`; (2) **between fixtures** (TOP of each per-fixture iteration in `play_season_task` / per-stage drain iteration in `play_playoffs_task`, BEFORE `simulate_scheduled_round(...)` / `play_next_bracket_round(...)`) — **break** cleanly and return `{"completed": <k so far>, "total": <n>, "cancelled": True}`. The early return is a **NORMAL return** ⇒ Celery records **SUCCESS** ⇒ `play_status` maps it to `complete` (no new status string); already-played Rounds stay committed (per-Round atomic commit, ADR-0016), the Season stays `active` and is resumable (re-click Play resumes via the existing `played_keys` / `find_next_playable_node` skip). In BOTH tasks' existing `finally:` block, alongside `django.db.close_old_connections()`, clear the active-run column best-effort: `from matches.models import Season; Season.objects.filter(id=season_id).update(active_play_job_id=None)` (`.update(...)` not `.save()` — touches only the one column; fires on success, cancel-return, AND failure so a crashed run never leaves a stale `active_play_job_id`). The cancel flag is NOT cleared here (it's cleared at the next enqueue).
...
A 4th `Season.map_mode` value `rotate_by_matchday`: a whole **Season-level author-ordered** `ArenaMap` list (NOT a per-sub-league rotation — the originally-sketched `SubLeague` model is DEFERRED to a later slice after LG-07, and NO `SubLeague` model ships here). A Round's map = `ordered_ids[fixture.matchday % len(ordered_ids)]`, keyed on **matchday ALONE** — every fixture on matchday N uses the same arena, and because a Match's two Rounds sit on different matchdays, the two Rounds of a Match rotate onto different arenas. Fully deterministic, **NO RNG** (contrast `random_per_round`'s per-fixture `random.Random` seed — `rotate_by_matchday` is pure modular indexing, no seed chain touched), **no simulator change**, **no Score Calibration re-baseline obligation** (folds into the existing pending post-MOVE-01 re-baseline like LG-01j). Tournament / playoff fixtures are UNAFFECTED — they don't call `_resolve_fixture_map`. Slots in alongside the LG-01j 3 modes (`none` / `single` / `random_per_round`) as the 4th, mirroring LG-01j's seam discipline verbatim.

**Model fields.** `Season.MAP_MODE_CHOICES += ("rotate_by_matchday", "Rotate by matchday")` (4th enum tuple appended). TWO NEW fields: (1) `Season.map_rotation_ids_json` — the LIVE author-ordered list, `JSONField(null=True, blank=True, default=None)` (`None` pre-population, `[]` no-maps, `[id1, id2, …]` author order — NOT id-sorted); (2) `Season.starting_map_rotation_ids_json` — the activation snapshot, the **order-preserving twin** of LG-01j's `starting_map_pool_ids_json` (that one is `sorted()` ascending; THIS one PRESERVES author order, since rotation index depends on order). The LG-01j `map_pool` M2M is EMPTY for this mode (rotation lives entirely in the JSON fields, not the M2M). `Season.clean()` enum check UNCHANGED (Django `CharField.choices` already covers the new literal; the defensive set-membership guard from LG-01j auto-accepts it once the choice tuple is extended).

**Migration `matches/migrations/0054_season_map_rotation.py`** (dep `0053_fin05_luxury_tax_firing`): exactly 1× `AlterField(map_mode)` (widened `choices`) + 2× `AddField` (`map_rotation_ids_json`, `starting_map_rotation_ids_json`) — pure schema, NO `RunPython` / NO backfill (ADR-0004 — pre-SUB-01 Seasons take `map_mode` default `"none"`, `None` rotation snapshots, yielding 3-zone fallback at sim time, exactly the LG-01d/LG-01j preserved behaviour).

**`_resolve_fixture_map` NEW branch (`matches/tasks.py`).** Appends a `rotate_by_matchday` branch to the LG-01j 4-branch ladder: reads `ids = season.starting_map_rotation_ids_json or []`; defensive `if not ids: return None`; `return pool_by_id.get(ids[fixture.matchday % len(ids)])` — `None` on empty / admin-deleted arena. **NO RNG**, no `random.Random` construction in this branch. Purity preserved — still no ORM inside the helper; the new branch consumes only `season.starting_map_rotation_ids_json` + `fixture.matchday` + the passed `pool_by_id`. **THREE play-loop call sites** union BOTH snapshots into ONE `in_bulk` (so `pool_by_id` covers pool-mode and rotation-mode ids in a single query): `tasks.play_season_task`, `league_views.play_week`, `league_views.play_week_live` (the LG-01i RR live branch). Each builds `pool_by_id = ArenaMap.objects.in_bulk((season.starting_map_pool_ids_json or []) + (season.starting_map_rotation_ids_json or []))` once, OUTSIDE the per-fixture loop.

**`Season.start_season()` extension.** APPENDS ONE line alongside the LG-01j pool snapshot, inside the existing `@transaction.atomic`: `self.starting_map_rotation_ids_json = list(self.map_rotation_ids_json or [])` — author order PRESERVED (NOT `sorted()` — the deliberate divergence from the LG-01j pool snapshot); empty / `None` ⇒ `[]` (activated-with-no-maps, distinct from pre-activation `None`).

**`CreateLeagueForm` extension.** NEW hidden field `map_rotation` (widget DOM id `league-create-map-rotation`) carrying the composer payload. Inline order-preserving `clean()` parse → `cleaned_data["map_rotation_ids"]` (an ORDERED `list[int]`, duplicates allowed, order = author order from the composer); each id validated against `_maps_with_confirmed_config()` (reuses the LG-01j helper). **FULL 4×2 cross-guard validation matrix** across all 4 modes: every "off" mode requires its input EMPTY, every "on" mode requires its input the correct count — `rotate_by_matchday` requires rotation list ≥ 1 AND `map_pool` empty (mirrors the LG-01j `random_per_round` ≥ 1 rule but on the rotation list, with the pool forced empty).

**`league_create` / `next_season` view extensions.** `league_create` writes `map_rotation_ids_json = cleaned["map_rotation_ids"]` (author order). `next_season` carries `latest_completed.map_rotation_ids_json` forward VERBATIM (read from the live field — mirrors the LG-01j `map_mode` carry-forward), persisted via `save(update_fields=["map_rotation_ids_json"])`.

**`_build_map_config_label` 5th branch.** Appends to the LG-01j 4-branch ladder: `displayed_season.map_mode == "rotate_by_matchday"` ⇒ resolve ids from live `map_rotation_ids_json` (draft) or `starting_map_rotation_ids_json` (active / completed) — in AUTHOR order (NOT name-sorted, NOT id-sorted); non-empty ⇒ `f"Map: Rotating ({n} maps: {comma_joined_names_in_author_order})"`; empty ⇒ `"Map: Rotating (no maps)"`. Dashboards already render `{{ map_config_label }}` from LG-01j — **NO dashboard template edit**.

**Template `templates/leagues/create.html`.** Adds a vanilla-JS "+ Add map" composer (the LG-02b composer pattern — no JS framework, no inline `<script>` beyond the LG-02b-style vanilla block). DOM ids: composer container `league-create-map-rotation-composer`, add button `league-create-add-map`, per-row `league-create-map-rotation-row-{i}`, per-row select `league-create-map-rotation-select-{i}`, and the hidden aggregate field `league-create-map-rotation`. The JS serialises the ordered selects into the hidden field on submit. **No `SeasonAdmin` change** (the new `map_mode` literal surfaces via the existing default form render; `filter_horizontal` is unchanged since `map_pool` stays empty for this mode).

**Scope-out (locked).** **NO `SubLeague` model** (deferred to a later slice after LG-07 — `rotate_by_matchday` is Season-level only). **NO RNG / no seed-chain touch** (pure modular indexing). **NO simulator edit** (reuses the SIM-09 `arena_map=` kwarg via the same three play loops). **NO Score Calibration re-baseline obligation** (folds into the pending post-MOVE-01 re-baseline). **NO `RunPython` / backfill** (ADR-0004). **NO tournament / playoff impact** (those fixtures never call `_resolve_fixture_map`). **NO dashboard template edit** (LG-01j `{{ map_config_label }}` already renders). **NO new ADR / no new CONTEXT.md section** (the rotation vocabulary extends the existing `### League and seasons` entries — reversible model fields + a deterministic helper branch, mirroring LG-01j's no-ADR rationale).

**Locked names.** Model: enum literal `"rotate_by_matchday"` (display `"Rotate by matchday"`, appended to `Season.MAP_MODE_CHOICES`); NEW fields `Season.map_rotation_ids_json` (JSONField, null=True, blank=True, default=None, author-ordered live list) / `Season.starting_map_rotation_ids_json` (JSONField, null=True, blank=True, default=None, activation snapshot, author order PRESERVED — the order-preserving twin of `starting_map_pool_ids_json`); migration filename `matches/migrations/0054_season_map_rotation.py` (dep `0053_fin05_luxury_tax_firing`, 1× `AlterField` + 2× `AddField`, NO `RunPython`); EXTENDED helper `matches.tasks._resolve_fixture_map` (NEW `rotate_by_matchday` branch, NO RNG: `pool_by_id.get(ids[fixture.matchday % len(ids)])`); rotation-index formula (byte-locked) `ordered_ids[fixture.matchday % len(ordered_ids)]` (keyed on `fixture.matchday` ALONE); THREE EXTENDED play-loop call sites `matches.tasks.play_season_task` / `matches.league_views.play_week` / `matches.league_views.play_week_live` (union both snapshots into one `ArenaMap.objects.in_bulk(...)`); EXTENDED `Season.start_season()` (APPENDS `self.starting_map_rotation_ids_json = list(self.map_rotation_ids_json or [])` — author order, NOT sorted); form field (1 NEW on `CreateLeagueForm`) `map_rotation` (hidden, widget id `league-create-map-rotation`) → `clean()` emits ordered `cleaned_data["map_rotation_ids"]` (`list[int]`, author order, validated against `_maps_with_confirmed_config()`); FULL 4×2 cross-guard matrix in `CreateLeagueForm.clean()` (`rotate_by_matchday` ⇒ rotation ≥ 1 + pool empty); EXTENDED views `matches.views.league_create` (writes `map_rotation_ids_json`) / `matches.views.next_season` (carries `map_rotation_ids_json` forward verbatim via `save(update_fields=["map_rotation_ids_json"])`); EXTENDED label helper `_build_map_config_label` (5th branch); label string literals (2 NEW, byte-equal) `"Map: Rotating ({n} maps: {comma_joined_names})"` / `"Map: Rotating (no maps)"` (AUTHOR order); template (1 MODIFIED) `templates/leagues/create.html` (vanilla-JS LG-02b composer); DOM ids (5 NEW) `league-create-map-rotation-composer` / `league-create-add-map` / `league-create-map-rotation-row-{i}` / `league-create-map-rotation-select-{i}` / `league-create-map-rotation`; tests (6 EXTENDED, 0 NEW files) `matches/tests/test_season_map_config.py` + `test_league_create.py` + `test_league_next_season.py` + `test_league_play.py` + `test_league_dashboard.py` + `test_season_dashboard_view.py`; admin UNCHANGED (no `SeasonAdmin` edit — `map_pool` stays empty for this mode). Seam contract: [`.claude/worktrees/sub-01-rotate-by-matchday-seam-contract.md`](../../.claude/worktrees/sub-01-rotate-by-matchday-seam-contract.md).
...
forms POST to `play_single_round` / `play_playoffs`. *(RETIRED by NAV-01: the playoff Play forms `{season,league}-dashboard-play-single-round-form` / `-submit` / `-play-playoffs-form` / `-submit` / `-progress` are **DELETED** from the dashboards and relocate to the `Play ▾` topnav as `topbar-play-play-single-round` / `topbar-play-play-playoffs`; the `{season,league}-dashboard-view-bracket-link` read-only link is **KEPT** on the dashboard, and `playoff_phase_active` / `playoff_completed` / `has_following_tournament_phase` / `following_tournament_is_final` now come from `_build_play_controls_context`. See **NAV-01** above.)* **Conditional terminal-label
...
A **read-only / derived** league screen: per-Season awards recomputed **on render
(transient)** from frozen `PlayerRoundState` rows — **NO model change, NO migration,
NO simulator change, NO Score Calibration re-baseline, NO persisted award rows, NO
caching**. Mirrors the `matches/season_player_stats.py` / `matches/league_leaders_logic.py`
pure-module precedent and the LG-01z league-screen view shell. A new Django-free pure
module `matches/season_awards.py` computes the award set; the view does ALL ORM work and
hands the pure fn a flat `list[dict]`. The **regular-season corpus** is
`PlayerRoundState.objects.filter(game_round__match__season=season)` — season-embedded
**playoff** Matches carry `season=NULL` (the Part2c-1 #3 decision) and are therefore
naturally **excluded** from the regular-season awards; **Finals MVP** is computed
separately over the championship bracket node's rounds (below). Extends no ADR (no ADR
for this slice) and adds **no new CONTEXT.md term** (the **Season award** / **Season MVP**
/ **Finals MVP** glossary entries were finalised during grilling). Seam contract:
[`.claude/worktrees/lg-03-season-awards-seam-contract.md`](../../.claude/worktrees/lg-03-season-awards-seam-contract.md).

**Pure module `matches/season_awards.py`.** Frozen import allowlist — `dataclasses`,
`typing`, `collections` ONLY; **NO** Django / ORM / `random` / `datetime` / I/O / logging,
defended by `matches/tests/test_season_awards.py::TestNoDjangoImportsLeaked` (subprocess
fresh-import + `sys.modules` walk, mirroring `season_player_stats.py` /
`league_leaders_logic.py`). It declares its OWN `frozen` dataclasses (does **not** reuse
`LeaderRow`): `AwardWinner(player_id, player_name, role, team_id, team_name, value)` (6
fields, pinned order) and `AwardSet(most_points, best_accuracy, kd_by_role, best_medic,
most_efficient_nuke, season_mvp, finals_mvp)` (7 fields, pinned order). Every winner slot
is `Optional[AwardWinner]` → `None` when no qualifying player; `kd_by_role` is a mapping
with **exactly 5 keys**, one per role (`commander`/`heavy`/`scout`/`medic`/`ammo` — the
strings stored on `PlayerRoundState.role`), each an `Optional[AwardWinner]`. The view
builds one seam dict per round-appearance with the required keys `player_id, player_name,
role, team_id, team_name, points_scored, tags_made, times_tagged, accuracy, mvp,
resupplies_given, specials_used, own_specials_cancelled` — `accuracy` pre-computed from
`PlayerRoundState.get_accuracy` (a `@property`, NO parens, int 0–100) and `mvp` from
`PlayerRoundState.get_mvp` (a `@property`, NO parens, `float`), both stored as `float(...)`
per the `season_player_stats` precedent.

**The 7 awards + metrics (pinned).** `compute_season_awards(player_rounds, *, min_games)`
groups rows by `player_id` (last-row-wins for displayed name/role/team, rows passed
id-ascending) and fills six slots; the view stamps the seventh (`finals_mvp`) separately
(rebuilding the set, e.g. `dataclasses.replace(...)`). Metrics: `most_points` =
`SUM(points_scored)` (any role); `best_accuracy` = `MEAN(accuracy)` (any role);
`kd_by_role[role]` = `SUM(tags_made) / max(SUM(times_tagged), 1)` (per role, one winner
each → the 5 entries); `best_medic` = `SUM(resupplies_given)` (`role == "medic"` only);
`most_efficient_nuke` = `(SUM(specials_used) − SUM(own_specials_cancelled)) /
max(SUM(specials_used), 1)` (`role == "commander"` only); `season_mvp` = `MEAN(mvp)` (any
role). **Rate-award qualifier (LOCKED):** the **mean / rate** awards — `season_mvp`,
`best_accuracy`, `most_efficient_nuke` ONLY — require `games(player) >= min_games`; the
**total / count** awards (`most_points`, `best_medic`, `kd_by_role`) are **NOT gated**. The
view passes `min_games = ceil(max_games_any_player / 2)` (`max_games_any_player` = max
per-player row count over the corpus; `0`/empty ⇒ `min_games = 0` and every slot `None`,
all 5 `kd_by_role` entries `None`). **Tiebreak ladder (deterministic):** primary metric
value → `games_played` desc → `player_id` asc (the `league_leaders_logic._rank` shape).

**Finals MVP (`pick_finals_mvp`) — bracket-format-only.** `finals_mvp` is set **only when**
the Season has a tournament/playoff phase **AND** that tournament's `format` is a **bracket**
format — `{single_elimination, double_elimination, round_robin_double_elim}`; for
`round_robin` / `swiss` phases and no-playoff Seasons it is `None`. Navigation (per
`matches/models.py`): `season.ordered_phases()` → the `SeasonPhase` with
`phase_type == "tournament"` and `tournament_id is not None` → `phase.tournament`, then check
`tournament.format`. **Deciding node = the terminal bracket node won by the champion:** for
`single_elimination` and `round_robin_double_elim`, the `BracketNode` with
`advances_to_id is None` whose `winner_id == tournament.champion_id`; for
`double_elimination`, the grand-final node **GF2** (`bracket_type == "grand_final"`,
`advances_to_id is None`), or **GF1** when GF2 is inert/bye (the bracket reset was skipped —
GF2 `is_bye` / no real Series). The Finals-MVP rounds are ALL `GameRound`s of ALL
`SeriesMatch` rows on the deciding node (`BracketNode.series_matches` → `SeriesMatch.match` →
`match.game_rounds` → `PlayerRoundState`); `pick_finals_mvp(final_round_dicts)` returns the
`AwardWinner` with the best `MEAN(get_mvp)` over those rounds (same tiebreak ladder; empty
input ⇒ `None`). The view stamps the result onto the `AwardSet`.

**View + URL + page shell.** `matches.league_views.season_awards(request, season_id)` at
URL name **`season_awards`** / path **`/seasons/<int:season_id>/awards/`** (added to
`matches/season_urls.py` alongside `season_standings` / `season_schedule`, bare name, no new
include). It `get_object_or_404(Season, …)` (404 on missing), LOCKS a GET-only 405 guard
(`HttpResponseNotAllowed(["GET"])`) as its first line, writes
`request.session["last_league_id"] = season.league_id`, and renders
`templates/seasons/awards.html` (extends `base.html`, the `d-flex` +
`_partials/league_sidebar.html` shell) with `sidebar_links =
_build_league_sidebar_links(league, displayed_season, sidebar_active=None)` (no sidebar
entry matches the awards page, the `player_detail` precedent). It resolves teams via
`team_color` against `game_round.team_red`/`team_blue`, mirroring
`matches.league_screens.player_stats._build_round_dicts`. **LOCKED DOM ids:**
`season-awards-table` (outer table, present when ≥ 1 winner), `season-awards-most-points`,
`season-awards-best-accuracy`, `season-awards-kd-{role}` (the 5 — `kd-commander`/`kd-heavy`/
`kd-scout`/`kd-medic`/`kd-ammo`), `season-awards-best-medic`,
`season-awards-most-efficient-nuke`, `season-awards-season-mvp`, `season-awards-finals-mvp`,
and `season-awards-empty-notice` (when the Season has no completed regular-season rounds). A
`None` winner renders the em-dash `—` (the LG-01f / `player_detail` convention). Entry-point
links: `season-dashboard-awards-link` on `templates/seasons/dashboard.html` and
`league-history-awards-link-{season_id}` per row on `templates/leagues/history.html` (both
`{% url 'season_awards' … %}`).

**League History row — `_build_history_row` 11 → 13 keys (blast radius).**
`matches.league_views._build_history_row(season, teams_by_id, *, is_in_progress)` grows from
**11 keys** to **13** — appends `season_mvp` and `finals_mvp` (each `AwardWinner | None`).
**REUSE policy (LOCKED):** the History row **reuses `compute_season_awards`** (building the
same regular-season seam dicts) and takes only its `.season_mvp`, plus the `pick_finals_mvp`
path for `.finals_mvp` — NOT a lighter bespoke helper; the regular-season-dicts +
finals-corpus assembly is factored out of the `season_awards` view into a **shared private
helper** so the view and the History row call ONE path. Query cost is accepted: at most 10
extra `PlayerRoundState` queries on the paginated (10-row) History page. The template
(`templates/leagues/history.html`) gains **2 rightmost columns** — Season MVP and Finals MVP —
appended after the top-3 / Tournament-Champion cells, with cell DOM-id substrings
`league-history-season-mvp-{season_id}` / `league-history-finals-mvp-{season_id}` (`None` →
`—`). **Blast radius:** `matches/tests/test_league_history.py` — the existing 11-key shape and
per-row column-count assertions (`TestLeagueHistoryCompletedRows`) break and are updated to the
13-key / +2-column shape.

**League player page — `league-player-awards-stub` → live `league-player-awards`.**
`matches.league_screens.player_detail.player_detail` adds a new context key `player_awards:
list[dict]` — one entry per Season in which this player won ≥ 1 award in THIS league:
`{"season_id": int, "season_name": str, "award_labels": list[str]}` (`award_labels` = the
human labels of the awards the player won that Season, e.g. `["Season MVP", "Best Medic",
"K/D — Scout", "Finals MVP"]`). It iterates the league's Seasons, runs the same shared helper
(`compute_season_awards` + the finals-MVP path) per Season, and keeps the awards whose
winner's `player_id == player.id`. `templates/leagues/player_detail.html` **replaces** the
existing `league-player-awards-stub` block (the "Coming soon — awaiting an awards model."
placeholder) with a live element that keeps the DOM id **`league-player-awards`** (the
`-stub` suffix is removed), rendering one block per Season; when `player_awards` is empty it
renders an empty notice inside `league-player-awards`. The global HX-01 career page
(`player_career_stats` / `/players/<id>/stats/`) stays **UNCHANGED**.

**Scope-out (LOCKED).** NO `PlayerRoundState` / `Match` / any model field, NO migration, NO
simulator change, NO Score Calibration re-baseline, NO ADR, NO new CONTEXT.md term, NO change
to the global HX-01 career page, NO persisted award rows (every award recomputed on render),
NO awards caching, NO API/DRF endpoint.

**Locked names (NEW / CHANGED).**
- `matches/season_awards.py` — **NEW** pure module (allowlist `dataclasses`, `typing`,
  `collections`): `AwardWinner` / `AwardSet` (frozen dataclasses),
  `compute_season_awards(player_rounds: list[dict], *, min_games: int) -> AwardSet`,
  `pick_finals_mvp(final_round_dicts: list[dict]) -> AwardWinner | None`.
- `matches.league_views.season_awards(request, season_id) -> HttpResponse` — **NEW** view.
- `season_awards` / `/seasons/<int:season_id>/awards/` — **NEW** URL name / path in
  `matches/season_urls.py`.
- `templates/seasons/awards.html` — **NEW** template.
- `matches.league_views._build_history_row` — **CHANGED** (11 → 13 keys: `+ season_mvp`,
  `+ finals_mvp`); `templates/leagues/history.html` gains 2 columns + the awards link.
- `matches.league_screens.player_detail.player_detail` — **CHANGED** (`+ player_awards`
...
The manager-mode live surface over the LG-01d Play dropdown — a **"One Week (Live)"** entry that **plays `League.current_team`'s next game immediately** (committing it), kicks off the **rest** of the matchday / bracket stage as a **background** run, and opens a read-only **replay** of the just-played game (the SIM-05 client playback engine, fed from the *persisted* event log). The locked model is **play-now-and-watch, NOT preview-then-commit and NOT server-side tick streaming**: results are **final the moment the action runs** — there is **no preview, no commit step, no discard, no retry**. Because the manager's game is committed by the existing per-Round / per-Match sim path with no injected seed, this slice is a thin **view + template + serializer-reuse** layer: **no model change, no migration, no engine-mechanics change, no Score Calibration re-baseline**. The **One Week (Live)** glossary term (above **Current team** in CONTEXT.md `### League and seasons`) carries the domain language. Seam contract (historical — describes the superseded preview-then-commit design): [`.claude/worktrees/lg-01i-one-week-live-seam-contract.md`](../../.claude/worktrees/lg-01i-one-week-live-seam-contract.md).

> **History:** LG-01i originally shipped as a **preview-then-commit** flow (an in-memory preview held as a session-pinned RNG seed, watched, then committed or discarded — with an injected-seed seam `rng_seed=` / `rng_seeds=` on the sim methods + `preview_scheduled_round` / `preview_tournament_match` / `_serialize_events_for_preview`). A follow-up redesign **replaced** it with the play-now model below; the preview methods, the injected-seed kwargs, the session pin, and the commit/discard views were all **removed**. The seam contract + the original PLAN note predate that change.

**Two watchable cursor types.** (1) **RR cursor** — `current_team`'s **single next-matchday Round**. A **bye** that matchday ⇒ no game to watch (the action just runs the week in the background, then redirects to the dashboard). (2) **Playoff cursor** — `current_team`'s **next undecided 2-Round playoff Match** (the next undecided `BracketNode` it occupies, `series_matches.count() == 0`), offered ONLY while that team is **alive** (an incomplete, non-bye node it participates in). Eliminated / not-a-participant ⇒ no live entry.

**Two views (`matches/league_views.py`).** (a) **`play_week_live(request, season_id) -> HttpResponse`** — **POST-only** (`HttpResponseNotAllowed(["POST"])` first line), `get_object_or_404(Season)`, sets `request.session["last_league_id"]`; non-active ⇒ `_render_season_dashboard_error(...)` (400 dashboard re-render); resolves the live cursor; an `rr_bye` cursor ⇒ `play_season_task.delay(season.id, max_matchdays=1)` + redirect to `season_dashboard`; no live entry ⇒ 400. Otherwise (inside a `try` catching `ValidationError`/`ValueError` → 400): **RR** resolves the fixture's arena map (`ArenaMap.in_bulk(starting_map_pool_ids_json)` + `matches.tasks._resolve_fixture_map`) and plays the manager's Round via `BatchSimulator().simulate_scheduled_round(...)` ⇒ `round_ids = [game_round.id]`; **playoff** plays the manager's node via `tournament_engine.play_specific_node(node)` then reads the just-played Match's 2 `game_rounds` (the node's latest `series_matches` row) ⇒ `round_ids = [r1, r2]`. Then it **enqueues `play_season_task.delay(season.id, max_matchdays=1)`** to play the rest of the matchday / drain the rest of the bracket stage in the **background** (the task skips the manager's just-played game via its played-keys / `find_next_playable_node` checks), stashes `request.session["live_watch"] = {"season_id", "kind", "round_ids"}` (just "what to show" — the game is already committed, NOT a retry pin), and **redirects** to `play_week_live_watch` ⇒ **302**. (b) **`play_week_live_watch(request, season_id) -> HttpResponse`** — **GET-only** (405 otherwise), 404 on missing Season, reads the `live_watch` session handoff (400 dashboard re-render when absent / for another Season / empty), loads the committed `GameRound`(s), builds the SIM-05 `(events_data, players_data)` payload from the **persisted** events via the reused `matches.views.round_playback_payload(game_round, include_movement=False)`, and renders `templates/seasons/play_week_live.html` ⇒ **200**.

**`round_playback_payload` (reused committed-events serializer).** `matches/views.py::round_playback_payload(game_round, *, include_movement=True) -> (events_data, players_data)` is **extracted from `game_round_events`** (which now calls it) so the watch view reuses the exact same 13-key `events_data` (`type, ts, tf, icon, desc, pts, aid, an, at, tid, tn, tt, meta`) + 6-key `players_data` (`{id, name, team, role, sl, ss}`) serialization the SIM-05 playback engine consumes. `include_movement=False` drops `event_type="movement"` rows (the live watch shows only gameplay events on its timeline; the scoreboard reconstruction ignores movement either way).

**Cursor dispatch.** `matches.league_views._resolve_live_cursor(season) -> dict | None` and `_alive_playoff_node(tournament, team) -> "BracketNode | None"` are unchanged from the original slice: `_resolve_live_cursor` returns a `{"kind": "rr"|"playoff"|"rr_bye", ...}` descriptor (RR carries `fixture` / `season_phase` / `red_team` / `blue_team`; playoff carries `node` / `tournament` / `red_team` / `blue_team`) or `None`; `_next_matchday_to_play(season) -> (to_play, phase_by_id)` is the shared next-matchday-fixtures helper. `_alive_playoff_node` selects the lowest `(_BRACKET_RANK[bracket_type], bracket_round, position)` undecided non-bye node the team occupies with no Series started (`select_related("team_a","team_b")`).

**Engine — `play_specific_node` (no injected seed).** `matches/tournament_engine.py::play_specific_node(node) -> "BracketNode | None"` (`@transaction.atomic`) is the per-Match resolve/advance body extracted from `play_next_node` (which delegates to it). It takes the node **directly** so the live-watch playoff path can play the manager's **specific** node; `play_next_node` plays the next playable one. (No `rng_seeds` — the redesign removed the injected-seed seam.)

**URLs (`matches/season_urls.py`), before `standings/` / `schedule/`, bare names:** `play_week_live` (`/seasons/<int:season_id>/play-week-live/` → POST) and `play_week_live_watch` (`.../watch/` → GET).

**Template `templates/seasons/play_week_live.html`.** Extends `base.html`, `d-flex` + league-sidebar shell (`sidebar_active=None`). It **duplicates** the minimal SIM-05 playback block (scrubber DOM + the `pbPlayers`/`pbEvts`/`pbApply`/`pbSetTime`/`pbRefresh` engine + `MAX_MISSILES` role constant + the `events-data`/`players-data` json_script reads) — NOT extracted into a shared partial, because the SIM-05 engine in `game_round_events.html` is coupled to page-specific `{{ round.* }}` context + the charts / kill-feed / timeline-window pager. Team identity is fed from the per-round `red_team_id` / `red_team_name` / `blue_team_id` / `blue_team_name`. **Scoreboard reads each event's authoritative `actor_`/`target_` metadata snapshots** (`actor_shots`/`actor_lives`/`actor_points`/`sp`, `target_shots`/`target_lives`/`target_points`) for exact shots/lives/score/SP — NOT re-derived — and decrements **missiles** (role-fixed `MAX_MISSILES`, Commander/Heavy 5) on each `locking` event; a resupply / down flashes the respawn colouring. A **role-paired combined scoreboard** (each row pairs the two teams' same-role players, sorted by a canonical role order; a per-team **Missiles** column; three down-state row colourings) plus a **play-synced event timeline** (feed-style, newest at the bottom) and a **"Game start" anchor** (the page opens on a pre-event cursor so the tick-0 events aren't pre-applied). **NO commit / discard controls** — only a **"Back to dashboard"** link (DOM id `play-week-live-done`). **LOCKED DOM ids:** `play-week-live-container`; the reused `pb-*` ids (`pb-time-display`/`pb-scrubber`/`pb-step-back`/`pb-play`/`pb-step-fwd`/`pb-speed`/`pb-scoreboard`/`pb-timeline`); `play-week-live-round-toggle` (+ `play-week-live-round-1`/`-round-2`, playoff only); `play-week-live-done`. **`json_script` ids:** RR emits `events-data` / `players-data`; playoff emits `events-data-1`/`players-data-1`/`events-data-2`/`players-data-2`.

**Dashboard wiring.** Context key **`live_preview_available: bool`** in `_build_dashboard_context` — `True` iff `_resolve_live_cursor(season)` returns a `"rr"` / `"playoff"` descriptor (NOT `None` / `"rr_bye"`). The dropdown entry is a **POST `<form>`** (id `season-dashboard-play-one-week-live` / `league-dashboard-play-one-week-live`) to `play_week_live`, rendered ONLY when `live_preview_available`, alongside the LG-01d One Week / Two Months / Until-End entries. *(RETIRED by NAV-01: the One Week Live entry relocates to the `Play ▾` topnav as `topbar-play-one-week-live`, gated on the same `live_preview_available`; `live_preview_available` is now emitted by `_build_play_controls_context` rather than `_build_dashboard_context`. See **NAV-01** above.)*

**Scope-out (LOCKED).** No model change, no migration, no ADR, no engine-mechanics change, no Score Calibration re-baseline. No preview / commit / discard / retry. No server-side tick streaming. No watching non-manager games (only `current_team`'s game has a replay surface; the rest run headless in the background). No playoff-live when eliminated. No SIM-05 partial extraction (the playback JS is duplicated).

**Locked names (quick index).** Views `matches.league_views.play_week_live` (POST 302 / 400 / 405) / `play_week_live_watch` (GET 200 / 400 / 405); cursor helpers `_resolve_live_cursor` / `_alive_playoff_node` / `_next_matchday_to_play`; engine `tournament_engine.play_specific_node(node)` (extracted from `play_next_node`); serializer `matches.views.round_playback_payload(game_round, *, include_movement=True)`; URLs `play_week_live` (`/seasons/<id>/play-week-live/`) / `play_week_live_watch` (`.../watch/`); session handoff `request.session["live_watch"]` = `{season_id, kind, round_ids}`; background task `play_season_task.delay(season_id, max_matchdays=1)`; context key `live_preview_available`; template `templates/seasons/play_week_live.html` (DOM ids above); tests `TestLg01iCursorDispatch` / `TestLg01iPlayWeekLive` / `TestLg01iLiveWatch` in `views_tests.py`, `TestLg01iDashboardEntry` in `test_league_dashboard.py` / `test_season_dashboard_view.py`.

## CAR-02 manager firing (owner mood)

Manager-mode performance firing, implemented as a **ZenGM-faithful owner-mood model** (NOT a single configurable threshold — the PLAN's "configurable threshold" wording was superseded at grilling). The team **Owner** judges the **Manager** (the implicit local user = `League.current_team`, CAR-01 — **no `Manager`/`User` model**) once per completed Season across **three cumulative Mood factors** (*wins* / *playoffs* / *money*), fires the Manager when summed mood `<= -1` strictly past a 2-Season Grace period, and routes a fired Manager through a **New Team** picker (worst-5 eligible, old team excluded) before the pre-season rollover can run. **No simulator change → no Score Calibration re-baseline.** Decision: [ADR-0026](../../docs/adr/0026-manager-firing-owner-mood.md). Seam contract: [`.claude/worktrees/car-02-performance-based-firing-seam-contract.md`](../../.claude/worktrees/car-02-performance-based-firing-seam-contract.md).

**Pure module `matches/owner_mood.py`.** Django-free, **frozen import allowlist** `dataclasses` / `typing` / `collections` ONLY — NO Django / ORM / `random` / `datetime` / `math` / I/O / logging (defended by `TestNoDjangoImportsLeaked`, the subprocess fresh-import + `sys.modules` walk mirroring `season_awards.py` / `development.py` / `standings.py`). Constants (LOCKED values): `WINS_FACTOR = 1.0`, `WINS_BASELINE_SCALE = 0.25`, `PLAYOFF_TITLE = 0.2`, `PLAYOFF_MISS = -0.2`, `PLAYOFF_ADVANCE_SCALE = 0.16`, `MOOD_FACTOR_CAP = 1.0` (per-factor cumulative ceiling, **upside-only — no negative floor**), `FIRE_THRESHOLD = -1.0`, `GRACE_PERIOD_SEASONS = 2` (flat — ZenGM's +3-if-joined-at-playoffs nuance dropped). Three frozen dataclasses with pinned field order: `MoodDeltas(wins, playoffs, money)` / `MoodTotals(wins, playoffs, money)` (`money` always `0.0` this slice — dormant) / `Verdict(outcome, hot_seat_level)` (`outcome ∈ {"retained", "hot_seat", "fired"}` LOCKED strings; `hot_seat_level` 0 for retained/fired, 1/2 only for hot_seat). **Four pure fns:** `compute_wins_delta(won, games) -> float` (`WINS_FACTOR * WINS_BASELINE_SCALE * (won - games/2) / (games/2)`; **`games == 0` ⇒ `0.0`** no div-by-zero); `compute_playoffs_delta(playoff_result, rounds_won, num_rounds) -> float` (`"champion"` ⇒ `+0.2`, `"seeded"` ⇒ `(0.16/num_rounds)*rounds_won` with **`num_rounds == 0` ⇒ `0.0`**, `"missed"` ⇒ `-0.2`, `"none"` ⇒ `0.0`, unknown string ⇒ `0.0` forgiving); `cap_cumulative(prev_cumulative, delta) -> float` (`min(prev + delta, MOOD_FACTOR_CAP)` — cap on the **upside only**); and **one** decider `decide_verdict(totals, deltas, *, seasons_in_tenure) -> Verdict` returning BOTH the outcome AND the hot-seat level (the firing decision + hot-seat projection share the cumulative total + delta, so splitting them would duplicate the threshold math). `decide_verdict` algorithm (LOCKED): `past_grace = seasons_in_tenure > GRACE_PERIOD_SEASONS`; past-grace `total <= -1` ⇒ `Verdict("fired", 0)`; past-grace `total + delta < -1` ⇒ `("hot_seat", 1)`; past-grace `total + 2*delta < -1` ⇒ `("hot_seat", 2)` (level 1 wins when both hold — checked first); else `("retained", 0)`. Inside the grace period the verdict is **always `"retained"`** regardless of mood. Firing compares `<=`; the hot-seat projections compare `<` (strict, matching the spec).

**Model `matches.models.OwnerEvaluation` + migration `0049_ownerevaluation`.** One **immutable** per-`(League, completed Season)` snapshot, placed in `models.py` immediately after `PlayerSeasonRating`. FKs: `league` (`CASCADE`, `related_name="owner_evaluations"`), `season` (`CASCADE`, `related_name="owner_evaluations"`), `team_managed` (`teams.Team`, `SET_NULL`, `null=True/blank=True`, `related_name="+"` — the Team the Manager ran that Season). Factor fields: `wins_delta` / `playoffs_delta` (pre-cap, this Season only) + `money_delta` (`default=0.0`, **DORMANT — always 0.0**); `wins_total` / `playoffs_total` (per-factor cumulative-capped across the tenure) + `money_total` (`default=0.0`, dormant); `verdict` (`CharField(max_length=16, choices=VERDICT_CHOICES)` = `retained`/`hot_seat`/`fired`), `hot_seat_level` (`PositiveSmallIntegerField(default=0)` 0/1/2), `created_at` (`auto_now_add`). `Meta.ordering = ["league_id", "season_id"]` + a `UniqueConstraint(fields=["league", "season"], name="uniq_league_season_owner_evaluation")`. Migration is **CreateModel-only — NO `RunPython` / `RunSQL` / backfill** (ADR-0026 + the ADR-0004 disposable-data posture, the `0048` / `0029` precedent); existing Leagues get no historical rows, the lazy writer fills them in Season order on first reach. Dependency `("matches", "0048_playerseasonrating")`. **Tenure boundaries + grace derive from the snapshot chain** — a `team_managed` change between two consecutive rows (by Season order) marks a NEW tenure (cumulative + grace reset) — **there is no `tenure_id` field** (firings mutate `League.current_team`, so a past Season's managed team is otherwise unrecoverable). The money column is persisted-but-dormant so a future finance subsystem (PLAN **FIN-01**) lights it up **without a migration**.

**Writer `matches.league_views._ensure_owner_evaluations(league, up_to_season) -> None`.** **Lazy + idempotent.** Ensures an `OwnerEvaluation` row for **every completed Season of `league` up to and including `up_to_season`**, written **oldest→newest in Season order** (so the per-factor caps + cumulatives + tenure derivation are correct), `get_or_create`-keyed on `(league, season)` — a present row is left untouched (NO backfill of Seasons before the first computable one, ADR-0004). It walks `league.seasons.filter(state="completed", id__lte=up_to_season.id).order_by("id")` threading three per-factor running totals + a tenure marker (`prev_team_managed` + a `seasons_in_tenure` counter): for each Season it derives `team_managed` from the snapshot chain (no prior in-tenure row ⇒ `league.current_team`, a `"fired"` prior row ends the tenure so the next row reads `current_team` again — the post-Reassignment team, else inherit the prior row's `team_managed`); resets the running totals + `seasons_in_tenure = 1` on a tenure change else `+= 1`; builds the flat inputs (§ below) → `compute_wins_delta` / `compute_playoffs_delta` (`money_delta = 0.0`); cap-chains `cap_cumulative(running.*, delta)`; calls `decide_verdict(MoodTotals(...), MoodDeltas(...), seasons_in_tenure=...)`; and `get_or_create`s the row. **Idempotency note:** when a row already existed, the writer re-reads its **persisted** deltas/totals/`team_managed` into the running state before continuing (the persisted row, not a recompute, is the source of truth for prior Seasons).

**Flat-input assembly (view-side, per Season) — REUSE the standings path.** *wins / games:* reuse `Season._final_standings_for_phase(season.ordered_phases()[-1])` to read the regular-season `StandingsRow` for `team_managed.id` → `won = row.wins`, `games = row.matches_played` (the standings query already excludes playoff Matches — they carry `season=NULL`). *playoffs:* locate the Season's `tournament` `SeasonPhase` (`phase_type == "tournament"`, `tournament_id is not None`); none ⇒ `("none", 0, 0)`; else `num_rounds = max(bracket_round)`, classify `champion` (`tournament.champion_id == team_managed.id`) / `seeded` (in bracket, not champion; `rounds_won` = count of distinct `bracket_round`s with a `BracketNode.winner_id == team_managed.id`) / `missed` (cut / not seeded). The pure module receives only `(won, games)` + `(playoff_result, rounds_won, num_rounds)` — never a Django object.

**`next_season` refactor + verdict gate.** The existing `@transaction.atomic next_season` rollover body (create the new draft Season, carry teams / map pool / phases, `_develop_league_for_new_season`) is extracted **verbatim** into a plain (undecorated) `_run_season_rollover(league, latest_completed) -> Season` shared by BOTH `next_season` and `reassign_team` (each owns its own atomic boundary). `next_season` (still `@transaction.atomic`) becomes the **verdict gate**: after the unchanged 405 / 404 / active-Season-guard / `latest_completed`-resolve steps it calls `_ensure_owner_evaluations(league, latest_completed)`, reads the `(league, latest_completed)` eval, and — **if `verdict == "fired"` AND the Manager has not yet reassigned** (`league.current_team` still == the row's `team_managed`) — **redirects to `new_team_picker`** (a fired-and-unreassigned Manager cannot roll); else runs `_run_season_rollover` + redirects to the new Season's dashboard. **Byte-equivalence guarantee:** for a non-fired Manager the rollover output is byte-equivalent to today — the only added work is the ensure write + the verdict read, which never touch the rollover body.

**Views + URLs.** `owner_evaluation(request, season_id) -> HttpResponse` (GET-only league-screen mirroring the `player_detail` / `season_awards` shell: 405 guard first line, `get_object_or_404(Season)`, `request.session["last_league_id"] = season.league_id`, `_ensure_owner_evaluations(league, season)` then read the row, **404 when the Season is not completed**, `_build_league_sidebar_links(..., sidebar_active=None)`, render — context carries `evaluation` + the CTA flags `is_fired` / `reassigned`; **browsable for past Seasons**). `new_team_picker(request, league_id) -> HttpResponse` (GET-only; builds the **worst-`WORST_N_ELIGIBLE` (5)** teams by `latest_completed`'s final Standings — highest `rank` = worst — **EXCLUDING `league.current_team`**, the team that fired the Manager). `reassign_team(request, league_id) -> HttpResponse` (POST-only, `@transaction.atomic`; reads + **server-side re-validates** `team_id` against the worst-N set, 400 on a tampered id; then LOCKED order: set `league.current_team = picked` + `league.save(update_fields=["current_team"])` — **starts a new tenure**; `_run_season_rollover(league, latest_completed)`; 302 to the new Season's dashboard). URL names (bare, no `app_name`): `owner_evaluation` (`/seasons/<int:season_id>/owner-evaluation/`, in `season_urls.py` after `awards/`), `new_team_picker` (`/leagues/<int:league_id>/new-team/`) + `reassign_team` (`/leagues/<int:league_id>/reassign-team/`, in `league_urls.py` after `next-season/`, before the `league_list` catch-all).

**Templates + LOCKED DOM ids.** `templates/seasons/owner_evaluation.html` (extends `base.html`, `d-flex` + `{% include "_partials/league_sidebar.html" %}` shell): `owner-evaluation` (root), `owner-evaluation-verdict` (badge, text == `evaluation.verdict`), `owner-evaluation-hot-seat-warning` (only on `hot_seat`; level 1 ⇒ "another season…", 2 ⇒ "a couple more…"), `owner-evaluation-factor-wins` / `-factor-playoffs` / `-factor-money` (delta + cumulative-capped total; **money renders `0.0` — dormant**), `owner-evaluation-total` (`wins_total + playoffs_total + money_total`), `owner-evaluation-cta-start-next` (the Start Next Season form, rendered when `not is_fired`, POSTs `next_season`), `owner-evaluation-cta-choose-team` (the Choose New Team link, rendered when `is_fired and not reassigned`, links `new_team_picker`). `templates/leagues/new_team.html` (same shell): `new-team-picker` (root), `new-team-form` (`<form method="post" action="{% url 'reassign_team' league.id %}">`), `new-team-option-{team_id}` (one per eligible team), `new-team-submit`. `{% csrf_token %}` mandatory in both POST forms.

**Dashboard reroute (gates "Start Next Season" through the eval screen).** In BOTH `templates/seasons/dashboard.html` and `templates/leagues/dashboard.html`, the `action_button_state == "start_next_season"` branch (was a POST `<form>` to `next_season`, ids `{season,league}-dashboard-next-season-form`) becomes a **GET `<a>` to `{% url 'owner_evaluation' displayed_season.id %}`** — new DOM id `{season,league}-dashboard-owner-evaluation-link`, **carrying `data-action-state="start_next_season"`** so the LG-01c/e dashboard-wiring tests keep passing (the outer `{season,league}-dashboard-action-button` wrapper id is preserved). A **"View past evaluations"** dashboard link (`{season,league}-dashboard-past-evaluations-link`) makes past-Season evals reachable (simplest target: the League History page, each row exposing `league-history-owner-evaluation-link-{season_id}` → `owner_evaluation`). **Blast radius:** LG-01e's `TestLg01eDashboardWiring` (in `test_league_dashboard.py` + `test_season_dashboard_view.py`) asserted the completed branch rendered a `-next-season-form` POST form — those assertions are **updated** to the rerouted GET-link shape (the `data-action-state` attribute survives; the `-next-season-form` id is replaced by `-owner-evaluation-link`).

**Scope-out (locked).** **Money factor DORMANT** (always `0.0` — no finance subsystem; PLAN **FIN-01** lights it up). Challenge-mode firings (miss-playoffs / luxury-tax) + voluntary rival-offer switching **DEFERRED**. **No `Manager`/`User` model** (the Manager is `League.current_team`). **No simulator change → no Score Calibration re-baseline.** **No backfill / `RunPython`.** **No new CONTEXT.md term** (all 10 finalised at the grill). CAR-03 will later gate firing to single-player `league` mode (this slice already operates only there).

**Locked names (quick index).** Pure module `matches/owner_mood.py`; constants `WINS_FACTOR` / `WINS_BASELINE_SCALE` / `PLAYOFF_TITLE` / `PLAYOFF_MISS` / `PLAYOFF_ADVANCE_SCALE` / `MOOD_FACTOR_CAP` / `FIRE_THRESHOLD` / `GRACE_PERIOD_SEASONS` / `WORST_N_ELIGIBLE` (5); dataclasses `MoodDeltas(wins, playoffs, money)` / `MoodTotals(wins, playoffs, money)` / `Verdict(outcome, hot_seat_level)`; fns `compute_wins_delta` / `compute_playoffs_delta` / `cap_cumulative` / `decide_verdict`; verdict strings `"retained"` / `"hot_seat"` / `"fired"`; playoff-result strings `"champion"` / `"seeded"` / `"missed"` / `"none"`; model `matches.models.OwnerEvaluation`; constraint `uniq_league_season_owner_evaluation`; migration `matches/migrations/0049_ownerevaluation.py`; helpers `matches.league_views._ensure_owner_evaluations` / `_run_season_rollover`; views `owner_evaluation` / `new_team_picker` / `reassign_team`; URL names `owner_evaluation` (`/seasons/<id>/owner-evaluation/`) / `new_team_picker` (`/leagues/<id>/new-team/`) / `reassign_team` (`/leagues/<id>/reassign-team/`); templates `templates/seasons/owner_evaluation.html` / `templates/leagues/new_team.html` (DOM ids above); dashboard DOM ids `{season,league}-dashboard-owner-evaluation-link` (carrying `data-action-state="start_next_season"`) + `{season,league}-dashboard-past-evaluations-link`; tests `matches/tests/test_owner_mood.py` (pure-unit, incl. `TestNoDjangoImportsLeaked`) / `test_owner_evaluation_model.py` / `test_owner_evaluations_writer.py` / `test_owner_evaluation_view.py` / `test_reassign_team.py` (NEW) + extended `test_league_next_season.py` / `test_league_dashboard.py` / `test_season_dashboard_view.py`.

## CAR-03 career isolation from multiplayer

A **defensive gate only** layered on CAR-02 — the entire owner-evaluation / firing / reassignment lifecycle is made **inert unless `League.mode == "league"`**. The gate predicate is a **positive allowlist** `mode == "league"`, so `"sandbox"`, `"multiplayer"`, and **any future mode** are inert by default (a new mode is non-career until explicitly allowlisted). **No multiplayer creation flow, no new mode value, no model / migration / simulator change, no Score Calibration re-baseline, no new ADR, no new CONTEXT.md term** (the **Owner evaluation** glossary entry already carries the league-mode-only clause). Seam contract: [`.claude/worktrees/car-03-career-isolation-seam-contract.md`](../../.claude/worktrees/car-03-career-isolation-seam-contract.md).

**Shared helper (single source of truth).** `matches/league_views.py::_is_career_league(league) -> bool` is the **one** predicate (`return league.mode == "league"`), consumed by the writer, the two reassign views, and the dashboard context — there is no other place that branches on mode for the firing lifecycle.

**Writer chokepoint + two view guards (400).** The lazy writer `_ensure_owner_evaluations(league, up_to_season)` now **early-returns on its first line** when `not _is_career_league(league)` — the chokepoint: **no `OwnerEvaluation` rows are ever written for a non-career League**, so everything downstream that keys on the evaluation row degrades safely. The two reassign-path views `new_team_picker` (GET) and `reassign_team` (POST) add a `not _is_career_league(league)` guard **after** the `get_object_or_404(League, ...)` (so a bad id still 404s) and **before** any `current_team` write ⇒ **`HttpResponseBadRequest` / HTTP 400**, writing nothing (`HttpResponseBadRequest` is already imported in `league_views.py`).

**`next_season` / `owner_evaluation` UNCHANGED.** Neither is guarded — the writer no-op is sufficient. `next_season`'s verdict gate reads `evaluation is None` for a non-career League and **rolls the Season normally** (no firing, no New-Team redirect), byte-equivalent to pre-CAR-02. `owner_evaluation` naturally raises its existing `Http404` on the missing evaluation row for a non-career completed Season (the natural 404 suffices — no explicit guard added).

**Dashboard split (`is_career_mode`).** `_build_dashboard_context(displayed_season, season_mode)` gains an `is_career_mode: bool` key (`displayed_season is not None and _is_career_league(displayed_season.league)`; reachable only on the `start_next_season` branch, where `displayed_season` is the completed Season). Both `templates/seasons/dashboard.html` and `templates/leagues/dashboard.html` split the `start_next_season` arm on it: **`{% if is_career_mode %}`** → the CAR-02 owner-evaluation GET link (`{season,league}-dashboard-owner-evaluation-link`); **`{% else %}`** → a direct `next_season` POST `<form>` (`{season,league}-dashboard-next-season-form`, the pre-CAR-02 LG-01e shape, `league_id` = `season.league_id` / `league.id`). The outer `{season,league}-dashboard-action-button` wrapper `<span>` and `data-action-state="start_next_season"` are **preserved in BOTH arms** (LG-01c/e back-compat).

**Tests** extend the existing CAR-02 files with a representative **`mode="multiplayer"`** fixture (Django `TestCase`, real views/writer end-to-end, no ORM mocks; assert on row counts / status codes / `current_team` / rendered DOM ids, never point totals): `test_owner_evaluations_writer.py` (writer on a multiplayer completed Season writes **0** rows) + `test_league_next_season.py` (`next_season` → **302** to the new Season's dashboard, new draft Season exists, **no** `OwnerEvaluation` row, `current_team` unchanged) + `test_reassign_team.py` (`reassign_team` POST **400** + `new_team_picker` GET **400**, `current_team` unchanged) + `test_league_dashboard.py` / `test_season_dashboard_view.py` (multiplayer completed-Season dashboard renders `…-next-season-form` and NOT `…-owner-evaluation-link`; `league` mode still renders `…-owner-evaluation-link` and NOT the form).

**Locked names (quick index).** Helper `matches.league_views._is_career_league(league) -> bool` (predicate `league.mode == "league"`); guarded sites `_ensure_owner_evaluations` (early-return) / `new_team_picker` / `reassign_team` (400) / `_build_dashboard_context`; dashboard context key `is_career_mode`; dashboard DOM ids (non-career arm) `{season,league}-dashboard-next-season-form`; UNCHANGED `next_season` / `owner_evaluation`; tests extended `test_owner_evaluations_writer.py` / `test_league_next_season.py` / `test_reassign_team.py` / `test_league_dashboard.py` / `test_season_dashboard_view.py` (`mode="multiplayer"` fixture); seam contract `.claude/worktrees/car-03-career-isolation-seam-contract.md`.

## FIN-01 team finance subsystem

The finance epic CAR-02 deferred — it **lights up the dormant *money* mood factor** CAR-02 shipped as a permanent `0.0` column on `OwnerEvaluation`. A per-League team finance economy: **player salary** (derived from `overall_rating`), three per-Team **budget** levels (scouting / coaching / facilities) + a ticket price, **season-level profit** accounting (revenue vs expenses, ZenGM's `winp`-driven hype loop, a fixed single market), and the **money-axis wire** that replaces `_ensure_owner_evaluations`'s hardcoded `money_delta = 0.0` / `money_total = 0.0` with a real computed value. Gated by a **per-League `finance_enabled` toggle** ON TOP of CAR-03's `_is_career_league` mode gate; OFF ⇒ the whole subsystem is **inert and byte-identical to today**. **Cost-only this slice** — budgets are pure expense lines (facilities additionally feeds revenue), scouting/coaching buy NO gameplay edge yet (FIN-02/03). **Finances consume no RNG, are outside SIM-07/08, → NO Score Calibration re-baseline.** Decision: [ADR-0027](../../docs/adr/0027-team-finance-subsystem.md). Seam contract: [`.claude/worktrees/fin-01-seam-contract.md`](../../.claude/worktrees/fin-01-seam-contract.md).

**Model + migrations.** `teams.Player.salary` (`FloatField(null=True, blank=True, default=None)`, declared after `potential`, mirrors it exactly — derived from `overall_rating` at the finance-ensure pass, `None` outside any finance-enabled flow) + five `teams.Team` finance fields (after `is_draw_team`): `budget_scouting` / `budget_coaching` / `budget_facilities` (`PositiveSmallIntegerField(default=34)` — ZenGM 1–100 levels, `finance.DEFAULT_LEVEL = 34` neutral), `ticket_price` / `cash` (`FloatField(default=0.0)`, dollar amounts; no choices/validators — the toggle + AI defaults + form clamp keep them in band) ship in **`teams/migrations/0013_player_salary_team_finance.py`** (dep `0012_player_potential`). `matches.League.finance_enabled` (`BooleanField(default=False)`, the per-League master switch, set at create time) + **`matches.models.TeamSeasonFinance`** (one **immutable** per-`(Team, Season)` snapshot placed after `OwnerEvaluation`: `team` FK `teams.Team` **SET_NULL** `null/blank`/`related_name="season_finances"` — a deleted Team keeps its history, the `OwnerEvaluation.team_managed` precedent; `season` FK **CASCADE** `related_name="team_finances"` — a deleted Season drops its rows, the LG-01 `PlayerSeasonRating` precedent; 5 revenue lines `ticket`/`national_tv`/`local_tv`/`sponsor`/`merch`, 6 expense lines `payroll`/`scouting_cost`/`coaching_cost`/`facilities_cost`/`luxury_tax`/`min_payroll_penalty`, derived `revenue`/`expenses`/`profit`, the carried `hype`, `created_at`; `Meta.ordering = ["season_id", "team_id"]` + a `UniqueConstraint(fields=["team","season"], name="uniq_team_season_finance")`) ship in **`matches/migrations/0050_league_finance_teamseasonfinance.py`** (dep `0049_ownerevaluation` + the cross-app `("teams","0013_player_salary_team_finance")` — `TeamSeasonFinance.team` FKs `teams.Team`). Both migrations are **AddField/CreateModel-only — NO `RunPython`/backfill** (ADR-0004 disposable-data posture, the `0029`/`0048`/`0049` precedent); existing Leagues get no historical rows, the lazy writer fills them in Season order on first reach.

**Pure module `matches/finance.py`.** Django-free, **frozen import allowlist** `dataclasses` / `typing` / `math` / `collections` ONLY — NO Django / ORM / `random` / `datetime` / I/O / logging (defended by `matches/tests/test_finance.py::TestNoDjangoImportsLeaked`, the subprocess fresh-import + `sys.modules` walk mirroring `owner_mood.py` / `development.py` / `season_awards.py`). Constants are **locked-but-tunable magic numbers** (the LG-04 age-curve / LG-05 `POTENTIAL_MAX_SD` precedent — invented-by-analogy, calibration-deferred, sized so a typical Season's profit lands near the `EXPECTED_PROFIT_BASE = 15` anchor): `MAX_LEVEL = 100`, `DEFAULT_LEVEL = 34` (neutral — facility cost mid-range, effect 0), `BUDGET_LEVEL_SCALE = 1.1`, `BASELINE_SALARY_CAP = 90000.0` (frozen `salaryCapFactor` denominator), `SALARY_CAP = 90000.0` (== baseline this slice ⇒ `salaryCapFactor == 1.0`; tunable), `EXPECTED_PROFIT_BASE = 15.0`, `MONEY_DELTA_DIVISOR = 100.0`, `LUXURY_PAYROLL` / `LUXURY_TAX_RATE = 1.5` / `MIN_PAYROLL` (thresholds), plus the revenue/merch/sponsor/tv coefficients + caps (the doc-02 table, cap-scaled) — all tunable. **Three frozen dataclasses (pinned field order):** `RevenueLines(ticket, national_tv, local_tv, sponsor, merch)` / `ExpenseLines(payroll, scouting, coaching, facilities, luxury_tax, min_payroll_penalty)` / `TeamFinanceResult(revenue_lines, expense_lines, revenue, expenses, profit, hype, money_delta)`. **Nine pure fns + the aggregator:** `level_to_amount(level, salary_cap=SALARY_CAP)`, `salary_for_overall(overall, salary_cap=SALARY_CAP)`, `compute_hype(prev_hype, winp, winp_old)` (`hype += 0.01*(winp-0.55) + 0.015*(winp-winp_old)`, bound `[0,1]` — the doc-02 loop), `season_revenue(hype, ticket_price, facilities_level, salary_cap=SALARY_CAP) -> RevenueLines`, `season_expenses(payroll, scouting_level, coaching_level, facilities_level, salary_cap=SALARY_CAP) -> ExpenseLines`, `luxury_tax(payroll, salary_cap=SALARY_CAP)`, `min_payroll_penalty(payroll, salary_cap=SALARY_CAP)`, `season_profit(revenue, expenses)` (`revenue - expenses`), `money_delta(profit, salary_cap=SALARY_CAP)`; and the single entry **`compute_team_finance(*, payroll, scouting_level, coaching_level, facilities_level, ticket_price, prev_hype, winp, winp_old, salary_cap=SALARY_CAP) -> TeamFinanceResult`** the writer calls. **The flat inputs crossing the view↔pure seam are ints/floats/levels ONLY** — `payroll` (float, summed view-side from `Player.salary`), three budget **levels** (int), `ticket_price` (float), `prev_hype`/`winp`/`winp_old` (float), `salary_cap` (float); the module **never sees a `Team` / `Season` / `League` / ORM type**. **Money formula (LOCKED):** `salaryCapFactor = salary_cap / BASELINE_SALARY_CAP`; `expected_profit = 15 * salaryCapFactor`; `money_delta = (profit − expected_profit) / (100 * salaryCapFactor)`. The cumulative cap-chain through `owner_mood.MOOD_FACTOR_CAP = 1.0` / `cap_cumulative` happens **in the writer, NOT in `finance.py`**.

**Writer `matches.league_views._ensure_team_finances(league, up_to_season) -> None`** — the twin of `_ensure_owner_evaluations` (mirrors its structure). **First line early-return** when `not _is_career_league(league) or not league.finance_enabled` (the toggle gate ON TOP of the CAR-03 mode gate) ⇒ writes zero `TeamSeasonFinance` rows. Walks `league.seasons.filter(state="completed", id__lte=up_to_season.id).order_by("id")` **oldest→newest** (so hype carries across Seasons correctly); per Season, per enrolled Team it `get_or_create`s a row **keyed on `(team, season)`** (idempotent — a present row left untouched, no backfill), summing payroll from the active-roster `Player.salary` values (recomputed/refreshed for that Team before summing), reading `prev_hype` from the **prior** Season's `TeamSeasonFinance` for that Team, `winp` from `Season._final_standings_for_phase(...)`, `winp_old` from the prior-Season standings, computing `compute_team_finance(...)` and persisting the row (lines + derived totals + `hype` + `profit`), and accumulating `team.cash += profit` (cash carries across Seasons, ZenGM posture). **First-season hype seed:** when no prior snapshot exists for that Team, `prev_hype = 0.0` and `winp_old = 0.5` (the ZenGM neutral-0.500 new-team default) so `compute_hype` doesn't reward/punish a phantom prior Season.

**Salary-write additions** (recompute `Player.salary` from `overall_rating`, the LG-05 `potential` precedent — both **gated on `season.league.finance_enabled`**; OFF ⇒ `salary` stays `None`, byte-identical to today): `_write_baseline_ratings(season, players)` appends `p.salary = finance.salary_for_overall(p.overall_rating)` inside the per-player loop and adds `"salary"` to the existing `Player.objects.bulk_update(players, ["potential"])` ⇒ `["potential", "salary"]`; `_develop_league_for_new_season(league, new_season, latest_completed)` recomputes `player.salary = finance.salary_for_overall(player.overall_rating)` on the POST-development overall and appends `"salary"` to the develop `bulk_update` field list. (No per-`(Player, Season)` salary snapshot exists — a completed Season's payroll reads the current in-place `Player.salary`, the LG-04 "develop mutates in place; the season row is the audit trail" posture; `PlayerSeasonRating.overall_rating` could derive a snapshot payroll later if exact fidelity is ever required.)

**`_ensure_owner_evaluations` change (the money seam).** At the `money_delta = 0.0` (~L3050) and `money_total = 0.0` (~L3054) sites, when `league.finance_enabled` read the managed Team's `TeamSeasonFinance.profit` for that Season → `money_delta = finance.money_delta(profit)`, then `money_total = owner_mood.cap_cumulative(running_money, money_delta)` (the existing `running_money` thread, ~L3014/3031/3084, is already wired). Toggle OFF (or no finance row) ⇒ keep `money_delta = 0.0` / `money_total = 0.0` **exactly as today (byte-identical)**. The verdict already sums `money` in `MoodTotals` / `MoodDeltas` — **no `owner_mood` change**.

**Rollover ORDER (LOCKED) in `next_season`** (the verdict gate, `@transaction.atomic`): **(1)** `_ensure_team_finances(league, latest_completed)` (finance rows FIRST — they feed the money axis), **(2)** `_ensure_owner_evaluations(league, latest_completed)` (now reads the finance profit), **(3)** read the verdict; fired-and-unreassigned ⇒ redirect to `new_team_picker`, else `_run_season_rollover` (→ `_develop_league_for_new_season` → develop + resalary). The `_ensure_team_finances` call is inserted immediately **before** the existing `_ensure_owner_evaluations` line; `league_create` has no completed Season so it gets only the gated baseline salary write (no `_ensure_team_finances` call at create).

**Create form.** `matches.forms.CreateLeagueForm` gains `finance_enabled = forms.BooleanField(required=False, initial=False, label="Enable team finances", widget=CheckboxInput(attrs={"id": "league-create-finance-enabled"}))` (appended after the LG-01j `map_pool` field, 9→10 fields; DOM id **`league-create-finance-enabled`**). `league_create` passes `finance_enabled=form.cleaned_data["finance_enabled"]` into `League.objects.create(...)` inside the existing `@transaction.atomic`; `templates/leagues/create.html` renders one `{{ form.finance_enabled }}` row.

**UI — two screens (flip the LG-01h placeholders live, the LG-01z pattern).** Two NEW view modules in `matches/league_screens/`, both re-exported from `__init__.py` and both following the shared LG-01z contract (405 GET-guard first line, `get_object_or_404(League)`, `request.session["last_league_id"] = league.id`, `displayed_season = league.active_season or league.seasons.filter(state="completed").order_by("-id").first()`, `_build_league_sidebar_links(...)`, empty-state notice when no Season, and a **"Finances are disabled for this League" notice (DOM id `*-disabled-notice`)** in place of the body when `not finance_enabled`): **`team_finances.py`** → `team_finances(request, league_id)` (GET + a budget-edit POST branch; **keyed on `current_team`** via the LG-01g resolver chain — default `league.current_team`, optional `?team_id=` override, like `team_roster` / `team_stats`; renders the managed Team's `TeamSeasonFinance` history + current budget levels + ticket price + cash + live luxury-tax/min-payroll/payroll figures; the **budget-edit POST handler** writes `Team.budget_scouting/coaching/facilities` + `ticket_price` clamped `[1, MAX_LEVEL]` for levels, then redirects to the bare URL — the LG-01z `watch_list_toggle` POST-then-redirect precedent; finance OFF ⇒ the edit form is hidden/inert) and **`league_finances.py`** → `league_finances(request, league_id)` (GET-only, the league-wide revenue/profit/cash/payroll-per-enrolled-Team table from the `displayed_season`'s `TeamSeasonFinance` rows). **URL names + paths** (`matches/league_urls.py`, after the LG-01z live-screen block): `path("<int:league_id>/finances/", league_screens.league_finances, name="league_finances")` (replaces the placeholder `coming_soon_finances`) + `path("<int:league_id>/team/finances/", league_screens.team_finances, name="team_finances")` (replaces `coming_soon_team_finances`). **`_FEATURE_REGISTRY` drops 2 entries** — `"league_finances"` (~L1870) + `"team_finances"` (~L1880) both flip live, so their blocker entries are deleted (`players_trade` / `players_trading_block` keep their "needs the salary/contract model" blocker copy — FIN-01 adds salary but NOT contracts/cap-space). **Sidebar/topbar repoint** (`_build_league_sidebar_links`, the single source for BOTH the LG-01f sidebar AND the LG-01k topbar — both flip at once, the LG-01z precedent): LEAGUE > Finances (~L1706) `_cs("coming_soon_finances")` → `_cs("league_finances")`; TEAM > Finances (~L1717) `_cs("coming_soon_team_finances")` → `_cs("team_finances")`. **Templates** `templates/leagues/team_finances.html` / `league_finances.html` (the `d-flex` + `_partials/league_sidebar.html` shell). **LOCKED DOM ids:** Team Finances — `team-finances-table`, `team-finances-budget-form` / `team-finances-budget-scouting` / `team-finances-budget-coaching` / `team-finances-budget-facilities` / `team-finances-ticket-price` / `team-finances-budget-save`, `team-finances-empty-notice`, `team-finances-disabled-notice`; League Finances — `league-finances-table`, `league-finances-empty-notice`, `league-finances-disabled-notice`. **ZenGM-style Team Finances layout** (modelled on `Screenshots_and_video_examples/league_team_finances_view.png`): the rendered `team is not None` branch surfaces a `${{ live_payroll }}k` **payroll headline** (id `team-finances-payroll-headline`) + the hype→revenue explainer, a `team-finances-metrics` tile row (**Wins** record / **Hype** bar / **Revenue** / **Expenses** / **Cash**), the budget form restyled as an **Expense Settings** table (each of the three levels annotated with its `level_to_amount` `$`/season cost via the `budget_costs` context dict + the neutral-`DEFAULT_LEVEL` hint; the LOCKED ids above are preserved on the inputs/save button, the save label reads "Save Expense Settings"), and a `team-finances-salaries-table` **Player Salaries** table (per-active-roster `salary_for_overall` + a Total row) — all additive presentational ids/context (`roster_salaries` / `roster_salary_total` / `budget_costs` / `wins` / `losses` / `current_hype` / `latest_finance` / `default_level`); the budget model stays 1–100 levels (no dollar-input model change) and the screen reuses the league-views `_team_wins_games_for_season` helper for the record tile.

**Scope-out (LOCKED).** **Cost-only this slice** — scouting & coaching buy NO gameplay edge yet (wiring the coaching budget into LG-04 development = **FIN-02**, the scouting budget into LG-05 potential = **FIN-03**). **Health budget + injury/availability = FIN-04** (the fourth ZenGM budget; only three ship here). **Tax redistribution SKIPPED** (luxury tax is charged but not paid back out to under-cap teams). **Luxury-tax challenge-mode firing DEFERRED to FIN-05** (the term is written, no toggle). **No contracts / free agency / cap space** (salary is derived from `overall_rating`; `players_trade` / `players_trading_block` stay blocked). **Fixed single market** (no `pop` / popRank / per-game gauss variance). **Determinism / no re-baseline:** finances consume no RNG, are outside SIM-07/08, change no simulation mechanic ⇒ **NO Score Calibration re-baseline**, and LG-04/LG-05 develop output is byte-identical toggle ON or OFF. **No new CONTEXT.md term** (the `### Finance` glossary + the two "money dormant" caveat edits were finalised at the grill).

**Locked names (quick index).** Pure module `matches/finance.py`; constants `MAX_LEVEL` / `DEFAULT_LEVEL` (34) / `BUDGET_LEVEL_SCALE` / `BASELINE_SALARY_CAP` / `SALARY_CAP` / `EXPECTED_PROFIT_BASE` (15) / `MONEY_DELTA_DIVISOR` (100) / `LUXURY_PAYROLL` / `LUXURY_TAX_RATE` (1.5) / `MIN_PAYROLL` + revenue/expense coefficients; dataclasses `RevenueLines` / `ExpenseLines` / `TeamFinanceResult`; fns `level_to_amount` / `salary_for_overall` / `compute_hype` / `season_revenue` / `season_expenses` / `luxury_tax` / `min_payroll_penalty` / `season_profit` / `money_delta` / `compute_team_finance`; money formula `(profit − 15*scf)/(100*scf)`, `scf = salary_cap / BASELINE_SALARY_CAP`; fields `teams.Player.salary`, `teams.Team.{budget_scouting,budget_coaching,budget_facilities,ticket_price,cash}`, `matches.League.finance_enabled`; model `matches.models.TeamSeasonFinance`; constraint `uniq_team_season_finance`; migrations `teams/migrations/0013_player_salary_team_finance.py` + `matches/migrations/0050_league_finance_teamseasonfinance.py`; writer `matches.league_views._ensure_team_finances`; CHANGED `_write_baseline_ratings` / `_develop_league_for_new_season` (+`salary`) / `_ensure_owner_evaluations` (money seam) / `next_season` (rollover order); form field `CreateLeagueForm.finance_enabled` (DOM id `league-create-finance-enabled`); views `matches.league_screens.team_finances` / `league_finances`; URL names `team_finances` (`/leagues/<id>/team/finances/`) / `league_finances` (`/leagues/<id>/finances/`) — replacing `coming_soon_team_finances` / `coming_soon_finances`; templates `templates/leagues/team_finances.html` / `league_finances.html` (DOM ids above). Decision: [ADR-0027](../../docs/adr/0027-team-finance-subsystem.md). Seam contract: [`.claude/worktrees/fin-01-seam-contract.md`](../../.claude/worktrees/fin-01-seam-contract.md).

**Tests.** `matches/tests/test_finance.py` (pure-unit against `finance.py`, incl. `TestNoDjangoImportsLeaked`) / `test_team_finance_model.py` (`TeamSeasonFinance` create / `uniq_team_season_finance` / `team` SET_NULL / `season` CASCADE / `Meta.ordering`; `Player.salary` + the 5 `Team` field defaults) / `test_team_finances_writer.py` (`_ensure_team_finances` one row per `(Team, Season)` in Season order, idempotent, hype carries across Seasons, first-season seed `prev_hype=0.0`/`winp_old=0.5`; salary recompute in both write sites) / `test_finance_money_axis.py` (money-axis integration — `_ensure_owner_evaluations` reads `TeamSeasonFinance.profit`, `money_delta`/`money_total` non-zero + cap-chained, profit raises / loss lowers money mood) / `test_finance_toggle.py` (the **byte-identical-when-OFF invariant** — `finance_enabled=False` writes **zero** finance rows, `Player.salary` stays `None`, `OwnerEvaluation` rows byte-identical to a no-finance run) / `test_create_form_finance.py` (`CreateLeagueForm` accepts `finance_enabled`, a checked POST persists `League.finance_enabled=True`, the DOM id renders) / `test_finance_screens.py` (`team_finances` / `league_finances` 200/405/404/session-write/empty-state/disabled-notice/DOM ids; the budget-edit POST writes `Team.budget_*` + `ticket_price` then redirects; sidebar repoints live; `_FEATURE_REGISTRY` no longer contains `league_finances` / `team_finances`). Tests assert schema-level outcomes (row counts, field values, `money_delta` sign/magnitude, DOM ids, byte-identical-OFF) — **never** raw simulated point totals (assert the finance math against a fixed standings/profit fixture).

## FIN-02 coaching budget into player development

Lights up the **coaching** budget FIN-01 shipped cost-only (the LG-04 develop knob ADR-0024 shipped **fixed at 0** precisely as this deferral). A Team's effective coaching level now **directionally scales** its players' LG-04 age-curve development at each `next_season` rollover — better coaching speeds development, neglected coaching slows it. **Backend-only** (no UI / template change). **Finance-OFF — or any `coaching_effect=0.0` — develop is byte-identical to LG-04**, and **Potential (LG-05) is untouched** (coaching never touches Potential; FIN-03 owns scouting→potential). Coaching mutates Stat *inputs* for finance-ON leagues only and changes no simulation *mechanic* ⇒ **NO Score Calibration re-baseline**. Decision: the **FIN-02 Consequences addendum on [ADR-0024](../../docs/adr/0024-zengm-player-development-ratings-history.md)** (the ADR that recorded the coaching-knob deferral — **no new ADR**). The CONTEXT.md **Coaching effect** term + the Budget / Player-development edits are already written. Seam contract: [`.claude/worktrees/fin-02-coaching-development-seam-contract.md`](../../.claude/worktrees/fin-02-coaching-development-seam-contract.md). Depends on **FIN-01**.

**Mechanism — `matches/development.py::develop_player_stats` (no new RNG draw, no new import).** The signature gains a **keyword-only** `coaching_effect: float = 0.0`: `develop_player_stats(stats, age, rng, *, coaching_effect=0.0) -> dict[str, int]`. The body computes the per-player **effective base change** `effective = base_change(age) + base_change_noise(age, rng)` exactly as LG-04 (the single `gauss` noise draw fires FIRST, unchanged), then — **between that gauss draw and the 19 `develop_stat` `uniform(0.4, 1.4)` draws** — scales it **once**: `effective *= 1 + _sign(effective) * coaching_effect`. **`_sign(x) -> float`** is a tiny NEW module-private helper returning `-1.0` / `0.0` / `+1.0` (a hand-rolled sign so **`math` is NOT imported** — the frozen `dataclasses` / `typing` / `random` / `collections` allowlist holds and `test_development.py::TestNoDjangoImportsLeaked` stays green). The `_sign` factor makes the scale **directional**: a positive `coaching_effect` pushes a *gaining* player further up and a *declining* player **toward 0** (slower decline), a negative effect does the reverse — coaching multiplies the magnitude-with-sign, it is NOT a flat add. The scaled `effective` then flows into the 19 `develop_stat(current, name, age, effective_base_change=effective, rng)` calls **as before** — **`develop_stat` is byte-unchanged**. Because the scale lands **after the gauss and before any uniform draw**, the pinned **1-gauss-then-19-uniform** sequence is **unperturbed** and a seeded develop stays reproducible; **`coaching_effect=0.0` ⇒ multiplier exactly `1.0` ⇒ byte-identical to LG-04** (this is the load-bearing OFF guarantee — pinned by a regression test).

**Mapping — `matches/finance.py` (NOT `development.py`).** NEW pure fn **`coaching_effect(level) -> float`** + constant **`MAX_COACHING_EFFECT = 0.09`**, reusing FIN-01's `_bound` + `DEFAULT_LEVEL` (34) / `MAX_LEVEL` (100): the 1–100 ZenGM level maps **linearly each side of the neutral pivot** to **`0.0` at level 34**, **`+0.09` at 100**, **`-0.045` at 1`. **The level→effect mapping is deliberately homed in `finance.py`, not `development.py`** — `development.py` keeps its frozen no-finance import allowlist and only ever sees the resolved **float** crossing the view↔pure seam; the view does the level→float resolution. (`finance.py`'s frozen `dataclasses` / `typing` / `math` / `collections` allowlist is unaffected — `coaching_effect` is plain arithmetic over `_bound`.)

**Model + migration.** `matches.models.TeamSeasonFinance` gains `budget_scouting` / `budget_coaching` / `budget_facilities` (`PositiveSmallIntegerField(default=34)` — the FIN-01 ZenGM neutral level) + `games_played` (`PositiveSmallIntegerField(default=0)`), all four inserted **between the `hype` field and `created_at`**, in **`matches/migrations/0051_teamseasonfinance_budget_levels.py`** (dep `0050_league_finance_teamseasonfinance`; **4× `AddField`, NO `RunPython` / `RunSQL` / backfill** — the ADR-0004 disposable-data posture, the `0050` precedent). The snapshot now carries the per-Season **budget levels** AND the **games count** so the multi-Season games-weighted average has its inputs on the immutable row (the level on a `Team` can drift after the Season; the snapshot freezes what it was).

**Writer + wiring (`matches/league_views.py`).** `_ensure_team_finances` (the FIN-01 writer) now **also snapshots the four new fields** when it builds each `(Team, Season)` row — `budget_scouting` / `budget_coaching` / `budget_facilities` copied from the `Team`, and `games_played` = that Team's **regular-season `matches_played`** for the Season (read via `Season._final_standings_for_phase(...)`, the same standings source FIN-01 uses for `winp`). NEW **`_coaching_effect_by_team(league, latest_completed) -> dict[int, float]`** — **gated on `league.finance_enabled` (OFF ⇒ returns `{}`)** — computes, per enrolled Team, a **games-weighted mean of `budget_coaching` over that Team's last ≤3 completed-Season `TeamSeasonFinance` rows** (`Sum(level * games) / Sum(games)`, with a **current-level fallback** when a Team has no finance history yet / `Sum(games) == 0`), then maps that mean through `finance.coaching_effect(...)` to a float. This honours the CONTEXT.md **Budget level** multi-Season-average contract (ZenGM's `getLevelLastThree` smoothing). The dict is threaded into `_develop_league_for_new_season`'s develop call as **`coaching_effect=coaching_by_team.get(player.team_id, 0.0)`** — **active + bench players get their Team's effect; free-agent-pool players get `0.0`** (they have no `team_id`, so the `.get(..., 0.0)` default applies). **Call ORDER (LOCKED):** `_ensure_team_finances` runs **before** the rollover/develop (the FIN-01 rollover order already places it first in `next_season`), so `latest_completed`'s freshly-written snapshot is already inside the ≤3-Season window `_coaching_effect_by_team` reads.

**Scope-out (LOCKED).** **Potential untouched** — LG-05's `_project_stat_noise_free` / `compute_potential` are byte-unchanged; coaching feeds ONLY the LG-04 develop curve (scouting→potential = **FIN-03**). **Backend-only** — no view/template/DOM/form change (the Team Finances screen already renders `budget_coaching`). **`develop_stat` byte-unchanged** (the scale lives in `develop_player_stats`, not the per-stat fn). **No new RNG draw** — the multiplier reuses the already-drawn `effective`; the 1-gauss-then-19-uniform order holds. **The level→effect mapping is in `finance.py`, never `development.py`** (the no-finance-import rule for the develop module). **NO Score Calibration re-baseline** (Stat inputs only, finance-ON only; finance-OFF byte-identical). **No new ADR** (FIN-02 Consequences addendum on ADR-0024). **No new CONTEXT.md term beyond the already-written Coaching effect** entry.

**Locked names (quick index).** Pure fns `matches/development.py::develop_player_stats(stats, age, rng, *, coaching_effect=0.0)` (CHANGED — keyword-only arg + directional scale), `matches/development.py::_sign(x)` (NEW, no `math` import); `matches/finance.py::coaching_effect(level)` (NEW) + constant `MAX_COACHING_EFFECT = 0.09` (NEW); model fields `matches.TeamSeasonFinance.{budget_scouting, budget_coaching, budget_facilities, games_played}` (NEW, between `hype` and `created_at`); migration `matches/migrations/0051_teamseasonfinance_budget_levels.py` (dep `0050_league_finance_teamseasonfinance`, 4× `AddField`); writer `matches.league_views._ensure_team_finances` (CHANGED — snapshots the 4 new fields) + `_coaching_effect_by_team(league, latest_completed) -> dict[int, float]` (NEW, gated on `finance_enabled`, games-weighted mean over last ≤3 Seasons) + `_develop_league_for_new_season` (CHANGED — threads `coaching_effect=coaching_by_team.get(player.team_id, 0.0)`). Decision: [ADR-0024](../../docs/adr/0024-zengm-player-development-ratings-history.md) (FIN-02 Consequences addendum). Seam contract: [`.claude/worktrees/fin-02-coaching-development-seam-contract.md`](../../.claude/worktrees/fin-02-coaching-development-seam-contract.md).

**Tests.** `matches/tests/test_finance.py` (`coaching_effect` mapping — `0.0` at 34 / `+0.09` at 100 / `-0.045` at 1, `MAX_COACHING_EFFECT`) / `test_development.py` (the **directional scale** — positive effect speeds a gaining player + slows a decliner; **`coaching_effect=0.0` byte-identical to the no-arg call**; the **no-new-RNG-draw invariant** — same seed yields the same 1-gauss-then-19-uniform consumption with and without a non-zero effect) / `test_team_finances_writer.py` (the 4 new snapshot fields persist; `games_played` = the Team's regular-season `matches_played`) / `test_league_next_season.py` (`_coaching_effect_by_team` games-weighted mean over ≤3 Seasons + current-level fallback; **OFF ⇒ `{}`**; **free-agent-pool players develop with `0.0`**; active/bench get the Team effect). Tests assert schema-level + math outcomes against fixed fixtures — **never** raw simulated point totals.

## FIN-03 scouting budget into player potential

Lights up the **scouting** budget FIN-01 shipped cost-only (the LG-05 `compute_potential` scouting-noise band CAR-01 shipped **fixed at `DEFAULT_SCOUTING_BUDGET = 50`** precisely as this deferral). A Team's effective scouting level now sets the **width of its players' LG-05 Potential-estimate band** at each `next_season` rollover (and at founding) — better scouting **tightens** the Potential estimate around the noise-free projection, neglected scouting **widens** it. The **structural mirror of FIN-02** (coaching→development): same finance-ON-only gating, same games-weighted ≤3-Season budget-level read, same level→float resolution homed in `finance.py`. **Backend-only** (no UI / template change; the Team Finances screen already renders `budget_scouting`). **Finance-OFF — or any `scouting_budget=50` — Potential is byte-identical to LG-05**, and **development (LG-04) is untouched** (scouting never touches realised Stats; FIN-02 owns coaching→development). Scouting changes only the **seeded Potential *estimate*** for finance-ON leagues — read-only to the simulator, never a realised stat — and changes no simulation *mechanic* ⇒ **NO Score Calibration re-baseline**. **No migration** — `TeamSeasonFinance.budget_scouting` / `.games_played` (FIN-02) and `Team.budget_scouting/coaching/facilities` (FIN-01) already exist. Decision: the **FIN-03 Consequences addendum on [ADR-0027](../../docs/adr/0027-team-finance-subsystem.md)** (the ADR that recorded the scouting-knob deferral — **no new ADR**). The CONTEXT.md edits (Potential pointer CAR-01→FIN-03, Budget / Budget-level avoid-lines, the new **Scouting estimate edge** term) are already written. Seam contract: [`.claude/worktrees/fin-03-seam-contract.md`](../../.claude/worktrees/fin-03-seam-contract.md). Depends on **FIN-01**.

**Mapping — `matches/finance.py` (NOT `development.py`).** NEW pure fn **`scouting_budget(level: int) -> float`** + constants **`NEUTRAL_SCOUTING_BUDGET = 50.0`** / **`MAX_SCOUTING_BUDGET = 100.0`**, reusing FIN-01's `_bound` + `DEFAULT_LEVEL` (34) / `MAX_LEVEL` (100). Unlike FIN-02's dual-slope neutral-pivot `coaching_effect`, this is a **single-slope** map anchored at `DEFAULT_LEVEL`→neutral and `MAX_LEVEL`→max: `NEUTRAL_SCOUTING_BUDGET + (MAX_SCOUTING_BUDGET − NEUTRAL_SCOUTING_BUDGET) * (lvl − DEFAULT_LEVEL) / (MAX_LEVEL − DEFAULT_LEVEL)`, clamping the input `level` to `[1, MAX_LEVEL]` via `_bound` (the result already lands inside `[0, 100]`). Yields **level 1 → 25.0, 34 → 50.0, 100 → 100.0**. `NEUTRAL_SCOUTING_BUDGET = 50.0` deliberately just **equals** LG-05's `development.DEFAULT_SCOUTING_BUDGET = 50` by value — **`finance.py` MUST NOT import `development`**; the frozen `dataclasses` / `typing` / `math` / `collections` allowlist (NO Django, the `TestNoDjangoImportsLeaked` guard) is unchanged.

**`compute_potential` — UNCHANGED (LG-05).** `development.compute_potential(stats, age, rng, *, scouting_budget=DEFAULT_SCOUTING_BUDGET)`, `DEFAULT_SCOUTING_BUDGET = 50`, and `POTENTIAL_MAX_SD = 8.0` are consumed **verbatim**. The band is `sd = POTENTIAL_MAX_SD * (1 − scouting_budget/100)` (budget 100 ⇒ sd 0 ⇒ tightest estimate; budget 0 ⇒ widest), and **`compute_potential` always consumes EXACTLY ONE `rng.gauss` draw regardless of `scouting_budget`** — the band *width* never perturbs the `pot_rng` stream (already true in LG-05; the LG-05 regression already pins it). FIN-03 only ever **threads a different float** into the existing keyword arg.

**Wiring (`matches/league_views.py`).** NEW **`_scouting_budget_by_team(league, latest_completed) -> dict[int, float]`** — the **twin of `_coaching_effect_by_team`**, **gated on `league.finance_enabled` (OFF ⇒ returns `{}`)**: per developing Team (from `latest_completed.starting_team_ids_json`), the **games-weighted mean of `budget_scouting` over that Team's last ≤3 completed-Season `TeamSeasonFinance` rows** (`Sum(level * games) / Sum(games)`, with a **current-`Team.budget_scouting` fallback** when a Team has no finance history yet / `Sum(games) == 0`), then mapped through `finance.scouting_budget(...)` to a float (the ZenGM `getLevelLastThree` smoothing, honouring the CONTEXT.md **Budget level** multi-Season-average contract). Threaded into **CHANGED `_develop_league_for_new_season`** as `scouting_budget=scouting_by_team.get(player.team_id, development.DEFAULT_SCOUTING_BUDGET)` on the LG-05 `compute_potential(...)` call (alongside FIN-02's `coaching_by_team = ...` line) — free-agent-pool players (no `team_id`) take the `.get(..., 50)` default. **CHANGED `_write_baseline_ratings`** — the founding pass has **no completed Season**, so it does NOT call `_scouting_budget_by_team`; instead, when `season.league.finance_enabled`, it builds a per-team **current-level** band map `{tid: finance.scouting_budget(team.budget_scouting)}` over the founding teams and threads `scouting_budget=band_map.get(p.team_id, development.DEFAULT_SCOUTING_BUDGET)` into the baseline `compute_potential(...)` call (pool players / finance OFF ⇒ default 50). **Finance OFF ⇒ `{}` / no band map ⇒ every `.get` yields 50 ⇒ byte-identical to LG-05** (the load-bearing OFF guarantee, pinned by a regression test).

**Strength-seeding AI + manager budgets at create — NEW `_seed_team_budgets_by_strength(teams) -> None` (`matches/league_views.py`).** The user-decided scope addition: at **`league_create`, finance-ON only, BEFORE `_write_baseline_ratings`** (so the baseline reads the seeded levels), every enrolled Team — **including `League.current_team`** (the manager edits theirs later) — gets a starting budget seeded by talent. Ranks the teams by **mean active-roster `overall_rating` desc** (tie-break `team_id` **asc**, matching the existing talent ranking; via `Team.active_players` + `Player.overall_rating`), assigns a **rank-linear** level across the band **`[SEED_BUDGET_MIN, SEED_BUDGET_MAX] = [20, 90]`** (rank 0 strongest → 90, rank N−1 weakest → 20; **single team ⇒ `SEED_BUDGET_SINGLE = 55`**; rounded to int), sets the **SAME** level on **all three** `budget_scouting` / `budget_coaching` / `budget_facilities`, and persists via `Team.objects.bulk_update(teams, ["budget_scouting", "budget_coaching", "budget_facilities"])`. Pin the band + single-team fallback as tunable module constants **`SEED_BUDGET_MIN = 20` / `SEED_BUDGET_MAX = 90` / `SEED_BUDGET_SINGLE = 55`**. Seeded **ONCE at create, frozen forever** — `next_season` carries the Team rows forward untouched (**NO re-seed call in `next_season`**); CPU teams never adjust their budgets this slice (a future "CPU teams adjust budgets" feature is explicitly **deferred**), and the manager may edit their own Team's levels later on the Team Finances screen.

**Scope-out (LOCKED).** **Development untouched** — LG-04's `develop_player_stats` / `develop_stat` are byte-unchanged; scouting feeds ONLY the LG-05 Potential-estimate band (coaching→development = **FIN-02**). **Estimate-precision only** — scouting changes the *seeded Potential estimate*, never a realised Stat, and is read-only to the simulator. **`compute_potential` byte-unchanged** — FIN-03 only threads a different float into the existing `scouting_budget` kwarg; the **one-gauss-draw** guarantee is LG-05's, unperturbed. **The level→float mapping is in `finance.py`, never `development.py`** (the no-finance-import rule for the develop module; `NEUTRAL_SCOUTING_BUDGET` only equals LG-05's `DEFAULT_SCOUTING_BUDGET` by value). **AI + manager budgets strength-seeded ONCE at create, frozen for CPU teams** (manager-editable; "CPU teams adjust budgets" deferred). **NO migration** (the snapshot + Team fields already exist), **NO simulator change**, **NO Score Calibration re-baseline** (estimate-only, finance-ON only; finance-OFF byte-identical to LG-05). **No new ADR** (FIN-03 Consequences addendum on ADR-0027). **No new CONTEXT.md term beyond the already-written Scouting estimate edge** entry.

**Locked names (quick index).** Pure fn `matches/finance.py::scouting_budget(level: int) -> float` (NEW, single-slope: 1→25.0 / 34→50.0 / 100→100.0) + constants `NEUTRAL_SCOUTING_BUDGET = 50.0` / `MAX_SCOUTING_BUDGET = 100.0` (NEW; `finance.py` stays no-`development`-import); `matches/development.py::compute_potential` / `DEFAULT_SCOUTING_BUDGET` (50) / `POTENTIAL_MAX_SD` (8.0) UNCHANGED; helpers `matches.league_views._scouting_budget_by_team(league, latest_completed) -> dict[int, float]` (NEW, gated on `finance_enabled`, games-weighted mean over last ≤3 Seasons, current-level fallback, OFF ⇒ `{}`) + `_seed_team_budgets_by_strength(teams) -> None` (NEW, rank-linear `[20, 90]` desc by mean overall, `team_id` asc tiebreak, single-team→55, all three budget fields equal, every team incl. `current_team`, `bulk_update`) + constants `SEED_BUDGET_MIN = 20` / `SEED_BUDGET_MAX = 90` / `SEED_BUDGET_SINGLE = 55` (NEW); CHANGED `_develop_league_for_new_season` (threads `scouting_budget=scouting_by_team.get(tid, development.DEFAULT_SCOUTING_BUDGET)`) / `_write_baseline_ratings` (current-level band map, finance-ON) / `league_create` (the `_seed_team_budgets_by_strength` call, finance-ON, before `_write_baseline_ratings`). Decision: [ADR-0027](../../docs/adr/0027-team-finance-subsystem.md) (FIN-03 Consequences addendum). Seam contract: [`.claude/worktrees/fin-03-seam-contract.md`](../../.claude/worktrees/fin-03-seam-contract.md).

**Tests.** `matches/tests/test_finance.py` (`scouting_budget` mapping — `25.0` at 1 / `50.0` at 34 / `100.0` at 100, clamp below 1 / above 100; `NEUTRAL_SCOUTING_BUDGET` / `MAX_SCOUTING_BUDGET`) / `test_league_create.py` (`_seed_team_budgets_by_strength` — rank-linear `[20, 90]` desc by mean overall with `team_id` asc tiebreak, single-team→55, all three budget fields set equal, `current_team` included; seeded only finance-ON, before baseline) / `test_league_next_season.py` (`_scouting_budget_by_team` games-weighted mean over ≤3 Seasons + current-level fallback; **OFF ⇒ `{}`**; **byte-identical-OFF** baseline + develop Potential rows vs an LG-05/no-finance run, pool players default 50; **AI-budget-frozen-across-rollover** — `next_season` does NOT re-seed, carried Team budget rows unchanged) — all **extended**, not new files. Tests assert schema-level + math outcomes against fixed fixtures — **never** raw simulated point totals.

## FIN-04 health budget + injury/availability

The **fourth ZenGM budget** — the one FIN-01 deferred and the only one of the four with **no existing seam** in our domain (we had no injuries before). A per-Team **health budget** (a cost line like the other three, plus a ratings edge expressed as injury *duration*) backed by a minimal **injury / availability** model: injuries are rolled **OUTSIDE the tick loop**, resolved **in-memory at fixture time** (auto-substitute or play-hurt), with a per-Player availability counter decremented once per fixture the Team plays. Gated on **`_is_career_league(season.league) AND season.league.finance_enabled`** — OFF (or sandbox / multiplayer) ⇒ no rolls, no roster mutation, no `games_unavailable` change ⇒ **byte-identical to today**. The simulator is **byte-untouched** (no signature change, no `before_round_hook`) ⇒ **NO Score Calibration re-baseline**. Regular-season **round-robin fixtures only** — tournaments/playoffs are scope-out and stay untouched. Decision: [ADR-0028](../../docs/adr/0028-health-budget-injury-availability.md). Seam contract: [`.claude/worktrees/fin-04-health-injury-seam-contract.md`](../../.claude/worktrees/fin-04-health-injury-seam-contract.md). Depends on **FIN-01** (the budget-level + finance-toggle infrastructure).

**Model + migrations (AddField-only, NO `RunPython`/backfill — the ADR-0004 disposable-data posture).** `teams.Player.games_unavailable` (`PositiveSmallIntegerField(default=0)`, declared after the FIN-01 `salary` field) is the **availability counter**: `> 0` ⇒ the player is out; decremented 1 per fixture the player's Team plays (whether sub'd out or playing hurt); **reset to 0 at `next_season` rollover**. No injury-type taxonomy — a single integer countdown. `teams.Team.budget_health` (`PositiveSmallIntegerField(default=34)` — the `finance.DEFAULT_LEVEL` neutral level, mirroring `budget_scouting/coaching/facilities`) is the **fourth budget level**; `teams.Team.injury_policy` (`CharField(max_length=16, choices=INJURY_POLICY_CHOICES, default="auto_sub")`, class attr `INJURY_POLICY_CHOICES = (("auto_sub", "Auto-substitute"), ("play_hurt", "Play hurt"))`) is the manager-editable per-Team policy (AI teams stay `"auto_sub"`). `matches.TeamSeasonFinance.health_cost` (`FloatField(default=0.0)`) is the new expense line, declared **immediately after `min_payroll_penalty`** (before the derived `revenue`). Ship in **`teams/migrations/0014_player_team_health_injury.py`** (dep `0013_player_salary_team_finance`; three `AddField` — `Player.games_unavailable`, `Team.budget_health`, `Team.injury_policy`) + **`matches/migrations/0052_teamseasonfinance_health_cost.py`** (dep `0051_teamseasonfinance_budget_levels`; one `AddField` — `TeamSeasonFinance.health_cost`; the cross-app `teams 0014` dep is implicit via no FK, so only the matches dep is declared).

**Pure module `matches/injury.py` (Django-free).** **Frozen import allowlist** `dataclasses` / `typing` / `random` / `collections` ONLY — NO `django.*` / ORM / `datetime` / `math` / I/O / logging / `teams` / `matches` imports (defended by `matches/tests/test_injury.py::TestNoDjangoImportsLeaked`, the subprocess fresh-import + `sys.modules` walk, a `SimpleTestCase` — the `test_finance.py` / `test_development.py` precedent). `random` is allowlisted because the draws consume an **INJECTED** `random.Random` (production builds a **fresh one per fixture**, never the SIM-07/08 seed chain). The level→float **`health_effect`** map lives in `finance.py` (below), NOT here — `injury.py` **consumes** the float (the FIN-02/03 rule: the level→float resolution is homed in `finance.py`, the pure module sees only the resolved float). **Constants (LOCKED-but-tunable, calibration-deferred — the LG-04 age-curve / FIN-01 magic-number precedent):** `BASE_INJURY_RATE = 0.04` (flat per-fixture base probability a healthy fielding starter is injured, before age), `AGE_FACTOR_PIVOT = 27` (age where the age factor is 1.0), `AGE_FACTOR_PER_YEAR = 0.04` (linear age-factor slope per year past the pivot — older ⇒ more injury-prone), `AGE_FACTOR_MIN = 0.5` / `AGE_FACTOR_MAX = 2.5` (age-factor clamp), `DURATION_BASE_GAMES = 3.0` (mean drawn duration in matchdays before health-edge/age scaling), `DURATION_MIN_GAMES = 1` / `DURATION_MAX_GAMES = 12` (drawn-duration clamp), `PLAY_HURT_STAT_PENALTY = 12` (flat magnitude subtracted from each of the injured Player's 19 stat fields when fielded hurt, clamped `[0, 100]` by the caller). **Public fns (every signature + return type LOCKED):** `age_factor(age: int) -> float` (`1.0` at `AGE_FACTOR_PIVOT`, linear `AGE_FACTOR_PER_YEAR` each side, clamped `[AGE_FACTOR_MIN, AGE_FACTOR_MAX]`; a `None` age never reaches it — the caller coalesces `None → 25`, the LG-04 convention; consumes NO RNG), `injury_probability(age: int) -> float` (`BASE_INJURY_RATE * age_factor(age)` — **no Stat input**, flat base × age only; consumes NO RNG), `roll_injury(age: int, rng: random.Random) -> bool` (`rng.random() < injury_probability(age)` — exactly **ONE** `rng.random()` draw), `draw_duration(health_effect: float, age: int, rng: random.Random) -> int` (draws a duration in matchdays, scales it **DOWN** by `health_effect` — frequency is fixed, only duration is health-scaled — clamps `[DURATION_MIN_GAMES, DURATION_MAX_GAMES]`, returns int ≥ 1; exactly **ONE** RNG draw; a positive edge shortens the draw), `play_hurt_penalty() -> int` (returns `PLAY_HURT_STAT_PENALTY`; consumes NO RNG). The **RNG-consumption order across one starter roll is PINNED**: `roll_injury` (1 draw) THEN, only if injured, `draw_duration` (1 draw) — so seeded tests are deterministic.

**`finance.py` additions.** NEW pure fn **`health_effect(level: int) -> float`** — the sign-flipped ZenGM `healthEffect` analogue: `DEFAULT_LEVEL` (34) neutral → `0.0`, `MAX_LEVEL` (100) → `MAX_HEALTH_EFFECT`, level 1 → a negative effect; reuses `_bound` + `DEFAULT_LEVEL` / `MAX_LEVEL`, mirrors `coaching_effect`'s dual-slope neutral pivot. NEW constant **`MAX_HEALTH_EFFECT = 0.5`** (LOCKED-but-tunable, calibration-deferred). The level→float mapping lives HERE (the FIN-02/03 rule); `injury.py` consumes the float. `finance.py`'s frozen `dataclasses` / `typing` / `math` / `collections` allowlist is **unaffected** (plain arithmetic over `_bound`). The frozen **`ExpenseLines`** dataclass gains a trailing **`health: float`** field, appended LAST (pinned order: `payroll, scouting, coaching, facilities, luxury_tax, min_payroll_penalty, health`). **`season_expenses(...)`** gains a keyword-only `health_level: int` parameter (appended after `facilities_level`, before `salary_cap`) and sets `health=level_to_amount(health_level, salary_cap)`. **`compute_team_finance(...)`** gains a keyword-only `health_level: int` parameter (after `facilities_level`), threads it into `season_expenses(...)`, and adds `expense_lines.health` to the `expenses` sum (the **seventh expense line**). **`TeamFinanceResult` is UNCHANGED in shape** — the new `health` expense flows through `expenses` / `profit` automatically. `finance.level_to_amount(budget_health)` is the health-budget cost (the same per-level dollar map as the other three budgets) — **no money-formula change**.

**Per-fixture seam — `matches/league_views.py`.** Two NEW module-level helpers wrap each `simulate_scheduled_round(...)` call. **`resolve_injuries_for_fixture(season, team_red, team_blue) -> dict`** is the single per-fixture injury resolver, called BEFORE the sim; its **first-line guard** `if not (_is_career_league(season.league) and season.league.finance_enabled): return {}` ⇒ no-op, byte-identical OFF. It returns an opaque **restore token** (the data needed to undo the in-memory mutations) which the caller passes to **`restore_after_fixture(token) -> None`**, which undoes every in-memory mutation (Team `slot_*` FKs and injured-Player 19 stat fields) restoring the pre-fixture state. **NEVER `.save()` the temporary roster** — the only persisted writes are the `games_unavailable` decrement/set, done explicitly with `update_fields` / `bulk_update`. **Per-fixture sequence (LOCKED order)**, for each of the two Teams: **(1) GATE** — return `{}` when not `_is_career_league AND finance_enabled`. **(2) SUBJECTS = the 6 active-roster STARTERS only** — the `slot_*` players as they stood BEFORE substitution (`team.active_roster` snapshot); fill-ins (bench / free-agent) are NEVER injury subjects (never roll, never tracked) so no orphan injuries arise. **(3) DECREMENT** — a starter with `games_unavailable > 0` decrements by 1 (whether sub'd out or playing hurt) and does **NOT** re-roll; persisted via `Player.objects.bulk_update(..., ["games_unavailable"])`. **(4) ROLL** — a healthy starter (`games_unavailable == 0`) that actually fields rolls a NEW injury via `injury.roll_injury(age, rng)`; on a hit, draws `N = injury.draw_duration(health_effect, age, rng)` and sets `games_unavailable = N` (persisted); `rng` is a **fresh `random.Random()` per fixture** (never the SIM seed chain), and `health_effect = finance.health_effect(team.budget_health)` read from the **LIVE current `Team.budget_health`** at fixture time. **(5) RESOLVE roster to a valid 6** — for each now-unavailable starter, apply the Team's `injury_policy`: **`auto_sub`** rewrites the in-memory Team `slot_*` FK to a substitute, source priority **bench (`Team.bench_players`) → League free-agent pool (`League.free_agent_pool`)** (available = `games_unavailable == 0`), with **`play_hurt` as the universal no-sub fallback** — when no available sub exists the injured starter plays hurt, so the injury ALWAYS resolves to a valid 6-roster; **`play_hurt`** rewrites the injured in-memory Player's 19 stat fields down by `injury.play_hurt_penalty()` (clamp `[0, 100]`) with the starter staying in its slot. **(6)** the two restore lists (mutated Team `slot_*` FKs; mutated Player stat fields + their originals) go into the token; the caller runs the sim against the mutated in-memory state, then `restore_after_fixture(token)`. **Roster resolution lives IN THE PLAY LOOP via in-memory mutate-then-restore** — NO simulator signature change, NO new `before_round_hook`; `_simulate_and_flush_round` reads `list(team.active_roster)` off the passed-in Team (confirmed at `matches/simulation/entrypoints.py:580-581`), so the in-memory `slot_*` / stat rewrites take effect for that round's sim and are restored after.

**Effective-level read (LIVE, NOT smoothed).** Injury frequency/duration reads `finance.health_effect(Team.budget_health)` from the **LIVE current `Team.budget_health`** at fixture time — **NOT** the games-weighted ≤3-Season smoothing that FIN-02 (`_coaching_effect_by_team`) and FIN-03 (`_scouting_budget_by_team`) use, so health responds to the manager's current setting immediately. Frequency is a fixed base rate × age; `health_effect` scales the drawn **DURATION** down only.

**Call sites (LOCKED).** Wrap each `simulate_scheduled_round(...)` call in these THREE paths with `token = resolve_injuries_for_fixture(season, team_red, team_blue)` before and `restore_after_fixture(token)` after (in a `finally`): **`matches/tasks.py::play_season_task`** (the per-fixture loop, ~L236), **`matches/league_views.py::play_week`** (the per-fixture loop, ~L2538), and the **LG-01i live path `matches/league_views.py::play_week_live`** — the **RR branch** `simulate_scheduled_round(...)` call (~L2765). The playoff branch (`play_specific_node` → `simulate_match`, tournament) is **UNTOUCHED** (tournaments/playoffs are scope-out). **`next_season` reset pass:** in `matches/league_views.py::_develop_league_for_new_season` (the develop loop already inside `_run_season_rollover`'s `@transaction.atomic`, over the developing set = active + bench + free-agent-pool players), each developing Player gets `player.games_unavailable = 0`, and `"games_unavailable"` is appended to the existing `Player.objects.bulk_update(players, [...])` field list.

**Gating + `_ensure_team_finances` change (byte-identical OFF).** `resolve_injuries_for_fixture` AND the health-cost write are gated FIRST-LINE on `_is_career_league(season.league) and season.league.finance_enabled` ⇒ OFF (or sandbox / multiplayer) means no rolls, no roster mutation, no `games_unavailable` change, byte-identical to today. The `_ensure_team_finances` writer already early-returns when `not _is_career_league(league) or not league.finance_enabled`, so the new thread is inside that gate: the `compute_team_finance(...)` call gains `health_level=team.budget_health`, and the `TeamSeasonFinance.objects.get_or_create(... defaults={...})` block gains `"health_cost": result.expense_lines.health`.

**UI — Team Finances screen (`matches/league_screens/team_finances.py::team_finances` + `templates/leagues/team_finances.html`, both already FIN-01-live).** The budget-edit POST handler reads + clamps `request.POST.get("budget_health")` via the existing `_coerce_level(...)` into `team.budget_health`, reads `request.POST.get("injury_policy")` (accept only `"auto_sub"` / `"play_hurt"`, else keep current) into `team.injury_policy`, and adds `"budget_health"` / `"injury_policy"` to the `team.save(update_fields=[...])` list (inert when finance OFF or no Team). Context adds `budget_costs["health"] = finance.level_to_amount(team.budget_health)`, `injury_policy=team.injury_policy`, and `availability` — a `list[{name, games_remaining}]` of the Team's players with `games_unavailable > 0` (by `games_unavailable` desc; `[]` when none). **LOCKED DOM ids:** `team-finances-budget-health` (the `<input>` for `budget_health`, in `team-finances-budget-form` alongside the existing scouting/coaching/facilities inputs), `team-finances-injury-policy` (the `<select name="injury_policy">` toggle, `auto_sub` / `play_hurt`, in the same form), `team-finances-availability` (the availability-display container rendered from `availability`) with a `team-finances-availability-empty` notice when the list is empty. Manager-editable (FIN-01 pattern); all existing FIN-01 DOM ids (`team-finances-budget-form` / `-budget-scouting` / `-budget-coaching` / `-budget-facilities` / `-ticket-price` / `-budget-save` / `-disabled-notice` / `-empty-notice` / `-table` / `-salaries-table` / `-metrics`) are **PRESERVED unchanged**.

**Scope-out (verbatim).** **tournaments/playoffs untouched, no injury-type taxonomy, no in-sim injuries, no frequency-from-health, no Stat-driven probability, FIN-05 deferred, NO Score Calibration re-baseline.**

**Locked names (quick index).** Pure module `matches/injury.py` (Django-free, import allowlist `dataclasses`/`typing`/`random`/`collections`); constants `BASE_INJURY_RATE = 0.04` / `AGE_FACTOR_PIVOT = 27` / `AGE_FACTOR_PER_YEAR = 0.04` / `AGE_FACTOR_MIN = 0.5` / `AGE_FACTOR_MAX = 2.5` / `DURATION_BASE_GAMES = 3.0` / `DURATION_MIN_GAMES = 1` / `DURATION_MAX_GAMES = 12` / `PLAY_HURT_STAT_PENALTY = 12`; fns `age_factor(age)` / `injury_probability(age)` / `roll_injury(age, rng)` / `draw_duration(health_effect, age, rng)` / `play_hurt_penalty()`; pinned RNG order `roll_injury` then (if hit) `draw_duration`. `matches/finance.py::health_effect(level)` (NEW) + constant `MAX_HEALTH_EFFECT = 0.5` (NEW); `ExpenseLines.health` (NEW trailing field); CHANGED `season_expenses` / `compute_team_finance` (keyword-only `health_level`, seventh expense). Fields `teams.Player.games_unavailable` / `teams.Team.budget_health` (default 34) / `teams.Team.injury_policy` (`auto_sub`/`play_hurt`, `INJURY_POLICY_CHOICES`) / `matches.TeamSeasonFinance.health_cost`; migrations `teams/migrations/0014_player_team_health_injury.py` + `matches/migrations/0052_teamseasonfinance_health_cost.py`. Helpers `matches.league_views.resolve_injuries_for_fixture(season, team_red, team_blue) -> dict` / `restore_after_fixture(token) -> None` (NEW); CHANGED call sites `matches.tasks.play_season_task` / `matches.league_views.play_week` / `play_week_live` (RR branch) / `_develop_league_for_new_season` (reset) / `_ensure_team_finances` (`health_level=` + `"health_cost"` default). UI DOM ids `team-finances-budget-health` / `team-finances-injury-policy` / `team-finances-availability` / `team-finances-availability-empty`. Decision: [ADR-0028](../../docs/adr/0028-health-budget-injury-availability.md). Seam contract: [`.claude/worktrees/fin-04-health-injury-seam-contract.md`](../../.claude/worktrees/fin-04-health-injury-seam-contract.md).
...
## GEN-01 persistence-fidelity tiers off one seed

GEN-01 generates a round at one of three cumulative **Persistence fidelity** tiers (CONTEXT.md) off the **same RNG seed**, choosing the cheapest tier sufficient for the surface that asked for it. The tiers are `scores` ⊂ `combat` ⊂ `full`: `scores` persists `GameRound` + `PlayerRoundState` only (the final scoreboard); `combat` adds the who-hit-who combat `GameEvent` rows (tag / missile / `locking` / `missiled` / resupply / down / elimination) + `highlights_json`; `full` adds movement `GameEvent`s + per-Advance `metadata["route"]` + `cell_occupancy_json`. See [ADR-0029](../../docs/adr/0029-persistence-fidelity-tiers-and-faithful-lazy-upgrade.md), the CONTEXT.md **Persistence fidelity** term, and the locked seam contract at [`.claude/worktrees/gen-01-seam-contract.md`](../../.claude/worktrees/gen-01-seam-contract.md).

**Persist tiers, NOT compute tiers.** The tick loop ALWAYS runs in full — movement gates combat (LOS/positioning, MOVE-01..04), so there is no cheaper "scores-only" compute path that would still match the full-fidelity scoreboard. The tiers differ **only** in what `flush_to_db` writes (and, at `scores`, in skipping event-buffer collection). Same seed ⇒ byte-identical game at every tier. A statistical non-spatial "scores estimator" (one that would *not* reproduce full-fidelity scores) is explicitly out of scope. **No Score Calibration re-baseline** — GEN-01 changes no simulation mechanic, consumes no new RNG in the tick loop, and the snapshot re-runs the *same* loop; the SIM-07/08 internal-determinism contract holds and the calibration targets are untouched.

**The two new `GameRound` fields** (see the **`GameRound`** model paragraph above for the field declarations + migration `0055`): `fidelity` (`max_length=6`, `default="full"`) records the persisted tier; `roster_snapshot_json` freezes the boosted per-player sim-stat inputs. The 13 snapshot stat keys are exactly `entrypoints._SIMULATION_STATS` (`accuracy, survival, player_awareness, game_awareness, decision_making, stamina, special_usage, resupply_efficiency, resupply_synergy, teamwork, communication, resource_awareness, speed`) — each a real `PlayerState` attribute. The snapshot is **built from the in-memory `PlayerState` lists** returned by `_simulate_round` (NOT the ORM `Player`), so it already holds the post-`stat_for_simulation` boosted values; the module-level helper `persistence.build_roster_snapshot(red_players, blue_players) -> dict` builds it (skipping falsy `player_id`, mirroring the `flush_to_db` player-skip rule) and `flush_to_db` calls it on **every** tier — every persisted round carries a snapshot. The migration is **two `AddField`s, no `RunPython`, no backfill** ([ADR-0004](../../docs/adr/0004-simulation-data-is-disposable.md)).

**The `fidelity` selector** is a **keyword-only** kwarg `fidelity: str = "scores"` threaded (default `"scores"` everywhere) through `BatchSimulator.simulate_match`, `simulate_single_round_detailed`, `simulate_scheduled_round`, `save_games`, `_simulate_and_flush_round`, `_flush_to_db` (all `entrypoints.py`), `persistence.flush_to_db`, and `tournament_engine.play_next_node` / `play_specific_node`. **The LG-01i live call sites override to `"full"`** (CONTEXT.md "play-now-and-watch" — you must see the game you are watching, replayed from the persisted rows in the same request): in `league_views.py::play_week_live` the RR branch passes `simulate_scheduled_round(…, fidelity="full")` and the live-playoff branch calls `play_specific_node(node, fidelity="full")`. **Every other caller inherits `scores`** — `tasks.py::play_season_task`, `play_week` non-live, `play_next_node`-driven background drains, the sandbox `simulate_match` / `simulate_single_round_detailed`, and `save_games`.

**`flush_to_db` gating + the `scores` no-buffer choice.** `flush_to_db`'s write-blocks are factored into module-level `persistence._write_*` helpers shared by BOTH the fresh flush and the lazy-upgrade backfill, gated on `FIDELITY_RANK[fidelity]`: the `GameRound` create (always — now also setting `fidelity=fidelity` + `roster_snapshot_json=<snapshot>` on the create, before winner calc) and `_write_player_states(...)` (always, returns the `players_by_id` ORM map); `_write_combat_events(...)` + `_write_highlights(...)` gated `rank >= combat` (≥ 2); `_write_movement_events(...)` + `_write_cell_occupancy(...)` gated `rank == full` (== 3, AND the existing `movement_ctx is not None` guard for occupancy — a `full` map-less round still leaves `cell_occupancy_json` null, matching today). The HX-02 `invalidate_role_benchmarks()` tail stays at the end, ALWAYS (cheap cache bump on every tier). At `scores` there is **no event buffer** at all: `_simulate_and_flush_round` (and `save_games`) choose `event_log = [] if FIDELITY_RANK[fidelity] >= FIDELITY_RANK["combat"] else None`, and `_simulate_round(event_log=None)` builds the null-object `EventLog(persist=False)` so no rows are collected; `build_highlights` is not called at `scores` (`highlights_json` stays `None` until an upgrade).

**`BatchSimulator.ensure_fidelity(game_round, target_fidelity)` — faithful lazy upgrade** (`@transaction.atomic`). Idempotent / defensive no-op (returns the row unchanged, writes nothing) iff `FIDELITY_RANK[gr.fidelity] >= FIDELITY_RANK[target]` **OR** `roster_snapshot_json is None` (the impossible legacy case — `fidelity<full` with a null snapshot — never crashes; renders scores-only). Otherwise it rebuilds the `_PlayerData` rosters from the snapshot (`list[tuple[role, _PlayerData]]`, where `_PlayerData.stat_for_simulation(stat, role)` returns `stats[stat]` verbatim — re-baking IDENTICAL `PlayerState`s), `load_map_context(gr.arena_map)`, `random.seed(gr.rng_seed)`, re-runs `_simulate_round(..., event_log=[], movement_ctx=...)`, then **backfills** the missing higher-tier rows onto the EXISTING row via the shared `_write_*` helpers (it builds `players_by_id` itself with a read-only query — NOT `_write_player_states`, which would duplicate scoreboard rows) and bumps `fidelity = target`. It **MUST NOT** rewrite the scoreboard (`GameRound` scalar columns) or `PlayerRoundState` — those are byte-identical from the original `scores` flush, so re-writing them is forbidden, not just unnecessary. Because the re-sim reads the **snapshot** (not live `Team.active_roster`), the upgrade is exact regardless of LG-04 development or any later roster edit; first click pays the re-sim + backfill once, later clicks are no-ops. *Alternatives rejected:* verify-then-degrade (would lose every prior season round's event log after one rollover) and re-sim-authoritative (would retroactively shift completed-season Standings) — see ADR-0029.

**View triggers** (`matches/views.py`, inserted right after the `get_object_or_404`, before context build): `game_round_events` and `movement_heatmap` call `ensure_fidelity(game_round, "full")` (the events log + charts + round-playback overlay and the heatmap both need movement + occupancy); `missile_log` calls `ensure_fidelity(game_round, "combat")`; `game_round_detail` is **UNCHANGED** — it stays `scores`, since the scoreboard / MVP already live from `PlayerRoundState` and need no upgrade.

**Residual map-edit caveat.** The snapshot freezes *roster* inputs only — a round's map context (per-cell sight-line / zone data, far too large to store per round) is still re-derived from its persisted `arena_map` FK at upgrade time, so re-painting an arena's zones/sight-lines *after* a round was played can still drift its replay. This stays under the pre-existing `rng_seed` "map config unchanged" caveat (SIM-07) — rare (editing happens in the map editor, not during play) and snapshotting full map context is deferred.
...
there is no legitimate cross-Conference champion until Worlds (disjoint
schedules make a cross-Conference record comparison meaningless; deciding it on the field is
the whole point of Worlds). CONF-04 shipped Worlds, but **this branch is unaffected**: a
Season whose *final* phase is a round robin has no non-`worlds` `tournament` phase, so
`_ensure_worlds_phase` creates nothing and such a Season still completes championless —
permanently. Concretely: a `>= 2`-Conference RR-final Season still flips
...
## CONF-03 worlds qualification + the last-chance qualifier

The third slice of the **Conference** epic, and the one that settles **who goes to Worlds**. In a
Season with **two or more** Conferences, the **final** (highest-ordinal) `tournament` `SeasonPhase`
now decides qualification: each Conference sends **1, 2 or 3 Worlds qualifiers** as a function of its
**size**, and the third slot — where it exists — is decided by a *second* per-Conference bracket, the
**Last-chance qualifier**, created eagerly and **unseeded** at phase activation and seeded only once
that Conference's Regional playoff has crowned a champion. The unioned field is ordered tier-first,
then by regular-season **rate**, and is **derived on demand and never persisted**. See
[ADR-0036](../../docs/adr/0036-worlds-qualification-size-tiered-with-last-chance-bracket.md), the
CONTEXT.md **Worlds** / **Worlds qualifier** / **Last-chance qualifier** terms, and the seam contract
[`.claude/worktrees/conf-03-seam-contract.md`](../../.claude/worktrees/conf-03-seam-contract.md).
**`Season.champion_team` still stays NULL** for a `>= 2`-Conference Season — this slice decides who
plays for it, CONF-04 crowns it — and there is **no Score Calibration re-baseline** (no simulation
mechanic changes; the only shifts are one new discriminator column, one extra `Tournament` row per
large Conference, and a pure derivation module).

**Qualifier count is a function of Conference SIZE.** Size is `len(conference.starting_team_ids_json
or [])` — the **activation snapshot** written by `Season.start_season()`. It is **not** read from the
Regional playoff's participant count and it is **not** affected by `SeasonPhase.tournament_cut`, so a
top-N cut can shrink a region's bracket without changing how many Teams that region sends to Worlds.

| Conference size (activation snapshot) | Qualifiers | Slot provenances, in tier order |
| --- | --- | --- |
| 0 or 1 | 0 | degenerate; CONF-01/CONF-05 forbid composing one |
...
The three provenances are fixed: **tier 1** is that Conference's Regional playoff
`Tournament.champion`; **tier 2** is the Conference's best **Standings** finisher not already
qualified (rank 1, or rank 2 when rank 1 also won the bracket, so the same Team never occupies two
slots); **tier 3** is the `champion` of that Conference's Last-chance qualifier.

**The no-Regional-playoff fallback, and its TWO causes.** A Conference of the final tournament phase
can legitimately end up with **no Regional playoff `Tournament` row** in two different ways, and both
land on the same fallback: (1) the Conference has **2-3 Teams**, fewer than `MIN_BRACKET_PARTICIPANTS`
(= 4), so its seed order is too short and `_build_tournament_for_phase`'s regional guard returns
`None`; or (2) the phase's **`tournament_cut` is set below 4** (a cut of 2, say), truncating even a
**large** Conference's seed order below the floor — the same guard, applied *after* the
`order = order[: phase.tournament_cut]` slice. In both cases the Conference still sends **exactly
one** qualifier: its Standings **rank 1**, recorded with `provenance = PROVENANCE_REGULAR_SEASON`
(it is a regular-season placing, not a bracket result) but seeded at `tier =
QUALIFIER_TIER_CHAMPION`, so it sorts among the champions. **No Conference is ever unrepresented at
Worlds.**

> **The fallback is NOT `regional is None` — this is the slice's sharpest trap.** A bare
> `regional is None` is *also* true when the tournament phase **has not been built at all**: the
> regular season is still running and `phase.regional_tournaments` is empty for *every* Conference.
> Firing the fallback there would emit a complete, plausible-looking Worlds field
> **mid-regular-season, before a single playoff Match had been played**, and the Worlds panel would
> render it. The fallback therefore fires **only** when this Conference has no regional row **AND**
> `phase.regional_tournaments.exists()` is `True`, which is why tier 1 is a **three-branch**
> decision — `regional is not None`; else not built (`return []`); else the genuine fallback.
> `exists()` is the right predicate rather than re-deriving size against `MIN_BRACKET_PARTICIPANTS`,
> because one read covers **both** causes above, including the `tournament_cut` case where size alone
> would wrongly predict a bracket. The bug bites hardest on a `>= 2`-Conference Season whose
> Conferences are **all 8 Teams or fewer**: such a Season never reaches a tier-3 slot, so nothing
> else in the derivation would ever return `[]`.

**Cross-region seeding — tier first, then RATE.** The unioned field sorts by `tier` ASC (1 champion,
2 regular-season, 3 last-chance), then within a tier by `league_points / matches_played` DESC, then
`round_wins / matches_played` DESC, then `total_score / matches_played` DESC, then `team_id` ASC —
every input taken off that Team's **Conference-scoped** `StandingsRow`. **Rate rather than raw
totals** because Conferences differ in size and therefore play different numbers of games: a 12-team
Conference's 11-game points total is not comparable to a 5-team Conference's 4-game total. Locked
degenerate rule: when `matches_played <= 0` **all three rates are `0.0`** — no division is attempted,
no `None`, no sentinel, no exception — so such a Team sorts below every positive-rate Team in its
tier and ties with other zero-rate Teams, falling through to the `team_id` tiebreak.

**CHANGED `Tournament` — ONE new discriminator column (`matches/models.py`, after the CONF-02
`conference` FK, before the `final_series_length` block).** `qualifier_stage =
models.CharField(max_length=16, choices=QUALIFIER_STAGE_CHOICES, blank=True, default="")`, with the
choices tuple declared as a class attribute immediately above it (the house precedent of
`SeasonPhase.TOURNAMENT_MODE_CHOICES` / `Tournament.TEAM_ASSEMBLY_CHOICES` /
`Tournament.ROLE_ASSIGNMENT_CHOICES`): `("", "Not a qualifier bracket")`, `("regional_playoff",
"Regional playoff")`, `("last_chance", "Last-chance qualifier")`. `max_length=16` is **exact** —
`"regional_playoff"` is 16 characters. `Tournament` still has **no `class Meta`** and none was added:
no index, no `unique_together` on `(season_phase, conference, qualifier_stage)`; the build path is
guarded by idempotence checks, not by the database.

> **LOCKED read rule — no backfill.** Migration `0059` has **no `RunPython`**, so an existing CONF-02
> regional row keeps `qualifier_stage == ""` forever. Every read in production code, tests and
> templates therefore tests **only** `tournament.qualifier_stage == "last_chance"`. Everything else
> — including that un-backfilled `""` — is a **Regional playoff** when `conference_id` is set, and
> not a qualifier bracket at all when it is NULL. **Forbidden:** `== "regional_playoff"` as a
> *positive* test for "is this a regional playoff", `!= ""` as a test for "is this a qualifier
> bracket", any `.exclude(qualifier_stage="")`, and any migration that would make those safe. A
> regional playoff is `conference_id is not None and qualifier_stage != "last_chance"`.
...
**Migration `0059_tournament_qualifier_stage.py`** (dep `0058_tournament_regional_linkage`): exactly
**one `AddField`**, no `RunPython`, no backfill, no data migration, no follow-up migration — per
[ADR-0004](../../docs/adr/0004-simulation-data-is-disposable.md)'s disposable-data posture, and
because the read rule above makes the un-backfilled `""` classify correctly on its own.

**NEW pure module `matches/worlds.py`.** Follows the `matches/standings.py` / `matches/bracket.py`
precedent — **no Django imports, no ORM, plain values in and out, unit-testable with zero DB**, with
a frozen import allowlist of `dataclasses` + `typing` pinned by a `TestNoDjangoImportsLeaked`
subprocess class. Exports the tier constants `QUALIFIER_TIER_CHAMPION` (1) /
`QUALIFIER_TIER_REGULAR_SEASON` (2) / `QUALIFIER_TIER_LAST_CHANCE` (3); the provenance constants
`PROVENANCE_CHAMPION` (`"conference_champion"`) / `PROVENANCE_REGULAR_SEASON` (`"regular_season"`) /
`PROVENANCE_LAST_CHANCE` (`"last_chance"`) with their `PROVENANCE_LABELS` map; `LAST_CHANCE_FIELD_SIZE
= 4`; the frozen dataclass **`WorldsQualifier`** (`team_id`, `team_name`, `conference_id`,
`conference_name`, `tier`, `provenance`, `matches_played`, `league_points`, `round_wins`,
`total_score`, `seed = 0`, plus a `provenance_label` property returning `""` for an unknown value);
and four pure functions — `qualifier_count_for_size(team_count) -> int` (the size table above; a
negative input returns `0`), `first_unqualified(ranked_team_ids, qualified_team_ids) -> int | None`
(the tier-2 rule *and* the fallback's rank-1 rule in one function),
`last_chance_field(ranked_team_ids, qualified_team_ids) -> list` (**up to** `LAST_CHANCE_FIELD_SIZE`
ids in rank order — never pads, never raises), and `order_worlds_qualifiers(qualifiers) -> list`
(the sort above, returning a **new** list with `seed` stamped 1..M via `dataclasses.replace`, exactly
as `compute_standings` / `rerank_round_robin` stamp `rank`). The rate helper `_rate(numerator,
matches_played)` is module-private. The four regular-season integers on `WorldsQualifier` are the
rate *inputs*, taken verbatim off the Conference-scoped `StandingsRow`; **no rate is stored**.

> **Naming hazards — three, and all three have real bite.**
> - `Tournament.season_phase` (the CONF-02 **forward FK**, reverse-accessed as
>   `phase.regional_tournaments`) vs `Tournament.season_phases` (the **pre-existing reverse manager**
>   of `SeasonPhase.tournament`). One character apart, **opposite directions**. CONF-03 reads the
>   forward one everywhere.
> - **`PROVENANCE_LAST_CHANCE` and `Tournament.qualifier_stage`'s `"last_chance"` share a string
>   value while being different axes** — one is a *qualifier's provenance*, the other a *bracket's
>   stage*. Neither may be used in place of the other, and the coincidence must never become an
>   import.
> - **`matches/worlds.py` must never import from `matches/models.py`.** Tier and provenance are
>   deliberately **separate axes** for the same reason: the 2-3-Team fallback is exactly the case
>   where they disagree (`QUALIFIER_TIER_CHAMPION` with `PROVENANCE_REGULAR_SEASON`), so never derive
>   one from the other.

**NEW `Season._final_tournament_phase(self) -> SeasonPhase | None`** (after `tournaments_for_phase`,
private) — walks `ordered_phases()` and returns the **last** `phase_type == "tournament"` phase, or
`None`. A non-persisted implicit fallback phase (`pk is None`) is never returned. **Only the final
tournament phase qualifies for Worlds and only it gets Last-chance brackets**; a mid-season
`tournament` phase still builds its regional brackets exactly as CONF-02 does — no `last_chance` row,
no qualification read, no new DOM id.

**NEW `Season._build_last_chance_tournament(self, phase, conference) -> Tournament | None`** (after
`_build_tournament_for_phase`, private; `conference` is **required**) — returns `None` when
`qualifier_count_for_size(size) < 3` (so a Conference of 8 or fewer gets **no row at all**), returns
`None` when a `last_chance` row for that Conference already exists (idempotence), and otherwise
creates the row and **returns it immediately** — copying every per-format field off the phase exactly
as the regional builder does, plus `season_phase=phase`, `conference=conference`,
`qualifier_stage="last_chance"`, named `f"{season.name} — {conference.name} Last Chance Qualifier"`
(em-dash U+2014, matching `Conference.__str__` and CONF-02's regional naming; the label is the
three-word proper-noun bracket name, distinct from CONTEXT.md's prose term *Last-chance qualifier*).
`_build_tournament_for_phase` itself changes by exactly **one kwarg** — its conditional
`season_phase` / `conference` block gains `"qualifier_stage": "regional_playoff"` — and takes **no
new parameter**; a regional build is always a Regional playoff.

> **UNSEEDED is load-bearing, not a bug — do not "fix" it.** The Last-chance row exists with
> `state="setup"`, **zero** `TournamentParticipant` rows, **zero** `BracketNode` rows and
> `lock_and_build()` not yet called. Two existing behaviours make that safe with **no engine change**:
> `_tournament_phase_complete` requires `all(t.state == "completed")` and `"setup" != "completed"`,
> so **the phase already refuses to advance** while a Last-chance bracket is unseeded (no new gate);
> and `Tournament.find_next_playable_node()` delegates to `bracket.find_next_node` over `self.nodes`,
> which is empty, so it returns `None` — `play_next_node` returns `None` and `play_next_bracket_round`
> returns `0`, and **both drain loops already skip an unseeded bracket harmlessly**. Do not add a
> `state="pending_seed"`, do not create placeholder participants, do not call `lock_and_build()`
> early, and do not filter unseeded rows out of `tournaments_for_phase`. The row is created
> **eagerly** precisely because both drain loops resolve `tournaments_for_phase(phase)` **once and
> cache the list**: a row created lazily mid-drain would be invisible to the loop that must play it,
> and `_tournament_phase_complete` would report the phase complete before the bracket existed.

**NEW public `Season.seed_pending_last_chance_brackets(self, phase) -> int`** (after
`_build_last_chance_tournament`; `@transaction.atomic`) — **the seeding seam**, returning the number
of brackets it seeded on that call and **idempotent** (`0` before a Regional playoff has a champion,
a positive count on the call that seeds, `0` forever after). It filters
`phase.regional_tournaments.filter(qualifier_stage="last_chance", state="setup")` — the *one*
permitted positive test of the discriminator — and for each candidate in `conference__ordinal, id`
order finds that Conference's regional sibling (`.exclude(qualifier_stage="last_chance").first()`),
skips it when the sibling is missing or has no `champion_id`, and otherwise computes the Conference's
Standings order via `_final_standings_for_phase(self._preceding_phase(phase), conference=...)`, builds
`qualified = {champion} | {first_unqualified(...)}`, takes `last_chance_field(ranked, qualified)`,
creates one `TournamentParticipant` per entry with `seed = position + 1`, and calls
`lock_and_build()`. A field shorter than `MIN_BRACKET_PARTICIPANTS` **deletes the row** rather than
deadlocking the phase on a bracket that can never be built — unreachable for a Conference of 9+
(at least `9 - 2 = 7` Teams are always unqualified) and present only so admin-mangled data cannot
brick a Season. Deleting a candidate mid-drain is safe: a deleted instance left in a cached
`tournaments_for_phase` list re-queries `nodes` to empty, so `stage_progress([])` is `(0, 0)` and
`find_next_playable_node()` is `None`.

**Four hook sites, seed-then-continue semantics — the engine is NOT changed.** `play_next_node`,
`play_specific_node`, `play_next_bracket_round`, `find_next_playable_node`, `find_next_node`,
`stage_progress` and `lock_and_build` are reused **verbatim**; every change is in the callers.
(1) `tasks.play_playoffs_task` — only its no-progress exit changes: when a full pass plays nothing it
calls `seed_pending_last_chance_brackets(phase)` and, if that returns `> 0`, **`continue`s** the
`while True:` loop (so the PLAY-01 between-stage cancel check runs again) instead of breaking.
(2) `tasks.play_season_task`'s tournament tail — the retry lives **inside** the local
`_drain_one_stage()`, which re-runs `sum(play_next_bracket_round(t) for t in tournaments)` once after
seeding when the first pass clinched `0`; because it fires only on a zero-progress stage, **one budget
unit is still exactly one stage**, and both loop bodies stay untouched. (3)
`league_views.play_single_round` — seeds **unconditionally** after the click (idempotent, `0` when
nothing is ready) and then plays one node if seeding just made something playable. This is what stops
"Play Single Round" becoming a **dead click**: the click that resolves a Regional playoff's final node
leaves that bracket `completed` and its Last-chance sibling `setup`, which the dashboard's
playoff-controls helper reads as *neither* `active` nor `completed` and so hides the control; seeding
before the redirect flips the sibling to `active`. (4)
`Season.activate_pending_tournament_phase` — the `>= 2`-Conference branch calls
`seed_pending_last_chance_brackets(phase)` **inside and before** the CONF-02 idempotence guard's
`return`, turning re-activation into a **recovery hook** (it runs after every scheduled Round, so a
Season can never be stranded with a permanently unseeded bracket); after the regional build loop it
runs a second loop of `_build_last_chance_tournament(phase, conference)`, gated on
`_final_tournament_phase()` being this phase. **In all four the cached `tournaments_for_phase` list is
deliberately NOT re-resolved**, no in-loop `t.state` check is added and no cached instance is
`refresh_from_db()`-ed — the eager row is already in the list and `tournament.nodes` is re-queried on
every call, so the very same cached instance that was node-less on iteration 1 plays correctly on
iteration 4.

> **The Season dashboard is deliberately NOT changed.** Its helper still sets
> `playoff_phase_active` from `any(t.state == "active")` and `playoff_completed` from
> `all(t.state == "completed")`. The one transient it cannot describe — all regionals `completed`
> while a Last-chance bracket is still `setup` — is closed by the four hook sites on every path that
> can leave a user looking at the dashboard. **Do not "fix" it by treating `setup` as active**: that
> would change 0/1-Conference behaviour and break the byte-identity invariant.

**NEW public `Season.worlds_qualifiers(self) -> list[WorldsQualifier]`** (after
`seed_pending_last_chance_brackets`) — the derivation seam, computed on demand with **nothing
persisted**. Returns `[]` immediately for a `< 2`-Conference Season (a 0/1-Conference Season has no
Worlds — its season-ending playoff crowns the Season champion directly) or when there is no
tournament phase. Otherwise it resolves `prior = self._preceding_phase(phase)` and — **once, before
the per-Conference loop, never per Conference** — `phase_built = phase.regional_tournaments.exists()`,
then walks Conferences in ordinal order gathering tier 1 (the three-branch decision above), tier 2
when `count >= 2` and tier 3 when `count >= 3`, resolving Team names through a single tolerant
`Team.objects.filter(id__in=...).in_bulk()` and the four regular-season integers off that
Conference's `StandingsRow` (defaulting to `0`), and finally returns `order_worlds_qualifiers(...)`.

> **LOCKED readiness rule — "empty list = not ready", never a partial list.** `worlds_qualifiers()`
> returns `[]` the moment **any** required bracket of the final tournament phase is missing its
> `champion`, or any Conference that should have a Last-chance row has none. It never returns a
> partially-filled field. Reason: CONF-04 will build the Worlds bracket straight off this list, and a
> partial list is **indistinguishable from a complete one at the call site** — a 5-of-7 field would
> silently build a wrong-size bracket. It also gives the UI exactly one branch ("render the panel, or
> don't") instead of two. A legacy Season predating 0059 has no Last-chance rows and so returns `[]`
> forever; accepted under ADR-0004.

**Playoffs screen (`matches/league_screens/playoffs.py` + `templates/leagues/playoffs.html`).** The
`brackets` entry shape gains two keys — **`stage`** (`""` / `"regional_playoff"` / `"last_chance"`)
and **`stage_label`** (`"Last Chance Qualifier"` for a Last-chance entry, `""` for everything else, so
**no new element renders** on a CONF-02 Season) — and changes one: `pending` becomes
`tournament.state == "setup"` instead of the literal `False`, which is byte-identical for every
pre-CONF-03 row because `_build_tournament_for_phase` always calls `lock_and_build()` and flips
`state` to `"active"` before the row is ever rendered. **The CONF-02 `key` discriminator gains a
`-lc` suffix on the Last-chance entry only** (`f"{phase.ordinal}-{conference.ordinal}-lc"`), so every
regional `key` (`"<ord>-<conf ord>"`) and every 0/1-Conference `key` (`str(phase.ordinal)`) — and
therefore every DOM id built from them — stays **byte-identical**. The template gains exactly three
things: a stage badge `id="league-playoffs-stage-{{ bracket.key }}"` rendered only when
`stage_label` is set; a Last-chance branch in the existing `{% if bracket.pending %}` alert (the
`{% else %}` keeps the old text **verbatim**, so every existing rendered stub is unchanged); and the
**Worlds panel**, one new context key `worlds_qualifiers` (the raw
`view_season.worlds_qualifiers()`, or `[]`) rendered under `{% if worlds_qualifiers %}` — which *is*
the readiness test, so there is deliberately **no `worlds_ready` boolean** and the list is **not**
re-mapped into dicts. Locked ids: `league-playoffs-worlds`, `league-playoffs-worlds-table`,
`league-playoffs-worlds-row-<seed>`, `league-playoffs-worlds-team-<seed>`,
`league-playoffs-worlds-conference-<seed>`, `league-playoffs-worlds-provenance-<seed>`. The panel is
**read-only** and, for a 0/1-Conference Season, **absent entirely** — no section, no heading, no
empty-state text, no new DOM id.

**Explicitly NOT changed.** The bracket engine and `Tournament.lock_and_build` (whose `>= 4`
participant check is exactly the field size we seed); `Tournament.find_next_playable_node` (no
`state` guard added); `Season.tournaments_for_phase` (same signature, body and
`("conference__ordinal", "id")` ordering — it simply returns **up to two rows per Conference** now,
and because activation builds all regional rows in loop 1 and all Last-chance rows in loop 2, every
regional `id` is lower than every Last-chance `id`, so **within a Conference the Regional playoff
always sorts before its Last-chance sibling**); `_tournament_phase_complete` / `_phase_complete`
(already the right gate); `_stamp_champion_for_final_phase` (its `regional` list now simply includes
the Last-chance rows, so `any(t.champion_id is None ...)` blocks completion until they too have
crowned a winner, and it still never assigns `champion_team`); `_final_standings_for_phase` /
`_seed_order_for_phase` / `start_season` / `ordered_conferences`; `compute_standings`; `SeasonPhase`
and `Conference` (no new field, no `Meta` change — size is read off the existing
`starting_team_ids_json`); `Match.conference` **stamping** (CONF-03 only reads it);
`league_views.play_week` / `play_playoffs` / `season_standings` and the dashboard helper
(`play_single_round` is the only function in that file this slice edits);
`league_screens/team_history.py` (CONF-02's two-term `Q` chain already picks up any Tournament with
`season_phase__isnull=False`, Last-chance brackets included, and CONF-03 asserts nothing about it in
either direction); `matches/tournament_views.py` (an unseeded Last-chance row appearing in the
`/tournaments/` list in `setup` state is **accepted**, per ADR-0036's Consequences); and the
simulator.

**Locked names (quick index).**
- **`matches/worlds.py`** (new, Django-free) — `WorldsQualifier` (+ `provenance_label`),
  `QUALIFIER_TIER_CHAMPION` / `_REGULAR_SEASON` / `_LAST_CHANCE`, `PROVENANCE_CHAMPION` /
  `_REGULAR_SEASON` / `_LAST_CHANCE`, `PROVENANCE_LABELS`, `LAST_CHANCE_FIELD_SIZE`,
  `qualifier_count_for_size`, `first_unqualified`, `last_chance_field`, `order_worlds_qualifiers`
  (private `_rate`). **Never imports `matches/models.py`.**
- **`Tournament`** NEW `qualifier_stage` (`CharField(max_length=16, blank=True, default="")` +
  `QUALIFIER_STAGE_CHOICES`); no `Meta`. Read rule: test **only** `== "last_chance"`.
- **`Season`** NEW `_final_tournament_phase()`, `_build_last_chance_tournament(phase, conference)`,
  **public** `seed_pending_last_chance_brackets(phase) -> int` and `worlds_qualifiers() ->
  list[WorldsQualifier]`; CHANGED `_build_tournament_for_phase` (one kwarg) and
  `activate_pending_tournament_phase` (branch body only).
- **Hook sites** `tasks.play_playoffs_task` (seed-then-`continue`), `tasks.play_season_task`'s
  `_drain_one_stage()` (seed-then-retry inside the helper), `league_views.play_single_round`
  (unconditional seed before the redirect), `Season.activate_pending_tournament_phase` (build +
  recovery).
- **Screens** `league_screens/playoffs.py` (new `stage` / `stage_label` bracket keys, `-lc` `key`
  suffix, `pending = state == "setup"`, new `worlds_qualifiers` context key) +
  `templates/leagues/playoffs.html` (`league-playoffs-stage-{key}`, the Last-chance pending message,
  the `league-playoffs-worlds*` panel).
- **Migration** `matches/migrations/0059_tournament_qualifier_stage.py` (dep `0058`, one `AddField`,
  no `RunPython` / backfill).
- **Tests** `matches/tests/test_worlds_qualification.py` (the pure module + `worlds_qualifiers()`,
  including the **premature-field regression guard**: an all-`5-8`-Team `>= 2`-Conference Season with
  the tournament phase unbuilt must return `[]`) + `matches/tests/test_last_chance_qualifier.py` (the
  build/seed/gate cycle, idempotence, the `tournaments_for_phase` order, the dead-click pin and both
  byte-identity pins) + appended classes in `matches/tests/test_regional_playoffs_drain.py` and
  `matches/tests/test_league_playoffs.py`. House rule unchanged: brackets are drained by **driving
  the engine or stamping node winners**, never by running the simulator, and never by patching the
  seam under test.
- **ADR / CONTEXT** —
  [ADR-0036](../../docs/adr/0036-worlds-qualification-size-tiered-with-last-chance-bracket.md) + the
  CONTEXT.md **Worlds** / **Worlds qualifier** / **Last-chance qualifier** terms. Invariants: a
  **0/1-Conference Season is byte-identical**; a **Conference of 8 or fewer Teams is byte-identical
  to CONF-02** in rows and DOM ids (no Last-chance row at all); **every CONF-02 DOM id is unchanged**;
  **`Season.champion_team` stays NULL**; the phase never advances early; the Worlds field is
  **all-or-`[]`**; no Conference is unrepresented; and **no Score Calibration re-baseline**.
  **DEFERRED to CONF-04 and later:** the cross-Conference Worlds `Tournament` row and phase, any
  bye / play-in handling for a non-power-of-two field, any placement / elimination-depth ranking API
  on `Tournament`, per-Conference map pools (CONF-06), and a persisted qualifier table.

## CONF-04 the worlds tournament phase

The fourth slice of the **Conference** epic, and the one that closes it competitively: a Season with
**two or more** Conferences grows a **derived, never-authored fifth-mode `SeasonPhase`** — Worlds —
carrying a **single Season-wide bracket** built straight off CONF-03's
`Season.worlds_qualifiers()`, drained through the **untouched** bracket engine, and finally
**crowning the Season champion** that CONF-01, CONF-02 and CONF-03 each deliberately left NULL. See
[ADR-0037](../../docs/adr/0037-worlds-is-a-derived-season-phase.md), the CONTEXT.md **Worlds** /
**Worlds phase** / **Season phase** terms, and the seam contract
[`.claude/worktrees/conf-04-seam-contract.md`](../../.claude/worktrees/conf-04-seam-contract.md).
**A 0/1-Conference Season is byte-identical** in rows, reads, champion stamping and rendered DOM ids
— it grows no Worlds phase at all — and there is **no Score Calibration re-baseline** (no simulation
mechanic changes; the only shifts are one choices-only `AlterField`, one derived `SeasonPhase` row
per `>= 2`-Conference Season, and one `Tournament` row per such Season).

**Worlds is a phase, not a third bracket on the regional phase.** The rejected alternative — hanging
a third `Tournament` off the *existing* final tournament phase alongside the regional and
Last-chance rows, discriminated by a new `qualifier_stage="worlds"` — would have landed the row
inside `phase.regional_tournaments`, which `tournaments_for_phase` orders by `conference__ordinal`;
the Worlds row's `conference` is NULL, and NULL sorts **first on SQLite and last on PostgreSQL**,
making the bracket's drain and render position **backend-dependent** in a project whose two
supported backends are exactly those
([ADR-0025](../../docs/adr/0025-postgresql-canonical-sqlite-dev-only.md)). It would also have forced
a rewrite of `_stamp_champion_for_final_phase`, whose `if regional: … return` is the very line
encoding "a Conference champion is not a Season champion". A fourth `SeasonPhase.phase_type` value
was rejected as too broad — every site keyed on `phase_type == "tournament"` (phase completion,
phase activation, both entry points, both drain loops, the Playoffs screen and the dashboard helper)
would need a parallel branch, and each branch is a place for the two kinds of tournament phase to
drift apart. `tournament_mode` already **is** the "what flavour of tournament phase is this?" axis.

**NEW `SeasonPhase.TOURNAMENT_MODE_CHOICES` value — exactly one constant, appended fifth.**
`("worlds", "Worlds")`, after `("random_draw", "Mid-season: drawn pool -> RR->DE")`; the four
existing pairs, their order and their label strings are **byte-unchanged**. The stored value is
6 characters, well inside the existing `max_length=16`. **No other constant is added anywhere** — no
new `Tournament.QUALIFIER_STAGE_CHOICES` value, no new `SeasonPhase.PHASE_TYPE_CHOICES` value, no
new `owner_mood.py` constant, and `MIN_BRACKET_PARTICIPANTS` (`matches/models.py:18`, value `4`) is
**not changed and not re-pointed** — it keeps guarding `_build_tournament_for_phase` and
`seed_pending_last_chance_brackets` at 4.

**The phase is DERIVED, never authored.** There is no composer wire token, no create-form control
and no admin surface for it. **NEW `Season._ensure_worlds_phase(self) -> SeasonPhase | None`**
(private, idempotent) creates **nothing** unless all three of: `len(self.ordered_conferences()) >=
2`; at least one **persisted** phase with `phase_type == "tournament"` **and** `tournament_mode !=
"worlds"` exists; and `self._worlds_phase() is None`. When only the third fails it returns the
pre-existing row; when either of the first two fails it returns `None`. Nothing in production reads
the return value — both call sites discard it — but it is returned so tests have a direct handle
(the ADR gives it no public caller). Every column of the row is passed **explicitly and
exhaustively**, so it cannot drift if a model default later changes:

| Column | Value | Note |
| --- | --- | --- |
| `ordinal` | `max(p.ordinal for p in self.phases.all()) + 1` | always the Season's highest ordinal; gate 2 guarantees the `max` never sees an empty sequence, and the value always respects `uniq_season_phase_ordinal` |
...
| `tournament_cut` | `0` | the Worlds field is never cut |
| `final_` / `semifinal_` / `quarterfinal_` / `earlier_series_length` | `1` | |
| `wb_advancers` / `lb_advancers` / `swiss_rounds` | `0` | |

**NEW `Season._worlds_phase(self) -> SeasonPhase | None`** — the **one** resolver, scanning
`ordered_phases()` for the first persisted phase with `tournament_mode == "worlds"`. A
non-persisted implicit fallback phase is never returned (it is always `round_robin`). At most one
Worlds phase can exist per Season — `_ensure_worlds_phase` is idempotent and the rollover skips it —
so the first match is the only match. `_ensure_worlds_phase`, `build_pending_worlds_bracket` and the
owner-mood classifier **all** go through it, so nobody re-implements the scan.

**Two creation hooks, and the rollover skip.** `Season.start_season()` calls
`_ensure_worlds_phase()` between its `self.save()` and its existing
`self.activate_pending_tournament_phase()` call — **Start Season is the earliest moment at which
every input is frozen** (the Conference partition, snapshotted immediately above; the phase
composition, authored at create). Creating the phase *lazily*, when qualification first resolves,
was rejected for a race: `complete_if_finished()` gates on `ordered_phases()[-1]`, so at the instant
the regional phase completes it *is* the final phase, the Season flips to `completed` with a NULL
champion, and Worlds would appear after the Season had already closed.
`activate_pending_tournament_phase()` calls it **again** as the first of its two new statements —
the same **recovery-hook** role CONF-03 gave that method for unseeded Last-chance brackets. Because
it runs after every scheduled Round and every input it reads is frozen at activation, a Season
**already active** when this slice shipped gains its Worlds phase there, with an identical row,
rather than through a data migration
([ADR-0004](../../docs/adr/0004-simulation-data-is-disposable.md)); a Season that had already
completed stays completed and championless. `league_views._run_season_rollover` gains one
`if src.tournament_mode == "worlds": continue` as the **first statement** of its phase-copy loop:
the rollover carries **no Conferences** forward, so a copied Worlds phase would land on a flat
Season whose `worlds_qualifiers()` returns `[]` forever and strand it at `active`. Because the
Worlds phase always holds the **highest** ordinal, skipping it leaves the copied ordinals contiguous
from 1 — **no renumbering is needed and none may be added**.

**CHANGED `Season._final_tournament_phase()` — narrowed to skip `worlds`-mode phases.** One
`continue` in the walk. **After this slice the method means "the final NON-Worlds tournament
phase"** — i.e. the Regional-playoff phase — which is exactly what both of its callers want
(`worlds_qualifiers()` and the Last-chance build gate inside `activate_pending_tournament_phase`).
Without that one line, appending Worlds silently redirects qualification at the **Worlds phase's own
bracket** — empty before the build, self-referential after it. `complete_if_finished` does **not**
call it (it uses `ordered_phases()[-1]`), so champion stamping is unaffected.

**NEW public `Season.build_pending_worlds_bracket(self) -> bool`** (`@transaction.atomic`) — the
idempotent build seam, returning `True` **if and only if** it built the bracket on that call and
`False` in every other case (no Worlds phase; already built; prior phase not complete;
qualification not ready). The gate is evaluated in exactly this order: `_worlds_phase()` resolves,
else `False`; `phase.tournament_id is not None` ⇒ `False`; `_preceding_phase(phase)` exists **and**
`_phase_complete(prior)`, else `False`; `worlds_qualifiers()` is non-empty **and** at least 2 long,
else `False`. It then creates the `Tournament`, one `TournamentParticipant` per qualifier, calls
`lock_and_build(minimum=2)`, wires `phase.tournament` with `save(update_fields=["tournament"])`, and
returns `True`. All of that is inside the method's own `@transaction.atomic`, so a failure at any
step leaves **no** partial Worlds bracket and the next hook-site call retries cleanly; nested inside
an already-atomic caller it is a savepoint — the CONF-03 `seed_pending_last_chance_brackets`
precedent.

> **The `>= 2` guard is defensive, not a live branch — do not delete it because it "cannot
> happen".** A legitimate `>= 2`-Conference Season always yields at least one qualifier per
> Conference (CONF-03: no Conference is ever unrepresented), so `M >= 2` by construction. The guard
> exists so admin-mangled data — a Conference snapshot emptied after activation, say — cannot reach
> `lock_and_build` with a one-participant field.

> **`seed=q.seed`, NOT `position + 1`.** `worlds_qualifiers()` returns the field already ordered and
> already stamped `1..M` by `order_worlds_qualifiers`. The enumerate index would silently work today
> (the list *is* in seed order) and break the instant anything filters or re-orders it. Use the
> stamped value.

**The Worlds `Tournament` row is deliberately FLAT — and that is what makes champion stamping work
unedited.** `name` is `f"{season.name} Worlds"` (**no em-dash**, no Conference qualifier — it mirrors
the flat `f"{season.name} Playoffs"` shape rather than CONF-02's `"… — <Conference> Playoffs"`),
`format` is the literal `"single_elimination"`, `team_assembly="preset"`, `state="setup"` until
`lock_and_build` flips it to `"active"` in the same transaction, and every per-format field is
copied off the phase. **Both CONF-02 linkage columns are LEFT UNSET** so they take their
`null=True` default, and `qualifier_stage` is left at its `""` default — structurally identical to
the closing playoff of a flat 0-Conference Season:

| Column | Worlds row | Why |
| --- | --- | --- |
...
| `qualifier_stage` | **NOT PASSED** ⇒ `""` | CONF-03's read rule stands — `== "last_chance"` is still the only permitted positive test |

Three existing behaviours then carry it with **no change at all**:
`tournaments_for_phase(worlds_phase)` finds `regional_tournaments` empty and returns
`[phase.tournament]` (or `[]` while unbuilt); `_tournament_phase_complete` requires that single
bracket be `"completed"`, and `bool([])` is `False`, so **the Season parks on an unbuilt Worlds
phase instead of completing championless**; and `_stamp_champion_for_final_phase(worlds_phase)` sees
an empty `regional` list, **skips** the CONF-02 "a Regional playoff crowns no Season champion" early
return, and runs the existing single-bracket branch —
`champion_team = worlds_phase.tournament.champion`, `state="completed"`. **Neither
`complete_if_finished` nor `_stamp_champion_for_final_phase` is edited.** That is the whole point of
the flat shape.

**CHANGED — the bracket floor becomes a keyword-only `minimum`, and only Worlds passes 2.**
`bracket.build_bracket(participants, *, minimum: int = 4)`,
`bracket.build_double_elim_bracket(participants, *, minimum: int = 4)` and
`Tournament.lock_and_build(self, *, minimum: int = 4)` each replace their hard-coded
`len(participants) < 4` check with `< minimum`. `minimum` is **keyword-only** in all three (a bare
`*`), so no positional call site can drift; `build_double_elim_bracket` **must forward
`minimum=minimum`** into its internal `build_bracket` call, and `lock_and_build` forwards it to the
elimination builders only (the `round_robin` / `round_robin_double_elim` and `swiss` early returns
never reach them). **The `ValueError` / `ValidationError` message text is unchanged — literally
`"A tournament requires at least 4 participants."` even when `minimum != 4`** — deliberately not
f-stringified, because existing tests and callers pin the string and the only non-4 caller's own
`>= 2` guard makes the message unreachable there. `bracket.build_rr_de_finals_bracket` keeps the
**default** and must **not** forward `minimum=`; an RR→DE finals build's `wb` is a power of two of
at least 4 by the create-form shape rule. Every other caller — `_build_tournament_for_phase`,
`seed_pending_last_chance_brackets`, `tournament_views` and the sandbox `/tournaments/` surface —
keeps the default and is byte-identical.

> **Why 2 is safe, and why the alternatives were rejected.** `M` can legitimately be 2: two
> Conferences of 2-4 Teams each send one qualifier apiece, and an 8-Team, 2-Conference League is
> exactly what CONF-05's create form produces by default. Skipping Worlds for such a Season would
> mean **the most common Conference setup a user can create silently never crowns anyone**, and
> declaring a bracket-less walkover would invent a second champion-stamping path outside
> `_stamp_champion_for_final_phase` and play no Worlds Match at all, leaving the Playoffs screen
> with a Worlds panel and nothing behind it. The pure builder is already correct below four: `n = 2`
> yields `size = 2`, one round, one node — **that node is the Worlds final**; `n = 3` yields
> `size = 4` with seed 1 taking a pre-resolved round-one bye. The `< 4` guard was inherited
> sandbox-form policy, not a property of the maths. Non-power-of-two fields (5, 7, 9) need no new
> mechanism — `build_bracket` has rounded up to the next power of two and given the top `size − N`
> seeds bye nodes since LG-02a, so the play-in round PLAN.md anticipated would have been a second,
> redundant mechanism.

**Five hook sites, and the ONE deliberate phase-boundary crossing.** Making Worlds its own phase
puts it behind a boundary the drain loops do not cross: `play_season_task` and `play_playoffs_task`
each resolve `season.current_phase()` and cache `tournaments_for_phase(phase)` **once**, and bracket
Matches run through `tournament_engine.play_next_node` rather than `simulate_scheduled_round`, so
the `activate_pending_tournament_phase` hook goes quiet the moment the regular season ends. Left
alone, a "Play whole season" run would stop with the regionals drained and Worlds unbuilt. We follow
CONF-03's seed-then-continue precedent rather than restructuring the loops (an outer per-phase loop
would fix this and any future multi-tournament-phase composition in one move, but it would re-thread
PLAY-01's cooperative cancel checks and `play_season_task`'s stage-budget arithmetic through a new
level of nesting, for a generality nothing yet needs):

| # | Site | Semantics |
| --- | --- | --- |
| A | `tasks.play_playoffs_task`, inside `if not progressed:` **after** the CONF-03 `seed_pending_last_chance_brackets` clause and **before** the `break` | build ⇒ **re-resolve** `phase` and `tournaments`, then `continue`; else `break`. `phase` / `tournaments` are plain locals of the function, so **no `nonlocal`** |
| B | `tasks.play_season_task._drain_one_stage`, after the CONF-03 seed-then-retry block, still **inside** the helper | build ⇒ `nonlocal phase, tournaments` re-resolve, then one more `sum(play_next_bracket_round(...))`. Keeping it inside `_drain_one_stage` is what leaves both loop bodies, the budget arithmetic, `rr_weeks_played` and the PROGRESS emissions untouched — **do not lift it out** |
| C | `league_views.play_single_round`, after the CONF-03 `seeded` block and **before** `season.complete_if_finished()` | fire-and-forget, return value discarded |
| D | `league_views.play_playoffs`, immediately after the `request.session[…]` write and **before** `phase = season.current_phase()` | fire-and-forget; **must precede the cursor read** |
| E | `Season.activate_pending_tournament_phase`, the **first two statements** of the method | `_ensure_worlds_phase()` then `build_pending_worlds_bracket()`, in that order and only that order — the build resolves the phase the ensure just created; both discarded |

Sites A and B are the **only** places the drain loops deliberately cross a phase boundary, because
unlike the Last-chance row the Worlds bracket lives on a **different** phase; because the build
fires only when nothing else progressed, **one budget unit is still exactly one stage**. C is not a
dead click: that click already played its regional node, and without the build the *next* click
would find the cursor on an unbuilt Worlds phase whose `tournaments_for_phase` is `[]` and 400 with
"No active playoff bracket to play." — the user could never start Worlds. Its `phase` /
`tournaments` locals were resolved **before** the build and nothing below re-reads them, so they are
deliberately **not** re-resolved. D must precede `phase = season.current_phase()` and not merely the
`if`, because `build_pending_worlds_bracket` writes `phase.tournament` on its **own** freshly-loaded
`SeasonPhase` instance: a `phase` object read before the build carries a stale
`tournament_id is None`, and `tournaments_for_phase` reads exactly that attribute, so a
build-after-read would still 409. Placing E's build before `phase = self.current_phase()` also
closes a structural hazard — were the cursor ever to reach the Worlds phase with the method running,
the `len(conferences) >= 2` branch below would fan regional brackets out onto it; building first
sets `phase.tournament_id`, so the existing `if phase.tournament_id is not None: return` guard fires
and the fan-out is unreachable. **Do not add a `tournament_mode` guard to that branch — the ordering
is the guard.** Everything else in `activate_pending_tournament_phase` (the four gates, the
`standings` / `strength` / `unseeded` mode split, the CONF-02 idempotence guard, the CONF-03
Last-chance loop and the 0/1-Conference tail) is **verbatim unchanged**.

> **The PROGRESS counts SHRINK at the phase boundary, and that is intended.** `_stage_counts()` in
> both tasks closes over the same `tournaments` name, so after the deliberate re-resolution it
> aggregates the **Worlds bracket alone** and the reported `{"completed", "total"}` drops the
> regional stages. The counts describe the **current** phase, matching CONF-02's "stage counts of
> the phase being drained" contract. Assert the **final** return (`completed == total`, `total > 0`)
> and the terminal DB state; **never** assert that the PROGRESS counts increase monotonically across
> a run.

**Owner mood — a TWO-TIER classification, and `matches/owner_mood.py` is NOT TOUCHED.**
`_classify_playoffs_for_team` picks the first `tournament` phase whose `tournament_id` is set; a
regional phase leaves that NULL, so a `>= 2`-Conference career League has always scored
`("none", 0, 0)` on the owner-mood playoff axis — and the Worlds phase, which **does** set it, would
have switched that axis on **by accident**, charging every Team that failed to qualify the full
`"missed"` penalty regardless of how far it got in its own region. The whole fix lives inside
`league_views._classify_playoffs_for_team`, whose signature, return triple
`(playoff_result, rounds_won, num_rounds)`, four result strings (`"champion"` / `"seeded"` /
`"missed"` / `"none"`) and single call site are **unchanged**. It gains one branch on
`len(season.ordered_conferences()) >= 2`; the `else` arm is **today's body, verbatim and
byte-identical** — including its `tournament_id is not None` scan, which is deliberately **not**
routed through `_worlds_phase()` because a flat Season never grows one, so the scan can never pick a
Worlds phase up. The `>= 2` arm resolves the Team's `conference` via `conference_by_team_id()`, its
Regional playoff via `season._final_tournament_phase()`'s `regional_tournaments`
`.filter(conference=…).exclude(qualifier_stage="last_chance")` (that `.exclude` **is** CONF-03's
read rule, and correctly picks up an un-backfilled `""` row), and the Worlds bracket via
`_worlds_phase().tournament`. Then:

| # | Condition (first match wins) | `playoff_result` | `rounds_won` | `num_rounds` |
| --- | --- | --- | --- | --- |
| 1 | `conference is None` (Team in no Conference — admin-mangled) | `"none"` | `0` | `0` |
| 2 | `regional is None and worlds is None` (no bracket at all) | `"none"` | `0` | `0` |
| 3 | `worlds.champion_id == team_id` | `"champion"` | `0` | two-bracket |
| 4 | Team is a participant of **either** bracket | `"seeded"` | two-bracket | two-bracket |
| 5 | otherwise (participant of neither) | `"missed"` | `0` | two-bracket |

> **Count per bracket, then ADD — never union the raw `bracket_round` integers.** `num_rounds` is
> `max(regional rounds) + max(worlds rounds)` and `rounds_won` is
> `len({distinct regional rounds won}) + len({distinct worlds rounds won})`. The two brackets number
> their rounds independently from 1, so a Team that won round 1 of its region **and** round 1 of
> Worlds has won **two** rounds; a set union would collapse them to one.

`num_rounds` is the same maximum path for every Team in a Conference, so the denominator is fair
within a region even when Conferences differ in size, and **the Last-chance bracket is excluded from
both the numerator and the denominator** — a Team cannot ride it past the maximum path its
Conference offers. `compute_playoffs_delta` is consumed verbatim: its `"seeded"` branch is already
depth-proportional (`(PLAYOFF_ADVANCE_SCALE / num_rounds) * rounds_won`), so feeding it the longer
two-bracket path yields the intended ladder — Worlds champion above Worlds finalist above Conference
champion above regional finalist above first-round exit — **for free**. Inventing per-tier constants
was rejected because CAR-02's values are faithful to ZenGM's `updateOwnerMood` and a new ladder would
have no such source; summing two independent per-bracket deltas was rejected because it doubles the
axis range in both directions against a `FIRE_THRESHOLD` tuned for a single bracket. Rows 2 and 5
are what keep the pre-CONF-04 accident from returning, and row 1 returning `"none"` (the neutral
`0.0` delta) rather than `"missed"` (the `-0.2` penalty) is a **pinned defensive choice**: an
unpartitioned Team in a partitioned Season is broken data, and broken data must not fire a Manager.

**Playoffs screen (`matches/league_screens/playoffs.py`) — and `templates/leagues/playoffs.html` is
NOT EDITED.** Both bracket derivations (the built entry and the pending stub) gain a Worlds branch
evaluated **FIRST**, because the Worlds `Tournament`'s `conference` is `None` and would otherwise
fall into the Season-wide branch. `is_worlds` is read from **`phase.tournament_mode == "worlds"`**
— the PHASE flavour — never from anything on the Tournament row:
...
The pending stub gets the same treatment (`name` `"Worlds"` instead of `"Playoffs"`, plus the same
`key` / `stage` / `stage_label`), so **a phase's DOM id is stable across the unbuilt → built
transition** and a `>= 2`-Conference Season shows a **"Worlds" pending section from the moment the
Season starts** — intended, since the phase is derived at `start_season` and the panel tells the
user Worlds is coming. Every id falls out of the existing `{{ bracket.key }}` interpolations and the
existing `{% if bracket.stage_label %}` badge block renders the Worlds badge for free:
`league-playoffs-phase-<ord>-worlds`, `-stage-`, `-champion-`, `-bracket-`,
`-round-<ord>-worlds-<bracket_round>`, `-node-…` and `-node-score-…`. The Conference sub-heading is
**absent** (`bracket.conference` is `None`). The Worlds pending stub reaches the existing
`{% if bracket.stage == "last_chance" %}` alert's `{% else %}` branch and renders that text
verbatim; that is accepted — **do not add a Worlds-specific alert branch**, the template stays
untouched this slice. The context dict is otherwise unchanged, CONF-03's `worlds_qualifiers` key
included.

> **DOM-id hazard.** `id="league-playoffs-worlds"` is **already taken** by CONF-03's Worlds
> *qualification* table (the seed / team / conference / provenance panel below the brackets). The
> CONF-04 bracket **section** is `league-playoffs-phase-<ord>-worlds`. They coexist on the same page
> for a fully-resolved Season and must never be conflated in an assertion.

**CHANGED `league_views._playoff_cursor_keys` — `following_tournament_is_final` generalised.** From
"the next tournament phase **is** the last phase" (`min(following_tournament_ordinals) ==
phases[-1].ordinal`) to "**nothing but tournament phases follows it**"
(`all(p.phase_type == "tournament" for p in phases if p.ordinal > next_tournament_ordinal)`).
Appending Worlds after the Regional-playoff phase would otherwise flip a season-ending playoff back
to the mid-season "Until Tournament" label. Verified to reproduce today's result on both pre-CONF-04
shapes: `RR(1) → t(2)` stays `True` (vacuously — no phase has a higher ordinal) and
`RR(1) → t(2) → RR(3) → t(4)` stays `False` (phase 3 is `round_robin`), while the new
`RR(1) → t(2) → worlds(3)` now correctly yields `True`. `has_following_tournament_phase` and every
other value the helper returns are **unchanged**.

**Migration `0060_alter_seasonphase_tournament_mode.py`** (dep `0059_tournament_qualifier_stage`):
exactly **one `AlterField`**, choices-only, with **no `RunPython`, no backfill, no data migration
and no second migration**. A choices-only `AlterField` has no database-level effect on PostgreSQL or
SQLite, and no column is added, dropped or re-typed anywhere.

**Explicitly NOT changed — the byte-identity invariants.** (1) **A Season with 0 or 1 Conference is
byte-identical** in rows, reads, champion stamping and rendered DOM ids: `_ensure_worlds_phase()`
returns `None`, `build_pending_worlds_bracket()` returns `False`, no `SeasonPhase` and no
`Tournament` row is added, and the rendered Playoffs screen contains **no `-worlds` substring**.
(2) **Every CONF-02 / CONF-03 DOM id is unchanged** — the `-worlds` suffix appears only on a Worlds
entry, `stage_label` stays `""` for Season-wide and regional brackets so no new element renders on a
CONF-02 Season, and `league-playoffs-worlds` continues to mean CONF-03's qualification table and
nothing else. (3) **Every existing caller of `build_bracket` / `build_double_elim_bracket` /
`lock_and_build` keeps `minimum=4`**, error message strings included. (4) **`matches/owner_mood.py`
is not touched** — no new constant, no new branch, no new `playoff_result` value. (5) **CONF-03's
`qualifier_stage` read rule survives**: every read still tests only `== "last_chance"`, and **there
is deliberately NO `qualifier_stage == "worlds"`** — the Worlds bracket is identified from
`phase.tournament_mode`. (6) **The bracket engine is untouched** — `play_next_node`,
`play_specific_node`, `play_next_bracket_round`, `find_next_node`, `advance_winner`,
`advance_loser`, `stage_progress`, `series_length_for_round`, `Tournament.find_next_playable_node`
and `_persist_elim_specs` are all reused verbatim. (7) **`complete_if_finished` and
`_stamp_champion_for_final_phase` are not edited.** (8) **`templates/leagues/playoffs.html` is not
edited.** (9) **`matches/worlds.py` is not edited** — CONF-03's pure module stays pure, frozen
import allowlist (`dataclasses`, `typing`), no Django, no ORM. (10) Exactly one migration,
choices-only. (11) **No Score Calibration re-baseline.** Also unchanged: `worlds_qualifiers()`,
`tournaments_for_phase()`, `_build_tournament_for_phase()` (the Worlds build does **not** go through
it), `seed_pending_last_chance_brackets()`, `_preceding_phase` / `_phase_complete` /
`_tournament_phase_complete` / `_final_standings_for_phase` / `_seed_order_for_phase` /
`ordered_phases` / `ordered_conferences` / `conference_by_team_id`, `matches/standings.py`,
`matches/tournament_views.py` and the simulator.

**Naming hazards, and every one of them has real bite.**
- **`tournament_mode == "worlds"` (the PHASE flavour) vs `qualifier_stage` (the BRACKET stage).**
  Different models, different questions. The Worlds `Tournament` carries `qualifier_stage == ""`;
  anything asking "is this the Worlds bracket?" asks the **phase**.
- **`_final_tournament_phase()` now means "the final NON-Worlds tournament phase"** — the
  Regional-playoff phase. For a `>= 2`-Conference Season it is **no longer** `ordered_phases()[-1]`.
  Use `_worlds_phase()` to reach the Worlds phase and `ordered_phases()[-1]` where "the last phase"
  is meant (`complete_if_finished` does exactly that, and is unchanged).
- **`minimum` (the builder / lock kwarg, default `4`) vs `MIN_BRACKET_PARTICIPANTS`
  (`matches/models.py:18`, value `4`).** They are **not** wired together — `matches/bracket.py` is a
  pure module and must not import `models.py`, so the builders' default is the literal `4`.
- **`Tournament.season_phase` vs `Tournament.season_phases`** (carried forward from CONF-02/CONF-03).
  The Worlds `Tournament` has `season_phase_id is None` **and** a non-empty `season_phases` whose
  `.first()` **is** the Worlds phase. Never use one to test for the other.
- **`worlds_qualifiers` (CONF-03's derived field, and the Playoffs-screen context key) vs the Worlds
  `Tournament`'s `participants`.** The first is a list of frozen `WorldsQualifier` dataclasses with
  a `seed` attribute; the second is a `TournamentParticipant` queryset. They carry the same team ids
  and the same seeds after the build, and are still different objects.
- **`build_pending_worlds_bracket` (returns `bool`) vs `seed_pending_last_chance_brackets` (returns
  `int`).** Both are idempotent "do it if ready" seams sitting side by side at four of the same hook
  sites, and their return types differ. `if season.build_pending_worlds_bracket():` and
  `if season.seed_pending_last_chance_brackets(phase) > 0:` are both correct as written — do not
  "harmonise" them.
- **`_ensure_worlds_phase` (creates the PHASE) vs `build_pending_worlds_bracket` (creates the
  BRACKET).** Both run at the top of `activate_pending_tournament_phase`, in that order, and only
  that order works.

**Locked names (quick index).**
- **`SeasonPhase`** NEW `TOURNAMENT_MODE_CHOICES` fifth entry `("worlds", "Worlds")`; no new
  `phase_type`, no new field.
- **`Season`** NEW `_ensure_worlds_phase() -> SeasonPhase | None`, `_worlds_phase() -> SeasonPhase |
  None`, **public** `build_pending_worlds_bracket() -> bool` (`@transaction.atomic`); CHANGED
  `_final_tournament_phase()` (narrowed to skip `worlds`), `start_season()` (one inserted call) and
  `activate_pending_tournament_phase()` (two inserted calls at the top). `complete_if_finished` /
  `_stamp_champion_for_final_phase` / `worlds_qualifiers` / `tournaments_for_phase` /
  `_build_tournament_for_phase` **unchanged**.
- **`bracket.py` / `Tournament`** CHANGED `build_bracket(participants, *, minimum=4)`,
  `build_double_elim_bracket(participants, *, minimum=4)` (**forwards** `minimum` inward) and
  `Tournament.lock_and_build(*, minimum=4)`; error strings unchanged;
  `build_rr_de_finals_bracket` keeps the default.
- **Hook sites** `tasks.play_playoffs_task` (build-then-re-resolve-then-`continue`),
  `tasks.play_season_task`'s `_drain_one_stage()` (`nonlocal` re-resolve then one retry),
  `league_views.play_single_round` (before `complete_if_finished`), `league_views.play_playoffs`
  (before the cursor read), `Season.activate_pending_tournament_phase` (ensure + build, the first
  two statements); plus `Season.start_season` (ensure) and `league_views._run_season_rollover`
  (skip the `worlds` phase).
- **Owner mood** `league_views._classify_playoffs_for_team` only — additive `>= 2`-Conference
  branch, signature and return triple unchanged; **`matches/owner_mood.py` untouched**.
- **Screens** `league_screens/playoffs.py` (Worlds branch first in both derivations: `key`
  `"<ord>-worlds"`, `stage` `"worlds"`, `stage_label` `"Worlds"`, stub `name` `"Worlds"`) +
  `league_views._playoff_cursor_keys` (`following_tournament_is_final` generalised);
  **`templates/leagues/playoffs.html` untouched**.
- **Migration** `matches/migrations/0060_alter_seasonphase_tournament_mode.py` (dep `0059`, one
  choices-only `AlterField`, no `RunPython` / backfill).
- **Tests** `matches/tests/test_worlds_tournament.py` (new — phase derivation and its gates, the
  build seam and its idempotence, `seed=q.seed`, the `M = 2` and `M = 3` paths, the drain to a
  champion, the flat-row shape, the `play_playoffs` 202 and `play_single_round` non-dead-click pins,
  `lock_and_build(minimum=…)`, and the Playoffs-screen Worlds section) plus one appended class each
  in `matches/tests/test_regional_playoffs_drain.py` (both task hook sites end to end),
  `test_season_playoffs.py` (parks unbuilt, crowns on completion), `test_owner_evaluations_writer.py`
  (the decision table through the writer), `test_league_next_season.py` (the rollover skip) and
  `test_bracket.py` (the `minimum=` kwarg, keyword-only, DE forwarding, unchanged message). House
  rule unchanged: brackets are drained by **driving the engine or stamping node winners**, never by
  running the simulator, and never by patching the seam under test.
- **ADR / CONTEXT** — [ADR-0037](../../docs/adr/0037-worlds-is-a-derived-season-phase.md) + the
  CONTEXT.md **Worlds** / **Worlds phase** / **Season phase** terms. Invariants: a
  **0/1-Conference Season is byte-identical**; **every CONF-02 / CONF-03 DOM id is unchanged**;
  **`owner_mood.py`, the bracket engine, `worlds.py` and the Playoffs template are all untouched**;
  **no `qualifier_stage == "worlds"`**; and **no Score Calibration re-baseline**. **DEFERRED to
  later slices:** per-Conference rotating map pools (CONF-06), any placement / elimination-depth
  ranking API on `Tournament`, a persisted qualifier or Worlds-standings table, and any authoring UI
  or composer wire token for the Worlds phase.
...
## CONF-06 rotate_by_conference per-Conference map pools

A **5th `Season.map_mode` value `rotate_by_conference`** (label `Rotate by Conference`, appended
as the fifth and last `MAP_MODE_CHOICES` tuple): each **Conference** carries its own
author-ordered `ArenaMap` rotation, and a regular-season fixture resolves its map from **its own**
Conference's rotation — "Nevada games on the Nevada map". Unambiguous because regular-season
fixtures are always **intra-Conference**, so `fixture.team_a_id` alone identifies the Conference.
The per-partition analogue of SUB-01's Season-level `rotate_by_matchday`, one level down on the
CONF-01 partition; the epic's closing slice. Fully deterministic, **NO RNG** (the SIM-07 seed
chain is untouched), **no simulator edit** (reuses the SIM-09 `arena_map=` kwarg through the same
three play loops), and **no Score Calibration re-baseline** (see *Scope-out*). `Season.clean()` is
**UNCHANGED** — it derives `valid_map_modes` from `MAP_MODE_CHOICES`, so the new literal validates
automatically.

**Model fields (2 NEW on `Conference`, declared immediately after `starting_team_ids_json`).**
(1) `Conference.map_rotation_ids_json` — the LIVE author-ordered id list, written by the
`manage_conferences` POST; (2) `Conference.starting_map_rotation_ids_json` — the **activation
snapshot**, written by `Season.start_season()` and the ONLY list the play loops read. Both
`models.JSONField(null=True, blank=True, default=None)`. `None` is the pre-authoring /
pre-activation sentinel and `[]` is an authored-empty rotation — the two are deliberately
distinct, and every consumer reads `(x or [])`. The order-preserving twin of the SUB-01
Season-level pair: **author order is PRESERVED, NEVER `sorted()`** in either field (the deliberate
...
**`Season.start_season()` extension (2 edits, both inside the existing method).** (a) An
**activation guard** placed after the existing `self.teams.count() < 2` check and before the
`starting_team_ids_json` assignment: under `map_mode == "rotate_by_conference"`, fewer than 2
Conferences **or** any Conference with an empty live rotation raises `ValidationError("A Season
with map mode 'Rotate by Conference' requires at least 2 conferences, each with at least 1
rotation map.")` — one locked string covering both failure shapes. (b) The **snapshot**, written
inside the EXISTING per-Conference loop (not a second loop) as
`conf.starting_map_rotation_ids_json = list(conf.map_rotation_ids_json or [])` with
`"starting_map_rotation_ids_json"` added to that loop's `update_fields`. The snapshot is written
for **every** Season that has Conferences regardless of `map_mode` — an unused snapshot is
harmless and keeps the loop mode-agnostic.

**`_resolve_fixture_map` — 4th argument + NEW branch (`matches/tasks.py`).** The helper grows a
fourth **keyword-with-default** parameter: `_resolve_fixture_map(season, fixture, pool_by_id,
conf_by_team: "dict[int, Conference] | None" = None) -> "ArenaMap | None"` (`Conference` joins the
`TYPE_CHECKING` import). **Backwards compatibility is load-bearing** — all 37 existing
3-argument call sites in `matches/tests/test_season_map_config.py` and `test_league_play.py` keep
working unchanged, and `None` is treated as `{}`. The new branch sits after the
`rotate_by_matchday` branch and before the final `raise`: look up `fixture.team_a_id` in
`conf_by_team`, read `ids = (conf and conf.starting_map_rotation_ids_json) or []`, `return None`
if empty, else `return pool_by_id.get(ids[fixture.matchday % len(ids)])`. Purity is preserved (no
ORM inside the helper) and the branch never constructs a `random.Random`. Every failure shape
returns `None` rather than raising: missing / `None` `conf_by_team`, a `team_a_id` absent from the
map, a `None`/`[]` snapshot, or an `ArenaMap` deleted after activation (`pool_by_id.get` misses).
Unknown modes still `raise ValueError(f"Unknown map_mode: {mode!r}")`.

> **The rotation index is 1-BASED and that is correct-by-contract.** `fixture.matchday` is
> Season-global and 1-based, and `matchday % len(ids)` is applied to that value **directly**,
> byte-mirroring the shipped `rotate_by_matchday` formula. With a 2-map rotation, matchday 1
> resolves `ids[1]` and matchday 2 resolves `ids[0]`. This is **NOT an off-by-one** — do **not**
> "fix" it to `(matchday - 1) % len(ids)`; a pinned test in `test_season_map_config.py` asserts
> the 1-based expectation.

**NEW pure helper `matches.tasks._fixture_map_ids(season, conf_by_team=None) -> list[int]`**
(module-level, immediately after `_resolve_fixture_map`, no ORM) — the union of every `ArenaMap`
id the resolver can reach this Season, and the single argument to the play loops' one
`ArenaMap.objects.in_bulk`. Order-preserving concatenation: `season.starting_map_pool_ids_json`,
then `season.starting_map_rotation_ids_json`, then each **first-seen** Conference's
`starting_map_rotation_ids_json` iterating `conf_by_team.values()` in dict order. Conferences are
deduped by `conf.id` (the same Conference appears once per member team); **map ids are NOT
deduped** — duplicates are harmless for `in_bulk`. With no Conferences it reduces to the old
`(pool or []) + (rotation or [])` expression **exactly**.

**NEW pure helper `matches.forms.parse_rotation_ids(raw: "str | None", valid_map_ids: "set[int]")
-> "tuple[list[int], list[str]]"`** (module-level, below `_maps_with_confirmed_config()`; no ORM,
no form state) — parses a comma-separated `ArenaMap`-id string into an **author-ordered** id list
plus a list of human error strings. Extracted **verbatim** from the old inline
`CreateLeagueForm.clean` token loop and now shared by the create form and the `manage_conferences`
POST. Locked behaviour: author order, never sorted; **duplicates kept** (`"3,3"` → `[3, 3]`);
blank tokens skipped silently; whitespace stripped (`" 3 , 4 "` → `[3, 4]`); one error string **per
offending token**, not deduped; valid ids from a partly-bad string still returned alongside the
errors; `None` / `""` → `([], [])`. Its two error strings are unchanged from LG-01j/SUB-01:
`"Map rotation contains an invalid id."` (non-integer token) and `"Map rotation contains an unknown
map id."` (id not in `_maps_with_confirmed_config()`). In `CreateLeagueForm.clean` the refactor is
behaviour-preserving — `cleaned_data["map_rotation_ids"]` keeps its exact current name and shape.

**Four guards** enforce the mode's shape — **≥2 Conferences** (Conferences are 0 or ≥2 by the
CONF-01 partition rule, so "≥2" is the only meaningful floor) each with **≥1 rotation map**:

| # | Where | Verbatim string(s) |
|---|---|---|
| 1 | `CreateLeagueForm.clean`, a `rotate_by_conference` block after the `rotate_by_matchday` one, using `add_error` (NOT `raise`) so all three surface in one submission | `Map pool must be empty when Map mode is 'Rotate by Conference'.` (field `map_pool`) / `Map rotation must be empty when Map mode is 'Rotate by Conference'.` (field `map_rotation`) / `Map mode 'Rotate by Conference' requires at least 2 conferences.` (field `number_of_conferences`) |
| 2 | `manage_conferences` POST, appended **once** (not per Conference) and only when `season.map_mode == "rotate_by_conference"`, `names` is non-empty and any parsed row is empty | `Each conference needs at least 1 rotation map.` (page-level `errors`, `#manage-conferences-errors`) |
| 3 | `Season.start_season()` activation guard | `A Season with map mode 'Rotate by Conference' requires at least 2 conferences, each with at least 1 rotation map.` |
| 4 | `_run_season_rollover` — the downgrade below | (no message; silent mode change) |

**Three play-loop call sites**, identical shape, each reordered so `conf_by_team` is built
**above** the `pool_by_id` assignment (all three previously built it *after* the `in_bulk`):
**build `conf_by_team = season.conference_by_team_id()` → `_fixture_map_ids(season, conf_by_team)`
→ `ArenaMap.objects.in_bulk(...)` → per-fixture `_resolve_fixture_map(..., conf_by_team=conf_by_team)`**.
The sites are `matches.tasks.play_season_task`, `matches.league_views.play_week`, and
`matches.league_views.play_week_live` (the manager live-game RR branch); the latter two extend
their deferred import to `from matches.tasks import _fixture_map_ids, _resolve_fixture_map`.
Nothing else at these sites changes — for every pre-existing Season (`map_mode !=
...
Conferences forward (and CONF-06 deliberately does not start doing so), so a carried
`rotate_by_conference` would meet an empty `conf_by_team`, resolve `None` for **every** fixture
all season, and silently produce a map-less Season that still claimed a map mode. Downgrading to
the 3-zone fallback makes that honest. Every other mode still carries **verbatim**, and the
Season-level `map_rotation_ids_json` carry-forward is untouched.

**Authoring — `manage_conferences` (view + context + template).** The rotation rides the CONF-05
page rather than a new screen, so it is editable **only while the Season is `draft`** — it
inherits that page's frozen-partition behaviour, and a non-draft POST still returns
`HttpResponseBadRequest("Conferences can only be edited while the Season is in draft.")`.
`_manage_conferences_context(...)` gains a keyword-only `submitted_rotations: "list[str] | None" =
None` and four context keys: `conf_rows` (`list[dict]` of `{"name", "rotation"}`, index-aligned
with the preserved `conf_names`, `rotation` being the comma-joined id string, from the submitted
value on a failed POST else from `conf.map_rotation_ids_json`), `confirmed_maps`
(`list(_maps_with_confirmed_config())`), `map_mode` (`season.map_mode`, gating the "required"
hint), and `readonly_groups[i]["rotation_names"]` (author-ordered names off
`starting_map_rotation_ids_json`, falling back to the live list when the snapshot is `None`;
deleted ids drop out silently). POST order of operations: read `conference_rotation` via `getlist`
index-aligned with `conference_name` → run the **UNCHANGED** `_validate_conference_partition` →
`parse_rotation_ids` per row **for every map mode** (a malformed id is invalid regardless of mode),
appending its errors in submitted Conference order → the once-only empty-rotation guard → on
failure re-render 200 with `submitted_rotations` and write nothing → on success set
`map_rotation_ids_json=per_conf_ids[ordinal - 1]` in the existing `Conference.objects.create(...)`
inside the existing `transaction.atomic()` (`normalized` preserves submitted index order), storing
the parsed list verbatim (`[]` for an authored-empty rotation, never `None` once saved).

**Template `templates/seasons/manage_conferences.html`** mirrors the `league-create-map-rotation`
precedent: a hidden comma-joined input per Conference row filled by a vanilla-JS composer on
`change` / `submit`. NEW DOM ids — `manage-conferences-confirmed-maps` (a
`<script type="application/json">` payload of `[{"id", "name"}, …]`, rendered once in the editable
branch), `manage-conferences-rotation-{i}` (the hidden `name="conference_rotation"` input, inside
`.conference-row`), `manage-conferences-rotation-composer-{i}`,
`manage-conferences-rotation-add-{i}` (label `+ Add map`), `manage-conferences-rotation-row-{i}-{j}`,
`manage-conferences-rotation-select-{i}` (`aria-label="Rotation map"`), and
`manage-conferences-readonly-rotation-{i}` (the non-draft author-ordered list inside the existing
`#manage-conferences-readonly` block). **Every existing CONF-05 DOM id is unchanged and must
survive** (`-empty` / `-errors` / `-form` / `-list` / `-name-{i}` / `-add` / `-team-{team_id}` /
`-submit` / `-readonly`). JS rules: each `.conference-row` owns one composer and re-serializes
**its own** hidden input in document order; `+ Add conference` must build the full new row (name
input **and** hidden rotation input **and** composer **and** add-map button) at the next `i` and
wire it; removing a Conference row removes its rotation input with it, keeping the two `getlist`s
index-aligned; all hidden inputs re-serialize on form `submit`; a pre-filled `conf_rows[i].rotation`
is **rehydrated into composer rows on page load** so an existing partition round-trips without
re-picking maps; ASCII only. The existing `rebuildSelects()` team-`<select>` behaviour is untouched.

**Scope-out (locked).** **Per-fixture map override DEFERRED** — a `ScheduleFixture` is a plain
dataclass regenerated on demand by `generate_schedule`, not a persisted row, so there is nowhere to
hang an override; reopening it needs its own model keyed on synthetic fixture identity plus an
authoring surface (PLAN.md **Deferred Items**). **Bracket / tournament maps out of scope by
construction, not by omission** — Regional-playoff and Worlds `Match` rows never carry an
`arena_map` at all and always run the 3-zone fallback, so `rotate_by_conference` governs
**regular-season fixtures only**. **NO Score Calibration re-baseline** — the new branch is
unreachable without the new mode and `_fixture_map_ids` reduces to the current expression when
there are no Conferences, so every existing seeded outcome stays byte-identical and no
fixture/expected-score file is regenerated. **NO `SeasonAdmin` / Django-admin surface** for the
per-Conference rotation. **NO carrying of Conferences (or their rotations) through `next_season`**
— the downgrade is the whole of the rollover story. **NO change to
`_validate_conference_partition`'s signature or strings** (`"Conference names cannot be empty."` /
`"Every team must be assigned to a conference."` / `"Each conference needs at least 2 teams."`
...
across all six locked properties and both error strings); `test_conference.py` (DB: author order
survives `start_season()` via a deliberately non-ascending list, `[]` snapshot from a `None`/`[]`
live list, the activation `ValidationError` for 0 Conferences and for any empty rotation, and its
silence under other modes); `test_manage_conferences.py` (draft GET renders
`manage-conferences-rotation-0` + `manage-conferences-confirmed-maps`, POST saves per-Conference
ids in submitted order, both guard re-renders at 200 with the verbatim strings, non-draft GET
renders `manage-conferences-readonly-rotation-0` with no editable input, non-draft POST still
400); `test_league_next_season.py` (the downgrade, other modes verbatim); and
`test_league_create.py` (the three `CreateLeagueForm` guards under their exact field keys plus a
valid `rotate_by_conference` submission). Plus the regression guard: an existing
`rotate_by_matchday` / `random_per_round` / `single` / `none` Season resolves byte-identical maps
with and without a `conf_by_team` argument. Seam contract:
[`.claude/worktrees/conf-06-seam-contract.md`](../../.claude/worktrees/conf-06-seam-contract.md);
CONTEXT.md **Map mode** / **Map pool** / **Per-fixture map resolution** / **Conference** terms;
PLAN.md **CONF-06**.
