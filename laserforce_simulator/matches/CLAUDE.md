...
The manager-mode live-watch surface over the LG-01d Play dropdown — a **"One Week (Live)"** entry that lets the manager **watch `League.current_team`'s next game replay in-browser** (via the SIM-05 client playback engine) and then **commit or discard** it. The locked shape is **preview-then-commit, NOT server-side tick streaming** (no WebSocket / SSE / Channels): a preview draws a seed (or a pair), `random.seed()`s it, runs the in-memory tick loop with **NO DB flush**, serializes the events to the SIM-05 `events_data` / `players_data` JSON shape, **pins the captured seed(s) in `request.session`**, and ships them to the page; the client plays the pre-baked JSON. Commit re-runs **only the watched game with the injected captured seed(s)** (SIM-07 ⇒ byte-identical to what was watched) then sims the rest of the matchday / bracket stage **fresh, sync, atomically** — exactly the existing Play One Week / weekly-playoff pacing. **Mode B is locked: only the manager's game is ever previewed; every other fixture / node is simmed fresh at commit** (their seeds drawn normally, never injected). Because the injected-seed kwargs default to `None` ⇒ verbatim-today behaviour ⇒ **byte-identical to every existing caller ⇒ NO Score Calibration re-baseline**. **No model change, no migration, no ADR, no CONTEXT.md edit** (the **One Week (Live)** glossary term was finalised at grilling, sitting above **Current team** in `### League and seasons`; preview / commit / pin are implementation language reusing the existing **Matchday** / **Season phase** / **Job** vocabulary). Seam contract: [`.claude/worktrees/lg-01i-one-week-live-seam-contract.md`](../../.claude/worktrees/lg-01i-one-week-live-seam-contract.md).

**Two watchable cursor types.** (1) **RR cursor** — `current_team`'s **SINGLE next-matchday Round** (the two Rounds of a Match are two separate `simulate_scheduled_round` calls, so the RR live surface carries exactly **1 seed**). If `current_team` has a **bye** that matchday (not in the next matchday's fixtures) ⇒ degrade to a **plain commit** (`play_week`'s matchday sim, no live surface). (2) **Playoff cursor** — `current_team`'s **next undecided 2-round playoff Match** (the next undecided `BracketNode` it occupies), shown with a round-1/round-2 toggle (**2 seeds**). Offered ONLY IF `current_team` is **alive** — has an incomplete, non-bye bracket node it participates in whose Series has not started (`series_matches.count() == 0`, so the watched Match is always **game 1** of the node's Series). Eliminated / not-a-participant ⇒ **no live entry**.

**The injected-seed seam (additive, keyword-only, `None` ⇒ verbatim today).** Three engine methods grow an appended keyword-only seed kwarg; `None` reproduces today's draw-fresh behaviour exactly (the byte-identical-default regression protects Score Calibration). (a) **`BatchSimulator._simulate_and_flush_round(..., rng_seed: int | None = None)`** — `seed = random.Random().getrandbits(63) if rng_seed is None else rng_seed`, then `random.seed(seed)`, and the SAME int is `random.seed()`'d AND persisted onto `GameRound.rng_seed` (so a committed round's `rng_seed == the captured preview seed`). (b) **`BatchSimulator.simulate_scheduled_round(season, team_a, team_b, round_number, *, arena_map=None, season_phase=None, leg=1, rng_seed: int | None = None)`** — threads `rng_seed` straight into the single `_simulate_and_flush_round` call (both the round-1 and round-2 branches); the Side-agnostic find-or-create, the post-round `activate_pending_tournament_phase()` then `complete_if_finished()` hooks, the per-round colour swap, and the `season_phase` / `leg` keys are UNCHANGED. (c) **`BatchSimulator.simulate_match(team_red, team_blue, match_type="friendly", *, arena_map=None, before_round_hook=None, rng_seeds: tuple[int, int] | None = None)`** — its two `_simulate_and_flush_round` calls pass `rng_seed=(None if rng_seeds is None else rng_seeds[0])` / `[1]`; the LG-02x-1 `before_round_hook` random-draw and the per-Match colour swap are orthogonal and unchanged. (d) **`matches/tournament_engine.py::play_specific_node(node, *, rng_seeds=None)`** — a NEW thin `@transaction.atomic` sibling **extracted from** `play_next_node`, taking the node **directly** (skips `find_next_playable_node`) and passing `rng_seeds=` into `simulate_match`; `play_next_node` is refactored to `node = tournament.find_next_playable_node(); if node is None: return None; return play_specific_node(node)` so the per-Match resolve/advance/clinch/drop/grand-final/champion tail is shared and `play_next_node` stays byte-identical (calls `play_specific_node(node, rng_seeds=None)`). The playoff commit calls `play_specific_node(watched_node, rng_seeds=captured_pair)` for the watched Match, then loops `play_next_node` for the rest of the stage.

**The preview engine (sim WITHOUT flush, returns a serializable bundle).** Two new `BatchSimulator` methods, each drawing the seed(s), `random.seed()`'ing, running `_simulate_round` (the pure in-memory tick loop), and serializing the in-memory `event_log` + `PlayerState` rosters into the SIM-05 shape with **NO `_flush_to_db`**. **`BatchSimulator.preview_scheduled_round(season, team_a, team_b, round_number, *, arena_map=None, season_phase=None, leg=1, rng_seed=None) -> dict`** (RR, 1 seed; `team_a` plays red / `team_b` blue, matching `simulate_scheduled_round`'s round-1 arg order). **`BatchSimulator.preview_tournament_match(team_red, team_blue, *, arena_map=None, rng_seeds=None) -> dict`** (playoff, 2 seeds; round 1 `red→blue`, round 2 `blue→red`, the per-Match colour swap exactly as `simulate_match`; arena `None` = 3-zone fallback per the tournament path). Both accept the optional `rng_seed=` / `rng_seeds=` so a **re-open replays the pinned game deterministically**. **The preview bundle (return-dict, LOCKED):** `{kind: "rr" | "playoff", seed: int (RR), seeds: [int,int] (playoff), rounds: [...], cursor: <identity §below>}`; each `rounds` entry is `{round_number, red_team_id, red_team_name, blue_team_id, blue_team_name, events_data, players_data, result}` where `result` is the slim banner dict `{red_points, blue_points, red_eliminated, blue_eliminated}` (1 entry for RR, 2 for playoff). The `events_data` / `players_data` **reuse the SIM-05 `game_round_events` view shapes** so the duplicated playback JS reads them unchanged: each event dict carries the **13 keys** `type, ts, tf, icon, desc, pts, aid, an, at, tid, tn, tt, meta` (`at` / `tt` are the actor/target **Team** ids resolved from each in-memory player's `team_color`; `tid` defaults `-1`, `tn` / `tt` default `""`; the SIM-05 JS derives `idx` / `sec` client-side — **do NOT bake them**); each player dict carries `{id, name, team, role, sl, ss}` (`team` is the **colour string** `"red"` / `"blue"`, NOT a Team id; sourced from `PlayerState.player_id` / `name` / `team_color` / `role` / `starting_lives` / `starting_shots`).

**`_serialize_events_for_preview` (single source, replicate-not-persist).** A private helper `matches/simulation/...::_serialize_events_for_preview(event_log, red_players, blue_players, red_team, blue_team) -> tuple[list[dict], list[dict]]` builds `players_by_id` from the two `PlayerState` lists, a `team_id_by_color = {"red": red_team.id, "blue": blue_team.id}`, **replicates** the 11-entry icon map (default `"•"`) and the mm:ss formatter (`total = int(ts * 0.5); f"{total//60:02d}:{total%60:02d}"`), and returns `(events_data, players_data)`. It **replicates** the icon / `÷2` mm:ss logic — it does **NOT** construct unsaved `GameEvent(...)` instances (accidental-save risk + ORM FK requirements). Those two behaviours are the only ones that must stay in sync with `GameEvent`; a test pins the preview icon for every event type against `GameEvent(event_type=t, timestamp=0).get_event_icon()`.

**Three new views (`matches/league_views.py`), reusing the LG-01d guards / redirect idioms.** (a) **`play_week_live_preview(request, season_id) -> HttpResponse`** — GET-only (`HttpResponseNotAllowed(["GET"])` first line), `get_object_or_404(Season)`, sets `request.session["last_league_id"] = season.league_id`; non-active Season ⇒ `_render_season_dashboard_error(...)` (the LG-01d `play_error` 400 dashboard re-render); resolves the live cursor (below) — **no live entry** (no `current_team` / RR bye / eliminated playoff / no tournament) ⇒ a 400 dashboard re-render (defensive — the dropdown is gated not to render in these states); on an RR / playoff cursor it reads the session pin, and a **VALID** pin for THIS cursor identity ⇒ re-run the matching preview method with the **pinned seed(s) injected** (re-open replays the same game), else runs the preview **fresh** (draws + writes the pin), then renders `templates/seasons/play_week_live.html` ⇒ **200**. (b) **`play_week_live_commit(request, season_id) -> HttpResponse`** — POST-only (405 otherwise), 404 on missing Season, sets `last_league_id`, non-active ⇒ 400; **missing / stale pin** (cursor identity moved) ⇒ `_render_season_dashboard_error(request, season, "Preview expired — re-open the live preview.")` (400, never silently sim fresh — that would defeat the byte-identical guarantee). **RR commit** (inside ONE `transaction.atomic`, mirroring `play_week`): resolve the next matchday's fixtures via the LG-02-Part2c-2 `by_phase` / `played_keys` / `select_play_fixtures(..., 1)` machinery; the **watched fixture** calls `simulate_scheduled_round(..., rng_seed=pin.seed, ...)`, every OTHER fixture calls it with **no** `rng_seed` (fresh), reusing the same `season_phase` / `leg` plumbing. **Playoff commit** (no outer atomic — per-Match atomicity from the engine, mirroring `play_week`'s playoff branch): `play_specific_node(watched_node, rng_seeds=tuple(pin.seeds))`, then drain the rest of the stage (commit the watched node first, then `play_next_bracket_round(tournament)` for the rest — `find_next_playable_node` skips the already-resolved watched node, so no double-commit), then `season.complete_if_finished()`. Both branches **clear the pin** then `redirect("season_dashboard", season_id=season.id)` ⇒ **302**. (c) **`play_week_live_discard(request, season_id) -> HttpResponse`** — POST-only (405 otherwise), 404 on missing Season, sets `last_league_id`, **clears the pin** (NO simulation, NO DB write), `redirect("season_dashboard", ...)` ⇒ **302**.

**The session pin — "locked once previewed".** Key `request.session["live_preview_pin"]` is a `dict` keyed by `str(season_id)` (so independent Seasons hold independent pins); each entry is `{kind: "rr" | "playoff", current_team_id: int, cursor: <identity-dict>, seed: int (RR) | seeds: [int,int] (playoff)}`. The **cursor-identity dict** is the auto-invalidation key — **RR:** `{type: "rr", season_id, season_phase_id, matchday, pair: [min_id, max_id], round_number, leg}` (mirrors the `(season_phase_id, frozenset(pair), round_number, leg)` played-key); **playoff:** `{type: "playoff", tournament_id, bracket_type, bracket_round, position}` (the watched node's coords). **Lifecycle:** a fresh preview **writes** the pin (drawn seed(s) + cursor identity + `current_team_id`, `request.session.modified = True`); re-open / commit **read & validate** — a pin is VALID iff its `cursor` dict EQUALS the freshly-recomputed cursor identity AND its `current_team_id == season.league.current_team_id`. There is **no explicit invalidation step** — the cursor-identity EQUALITY check **IS** the auto-invalidation: once the watched matchday / node is played, the recomputed cursor moves on, the stale pin no longer matches and is treated as absent (overwritten on the next preview, 400 on a commit). Discard and a successful commit both `live_preview_pin.pop(str(season_id), None)` + set `modified = True`.

**Cursor dispatch + bye / eliminated degradation.** A NEW module-level helper **`matches.league_views._resolve_live_cursor(season) -> dict | None`** returns a cursor descriptor or `None`, in LOCKED order: (1) `team = season.league.current_team`; `None` ⇒ return `None`. (2) `phase = season.current_phase()` — **playoff cursor** if `phase` is a built+active tournament phase (`phase_type == "tournament"`, `tournament_id is not None`, `tournament.state == "active"`): resolve `team`'s next undecided non-bye node via **`_alive_playoff_node(phase.tournament, team)`**; `None` (eliminated / not a participant / no undecided node) ⇒ `None` (no live entry), else a `{"kind": "playoff", "node", "tournament", "cursor", "red_team": node.team_a, "blue_team": node.team_b}` descriptor. **RR cursor** for an RR (or implicit-fallback) phase: compute the next unplayed matchday's fixtures (the `play_week` `by_phase` / `played_keys` / `select_play_fixtures(..., 1)` machinery); the fixture whose Side-agnostic pair contains `team.id` ⇒ `{"kind": "rr", "fixture", "season_phase", "cursor", "red_team": team_a, "blue_team": team_b}` (team_a plays red); **none (manager has a bye)** ⇒ a special `{"kind": "rr_bye"}` descriptor ⇒ the view degrades to a plain commit. (3) anything else ⇒ `None`. The alive-detection helper **`matches.league_views._alive_playoff_node(tournament, team) -> BracketNode | None`** returns `team`'s next playable undecided non-bye node it occupies (`winner_id is None`, `is_bye is False`, both slots filled, `team_a_id == team.id or team_b_id == team.id`, lowest `(_BRACKET_RANK[bracket_type], bracket_round, position)`, `series_matches.count() == 0`), or `None`.

**URLs (`matches/season_urls.py`), inserted BEFORE `standings/` / `schedule/` (first-match resolution), adjacent to the LG-01d play routes; bare names (no `app_name`):** `play_week_live` (`/seasons/<int:season_id>/play-week-live/` → `play_week_live_preview`), `play_week_live_commit` (`.../commit/`), `play_week_live_discard` (`.../discard/`).

**Template `templates/seasons/play_week_live.html`.** Extends `base.html`, `d-flex` + `{% include "_partials/league_sidebar.html" %}` shell with `sidebar_active=None` (the LG-01f league-screen convention). It embeds the SIM-05 playback engine fed by **in-memory events JSON** — it **cannot** reuse `/matches/game-round/<id>/events/` because no persisted round id exists at preview time. **SIM-05 reuse decision (LOCKED): the playback JS is DUPLICATED into this template, NOT extracted into a shared partial.** Rationale: the SIM-05 engine in `game_round_events.html` is inline and tightly coupled to page-specific context — it reads Django vars directly inside the `<script>` (`{{ round.team_red.id }}` / `team_blue.id` / `team_red.name|escapejs` / `team_blue.name|escapejs`) and is interleaved with the kill-feed, the three Chart.js charts, and the M-1 timeline-window pager; a clean shared partial would have to thread those four team-identity values and decouple the charts / kill-feed — out of scope for LG-01i. The minimal playback block (the scrubber DOM + the `pbPlayers` / `pbEvts` / `pbApply` / `pbSetTime` / `pbRefresh` engine, the `MAX_SH` / `MAX_LIVES` role constants, and the two `JSON.parse(getElementById('events-data'/'players-data'))` reads) is duplicated, dropping the charts / kill-feed / timeline-window code; the team identity values come from the bundle's per-round `red_team_id` / `red_team_name` / `blue_team_id` / `blue_team_name` instead of `{{ round.* }}`. (A future refactor MAY extract `_partials/playback.html`; LG-01i does not.) **LOCKED DOM ids:** `play-week-live-container` (outer wrapper); the reused SIM-05 ids kept **IDENTICAL** so the duplicated JS works — `pb-time-display` / `pb-scrubber` / `pb-step-back` / `pb-play` / `pb-step-fwd` / `pb-speed` / `pb-sb-red` / `pb-sb-blue`; `play-week-live-commit-form` / `play-week-live-commit-submit` (the Commit `<form method="post" action="{% url 'play_week_live_commit' season.id %}">` + `{% csrf_token %}`); `play-week-live-discard-form` / `play-week-live-discard-submit` (the Discard form to `play_week_live_discard`); `play-week-live-round-toggle` (+ `play-week-live-round-1` / `play-week-live-round-2`) — the playoff round toggle, **rendered ONLY when `kind == "playoff"`**, swaps the active `ALL` / `PLAYERS` arrays and re-runs `pbReset()` + `pbRefresh()`; `play-week-live-bye-notice` (substring `"bye"`, defensive — rendered when the page is reached on an `rr_bye` cursor). **`json_script` feed ids (LOCKED):** RR emits `{{ preview_round.events_data|json_script:"events-data" }}` / `players-data` (the UNCHANGED SIM-05 ids); playoff emits BOTH rounds under round-suffixed ids `events-data-1` / `players-data-1` / `events-data-2` / `players-data-2`.

**Dashboard wiring — the "One Week (Live)" Play-dropdown entry.** A NEW context key **`live_preview_available: bool`** computed in `_build_dashboard_context` (alongside the LG-02-Part2c `playoff_phase_active` etc.) — `True` iff `_resolve_live_cursor(season)` returns a descriptor whose `kind` is `"rr"` or `"playoff"` (NOT `None`, NOT `"rr_bye"`); an optional `live_preview_kind: str | None` may be exposed for the label, but `live_preview_available` is the gate. The dropdown entry links to `{% url 'play_week_live' season.id %}` (a GET) and renders **ONLY when `live_preview_available`** (a manager with a bye or an eliminated bracket sees no live option), sitting alongside the LG-01d One Week / Two Months / Until-End entries. Entry DOM ids: `season-dashboard-play-one-week-live` (inside the existing `season-dashboard-play-dropdown`) and `league-dashboard-play-one-week-live` (inside `league-dashboard-play-dropdown`).

**Scope-out (LOCKED — DO NOT build).** **No migration, no model change** (the seed lives in `request.session`; the committed round persists its seed via the existing `GameRound.rng_seed`). **No re-baseline** (injected-seed kwargs default `None` ⇒ existing seeded games byte-identical ⇒ no Score Calibration re-baseline). **No server-side tick streaming / WebSocket / SSE** — preview-then-commit, the client SIM-05 engine plays pre-baked JSON. **No watching non-manager games** — only `current_team`'s game is previewed, every other fixture / node sims fresh at commit. **No re-roll within a pin** — re-opening replays the SAME pinned game; the only path to a new game is Discard (clears the pin) then re-open. **No playoff-live when eliminated** — `_alive_playoff_node` returns `None` ⇒ no live entry (the manager uses plain Play Single Round / Play Playoffs). **RR + alive-playoff cursors ONLY** — no live surface for `member_night` / inert phases, no whole-bracket live drain. **No SIM-05 partial extraction** — the playback JS is duplicated, not factored into `_partials/playback.html`. **No ADR, no CONTEXT.md term** (the **One Week (Live)** glossary term was finalised at grilling).

**Locked names (quick index).** Engine seam: `_simulate_and_flush_round(..., rng_seed: int | None = None)` / `simulate_scheduled_round(..., rng_seed: int | None = None)` / `simulate_match(..., rng_seeds: tuple[int, int] | None = None)` / `tournament_engine.play_specific_node(node, *, rng_seeds=None)` (extracted from `play_next_node`). Preview: `BatchSimulator.preview_scheduled_round(season, team_a, team_b, round_number, *, arena_map=None, season_phase=None, leg=1, rng_seed=None) -> dict` / `BatchSimulator.preview_tournament_match(team_red, team_blue, *, arena_map=None, rng_seeds=None) -> dict` / private `_serialize_events_for_preview(event_log, red_players, blue_players, red_team, blue_team) -> (events_data, players_data)`. Views: `matches.league_views.play_week_live_preview` (GET 200 / 400-dashboard / 405) / `play_week_live_commit` (POST 302 / 400 / 405) / `play_week_live_discard` (POST 302 / 405). Cursor helpers: `matches.league_views._resolve_live_cursor(season) -> dict | None` / `_alive_playoff_node(tournament, team) -> BracketNode | None`. URLs: `play_week_live` / `play_week_live_commit` / `play_week_live_discard` (in `matches/season_urls.py`, before `standings/` / `schedule/`). Session: `request.session["live_preview_pin"][str(season_id)]` = `{kind, current_team_id, cursor, seed | seeds}`; RR cursor identity `{type, season_id, season_phase_id, matchday, pair, round_number, leg}`, playoff `{type, tournament_id, bracket_type, bracket_round, position}`. Bundle keys: `kind, seed | seeds, rounds:[{round_number, red_team_id, red_team_name, blue_team_id, blue_team_name, events_data, players_data, result}], cursor`; `events_data` = the 13-key SIM-05 shape `type, ts, tf, icon, desc, pts, aid, an, at, tid, tn, tt, meta`; `players_data` = `{id, name, team, role, sl, ss}`. Template: `templates/seasons/play_week_live.html`; DOM ids `play-week-live-container`, the reused `pb-*` ids (`pb-time-display` / `pb-scrubber` / `pb-step-back` / `pb-play` / `pb-step-fwd` / `pb-speed` / `pb-sb-red` / `pb-sb-blue`), `play-week-live-commit-form` / `-commit-submit`, `play-week-live-discard-form` / `-discard-submit`, `play-week-live-round-toggle` (+ `play-week-live-round-1` / `-round-2`), `play-week-live-bye-notice`; `json_script` ids `events-data` / `players-data` (RR) and `events-data-1` / `players-data-1` / `events-data-2` / `players-data-2` (playoff). Dashboard: context key `live_preview_available: bool` (+ optional `live_preview_kind`) in `_build_dashboard_context`; entry DOM ids `season-dashboard-play-one-week-live` / `league-dashboard-play-one-week-live`, rendered only when `live_preview_available`. Tests: `TestLg01iInjectedSeedDefault` / `TestLg01iSeedInjection` / `TestLg01iPreviewBundle` in `test_simulation_view_paths.py`; `TestLg01iCursorDispatch` / `TestLg01iLivePreviewView` / `TestLg01iLiveCommit` / `TestLg01iLiveDiscard` in `views_tests.py` (or `test_league_play.py`); `TestLg01iDashboardEntry` in `test_league_dashboard.py` / `test_season_dashboard_view.py`. Seam contract: [`.claude/worktrees/lg-01i-one-week-live-seam-contract.md`](../../.claude/worktrees/lg-01i-one-week-live-seam-contract.md).
