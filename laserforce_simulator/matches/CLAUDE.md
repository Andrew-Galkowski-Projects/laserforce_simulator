...
> Note: these targets were tuned on the non-spatial **3-zone fallback** model. MOVE-01 (movement decoupled from the Action) and MOVE-02 (**Path commitment** — route-commitment changes which equal-cost path is walked, [ADR-0008](../../docs/adr/0008-path-commitment-via-goal-keyed-cache.md), CONTEXT.md) both leave these targets unchanged for now; the **single pending post-MOVE-01 re-baseline** against the map model now also absorbs the MOVE-02 route-commitment delta (MOVE-02 creates no separate re-baseline obligation). Still pending separate measurement/discussion.
