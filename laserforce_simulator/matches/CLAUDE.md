...
- `_move_to_cell(player, second, goal_cell, movement_ctx)` — calls `astar_advance` for `cells_to_move(player.speed, zone_data)` cells (STAT-03 Phase 1 multi-cell step, not a single cell), updates `cell_row`/`cell_col`/`zone_fallback`, saves to DB, writes a `GameEvent(event_type="movement")` at the final cell. `BatchSimulator._move_player_in_memory` mirrors this in-memory. `speed` reaches RBS via a new `PlayerRoundState.speed` forwarding property (delegates to `self.player.stat_for_simulation("speed", self.role)`, no DB column) and BatchSim via a baked `PlayerState.speed` field.
