...
Static helpers:
- `_resolve_map_data(arena_map)` — validates the map's confirmed zone config, unwraps `zone_data` dict format (`{"zones": [...], "blocked_edges": {...}, "wall_meta": {...}}` in production; raw list in older/test data), batches base config queries, and loads `SightLineConfig`, `BaseSightLineConfig`, `MapCellRankingConfig`, and `HeavyStrongSpotsConfig`. Returns `(zone_size, spawn_cells, zone_grid, sight_data, base_sight_data, cell_ranking, strong_spots, wall_meta, spawn_pools)` (9-tuple) or `(None, {}, None, None, {}, [], [], {}, {})` when `arena_map` is `None`. `spawn_pools` is `{"red": [(r,c),...], "blue": [(r,c),...]}` read from `zone_data["red_spawn"]`/`zone_data["blue_spawn"]` — no extra DB query. Raises `ValueError` if the map has no confirmed config, a missing base, no computed sight lines, or no computed base sight lines.
...
- `_build_movement_ctx(zone_data, spawn_cells, sight_data=None, base_sight_data=None, cell_ranking=None, strong_spots=None, wall_meta=None)` — returns `{"adj": ..., "spawn_cells": ..., "zone_data": ..., "sight_data": ..., "base_sight_data": ..., "cell_los_counts": ..., "high_los_cells": ..., "strong_spots": ..., "wall_meta": ...}` or `None` when `zone_data` is `None`. `wall_meta` defaults to `{}` when not provided. All dicts are built once per round and passed through the call chain.
