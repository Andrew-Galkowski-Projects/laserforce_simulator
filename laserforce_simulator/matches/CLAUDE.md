...
- `_can_tag_through_windowed_wall(r1, c1, r2, c2, zone_grid, wall_meta) -> bool` — walks a Bresenham line from actor to target. High wall (0) → always False. Windowed wall (5): facing N/S allows same-column attacks (c1==c2); facing E/W allows same-row attacks (r1==r2); unknown/missing facing → False. Used by `_get_los_targets` for aperture targeting.
- `_get_los_targets(actor, candidates, movement_ctx)` — returns the subset of `candidates` visible to `actor`. With a map, looks up the actor's cell key in `sight_data` (a `{"r,c": frozenset}` dict) and filters to candidates whose cell is in the visible set. Additionally runs `_can_tag_through_windowed_wall` for any candidate not in normal sight when `wall_meta` is non-empty. Falls back to same-zone equality when `movement_ctx` is `None`, `sight_data` is absent, or the actor has no cell position. Used by both simulators' `_choose_tag_target` methods.
- `_elevation_hit_modifier(attacker_row, attacker_col, defender_row, defender_col, movement_ctx) -> float` — MAP-09. Returns a hit-chance multiplier based on relative elevation. Reads `elevation_grid` from `movement_ctx`; returns 1.0 when `elevation_grid` is `None` or either cell position is `None`. Inlines the two grid lookups (no inner closure) and delegates to the public `elevation_hit_modifier(attacker_elev, defender_elev)` for the formula. Applied in both simulators' tag-resolution path: `hit_chance *= _elevation_hit_modifier(...)`.
- `elevation_hit_modifier(attacker_elev, target_elev) -> float` — public pure formula: `max(0.5, 1 - 0.1 * max(0, target_elev - attacker_elev))`. Uphill penalty only; returns 1.0 when attacker is at the same elevation or higher. Importable directly for testing.
