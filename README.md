...
- **Sandbox tournaments** — Build a standalone single-elimination tournament from a seeded bracket: pick existing teams and/or generate new ones, reorder the seeding (default is overall-rating order), lock the bracket (arbitrary 4+ teams, with byes for the top seeds), then play it game-by-game while winners auto-advance up a visual bracket tree to a crowned champion
