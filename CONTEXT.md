...
**Job**:
A long-running unit of background work wrapped in async execution machinery so its caller does not block. Submitted via POST (returns a **Job id**), then polled via GET against the **Job id** until the **Job status** is terminal. Four kinds today: a **Batch run job** (simulate N games, surface progressive aggregates from `BatchSimulator.run_incremental`), a **Save-games job** (replay a list of `(seed, flipped)` pairs from a prior Batch run and persist them as `GameRound` rows), a **Play Season job** (run all unplayed Rounds in the next N matchdays of a Season — `N=8` for Two Months, `N=None` for Until End of Season), and a **Play Tournament job** (play every remaining decisive Bracket node of a Tournament to a champion; progress reported by completed Bracket stage). All four share the same per-Job lifecycle (`running` / `complete` / `error`) and the same expiry-asymmetry (`PENDING` after the 1h TTL is indistinguishable from a never-submitted Job id). All four execute via Celery: the UI POSTs at `/matches/simulate-batch/`, `/matches/save-games/`, `/seasons/<id>/play-two-months/`, `/seasons/<id>/play-until-end/`, and `/tournaments/<id>/play-all/` and the REST POST at `/api/simulate-batch/` all enqueue Celery tasks and return a Job id. Backed by **Celery + Redis** in production; tests run the task synchronously via `CELERY_TASK_ALWAYS_EAGER = True`.
...
**Season phase**:
One ordered unit of a **Season**'s structure — the composable building block (LG-02-Part2) that generalises the previously-implicit "a Season *is* a single **round-robin**" assumption into an **ordered list of typed phases**. Persisted as `SeasonPhase` (FK → **Season**, a 1-based `ordinal` for ordering, a `phase_type` enum). The **LG-02-Part2 grill (2026-06-04)** resolved that this phase model **is** the LG-06 phased-lifecycle model — off-season / regular / tournament are *phase types*, not a parallel abstraction. Phase types: **`round_robin`** (the existing `single_round_robin` fixture run via `generate_schedule` — the only type built in the **Part2a foundation slice**); **`tournament`** (a future type — an embedded **Tournament** held via a one-directional `SeasonPhase → Tournament` FK, so the Tournament stays season-agnostic per [ADR-0019](docs/adr/0019-tournament-bracket-model.md) and the *phase* points at it, lazily built on phase activation à la the RR→DE deferred build); and **`member_night`** (a future type — deferred to its own sandbox grill, see PLAN.md). A `tournament` phase comes in **two flavours by Season role** (the seeding distinction resolved in the **LG-02-Part2b grill (2026-06-05)**, but *modelled* only in Part2c): a **season-ending tournament** (the playoff / canonical closer) is **seeded from the *preceding* phase's Standings** and therefore *requires* a preceding fixture-producing phase; a **mid-season tournament** (random draw / duos / trios / swiss) needs **no preceding Standings** — it is seeded by *expected team strength* or *not at all* — and may sit anywhere in the order, including first. The per-phase seeding-mode field that captures this is a **Part2c** addition; Part2b's composer captures only ordered phase *types* and flags every `tournament` block "build coming in a later release". **Part2c-1** builds the **season-ending playoff** (a standings-seeded single-elimination `Tournament` auto-built the moment the round-robin completes, draining to the Season champion) and enforces its structural requirement at compose time (a `tournament` phase requires a preceding `round_robin` phase); the **per-phase seeding-mode field itself + the mid-season-tournament flavour** (strength-/un-seeded, may sit first) remain **Part2c-2**. Read-path contract: the play loop / **Standings** / Season completion read the Season's ordered phases, **not** `Season.schedule_format`; a Season with **zero** persisted phases falls back to an **implicit single `round_robin` phase** (today's exact behaviour — **no backfill, no `RunPython`**, the [ADR-0004](docs/adr/0004-simulation-data-is-disposable.md) disposable-data precedent). A new Season's phases are created explicitly at create time: at **create-League** the composer writes the ordered phase list (an empty composer defaults to one `round_robin` phase, Part2a-equivalent); at **next-Season** `next_season` **carries the previous Season's composition forward** by copying its live `phases` rows (`tournament` FK reset to NULL — the build is Part2c), mirroring the existing team-id / map-pool carry-forward. `Season.schedule_format` stays as the legacy per-Season format: **Part2b** adds a per-phase `schedule_format` **dormant column** (written at create, copied from the Season value) but the read-path chokepoint still reads `Season.schedule_format`; wiring the chokepoint to `phase.schedule_format` lands in a later slice alongside the first alternative format. Part2b also lands the **composer UI** (writes multiple ordered phases — `round_robin` + `tournament` blocks only, `member_night` "coming soon") and the **NULL `SeasonPhase → Tournament` FK column** (the build / seed / hand-off is Part2c); the Part2b read-path plays the **first `round_robin` phase** only, leaving every other phase persisted-but-dormant until Part2c. Recorded in [ADR-0023](docs/adr/0023-season-phase-composable-structure.md).
_Avoid_: calling a Season phase a tournament **Bracket round** or a tournament **stage** (those are *intra-Tournament* tiers; a Season phase is a *Season-level* unit, one of which may *contain* a Tournament); calling it a "block" except as the builder-UI verb ("**+** Add block"); reading `Season.schedule_format` as the source of truth for play (the ordered phase list is, with the implicit-single-`round_robin` fallback); pointing a **Tournament** back at a **Season**/**League** (the FK is one-directional `SeasonPhase → Tournament`; Tournament stays season-agnostic — ADR-0019 survives).
...
**Standings form**:
The recent-results trend columns on the **Standings** table (LG-06g) — **Streak** and **Last-5 (L5)** — surfaced in **two grains** side by side. The **Match-keyed** pair (overall trend) reads the same completed-**Match** corpus as the Standings W/L record: a Match result for a team is W (`Match.winner_id == team`), L (other team won), or T (`winner_id IS NULL`). The **Round-keyed** pair (week-to-week trend, one **Round** ≈ one **Matchday**) reads **every persisted Season Round** — including the first Round of an in-progress (`is_completed=False`) Match — with each Round's result taken from raw `GameRound.red_points` vs `blue_points` (tie when equal). **Streak** is the current run of identical results from the most recent game (`"W3"` / `"L2"` / `"T1"`); **Last-5** is the W-L-T over the last five (`"3-1-1"`). Both grains order chronologically by `date_played` then `id`, most-recent last.
_Avoid_: reading L5/Streak off **Round** outcomes for the Match-keyed pair (it mirrors the Match-keyed W/L record); excluding in-progress Matches' Rounds from the Round-keyed pair (the week-to-week grain deliberately counts them); using the **Match**-level `red_*`/`blue_*` fields for the Round result (those are team-position-keyed, not physical-side — see **Side split**).

**Side split**:
The per-physical-**Side** breakdown columns on the **Standings** table (LG-06g) — a **Team**'s record and points split by which physical **Side** (red or blue) it actually held each **Round**, over **every persisted Season Round** (the same Round-keyed corpus as the Round-grain **Standings form**, including in-progress Matches). Four numbers: a **Red record** (W-L-T of Rounds physically played on the red Side) and **Blue record**, plus **Red points** / **Blue points** (total points the team scored while on each Side). Read from the **Round** itself — `GameRound.team_red`/`team_blue` give the physical Side and `red_points`/`blue_points` the Side score — never from the **Match**-level `red_*`/`blue_*` fields (those are team-position-keyed: `Match.red_round2_points` is `team_red`'s points while it physically played *blue* in Round 2, so a Match-level read would mis-attribute the Side).
_Avoid_: deriving the Side split from `Match.red_rounds_won` / `red_total_points` (team-position-keyed, not physical Side); counting only completed-Match Rounds (the split is Round-keyed and counts in-progress Matches' Rounds, matching the Round-grain **Standings form**).

**Matchday**:
One slot in the `generate_schedule(...)` mirror pattern. The 1-based ``matchday`` field on `ScheduleFixture`. Multiple fixtures share a matchday — one per pairing (round-1 matchdays `1..N-1` carry the first leg; round-2 matchdays `N..2*(N-1)` carry the mirrored leg). The scheduling unit is the **Round**, not the matchday — a matchday holds multiple `ScheduleFixture` rows. The **Play One Week** action plays every Round in a single Matchday; **Play Two Months** plays the next 8 Matchdays; **Play Until End of Season** plays every unplayed Round regardless of Matchday — relabeled **"Until Playoffs"** when a `tournament` **Season phase** follows the round-robin (LG-02-Part2c-1; same `play_until_end` behaviour, label text only). Once the regular season completes and the playoff bracket is built, two playoff actions drain it: **Play Single Round** plays one bracket node (one tournament **Match**), and **Play Playoffs** drains the whole bracket to crown the **Season** champion. The calendar date of a Matchday is `Season.start_date + (matchday - 1) * 7 days`.
...
**Watch list**:
A **per-League** set of **Players** a user has flagged to follow — the bookmark backing the LG-06f **Watch List** screen, which renders those Players in the full **Player Stats** column set (zero-filled for a watched Player with no **Rounds** in the displayed scope, so the set stays stable across the `?season=` selector). Persisted **per browser session, keyed by League** (`request.session["watch_lists"]` = `{"<league_id>": [player_id, …]}`) — deliberately separate per League because Players are League-scoped entities and the same name may recur across Leagues as distinct **Player** rows. Not per-user until accounts land (UX-01 moves it from the session to a per-user model, the same deferral as the rest of the league screens). Membership is mutated only through the **Watch flag**; the Watch List screen additionally keeps a **Remove All** control (clears that League's list). Storage is disposable — switching from the pre-LG-06f global `session["watch_list"]` (singular) to the per-League `session["watch_lists"]` (plural) needs no migration ([ADR-0004](docs/adr/0004-simulation-data-is-disposable.md) precedent).
_Avoid_: treating the watch list as global / cross-League (it is per-League — a Player watched in League A is not watched in League B); treating it as per-user (it is per-browser-session until UX-01); expecting a watched Player with no games in the displayed Season to vanish (the screen zero-fills them so the bookmark set is stable).

**Watch flag**:
The in-row toggle control (a small flag after a **Player**'s name, ZenGM-style — grey when un-watched, red when watched) that adds/removes that Player from the current **League**'s **Watch list**. Rendered by a shared partial on every League screen that lists Players (Player Stats, Player Ratings, Free Agents, League Leaders, Statistical Feats, Team Roster, Team History, and the Watch List screen itself — where it **replaces** the per-row Remove button). Toggling is an instant client-side `fetch()` POST to the per-League toggle endpoint (`/leagues/<league_id>/players/watch-list/toggle/`) that flips the session set and returns the new watched state; the flag re-colours in place with no page reload (the LG-01d inline-fetch precedent). The **league-agnostic** player career page (`/players/<id>/stats/`) deliberately has **no** Watch flag — it carries no `league_id` to scope the toggle; pinning a player page to a League and flagging it there is the separate **LG-06h** task.
_Avoid_: putting the Watch flag on league-agnostic / sandbox player surfaces (it needs a `league_id` for the per-League toggle — that is LG-06h's job, not LG-06f's); expecting the flag to round-trip a full page load (it is an in-place `fetch()` toggle, like the LG-01d Play dropdown).
...
_Avoid_: treating a Tournament as a **Season** phase or a **League** (Part 1 Tournament is its own sandbox container, not owned by a League); routing a Tournament's *results-contingent advancement* through the **Season** `generate_schedule` path (that path is deterministic from the team set and cannot model advancement — an **elimination** Bracket must not use it; a **Round robin**, which has no advancement, legitimately reuses its pairing algorithm); confusing a Tournament's standalone Teams with a Season's enrolled M2M.
...
_Avoid_: calling an *elimination* Bracket a "schedule" (an elimination Bracket is a results-contingent tree, distinct from a **Season**'s flat `generate_schedule` fixture list); assuming one tree (double-elimination is two coupled brackets); assuming every Bracket is a tree at all (the **Round robin** format is a flat, edge-less Bracket whose pairings legitimately come from `generate_schedule` and whose champion comes from **Standings**, not a final).
...
**Round robin**:
A **Tournament** **format** (LG-02c) in which every enrolled **Team** plays every other — a flat all-play-all with **no Advancement** and no tree, whose champion is the **Standings** leader after every game is played (not a bracket final). Implemented as **flat Bracket nodes**: the pairings are produced by reusing the **Season** `generate_schedule` circle method as a **double round-robin** (each pair meets twice — both legs of `generate_schedule`), one `BracketNode` per fixture with both team-slots fixed at lock time, `advances_to = None`, `series_length = 1` (every pairing a single **Bo1** **Match** — **Series escalation** does not apply, there is no final or depth), and `bracket_type = "round_robin"`. The champion and the live ranking reuse `matches/standings.py::compute_standings` (the same `league_points → round_wins → total_score → team-name` ladder a **Season** uses); the Tournament completes only when *every* node is resolved (not on the elimination "final node decided" rule — in a round-robin every node has `advances_to = None`). The detail page renders an N×N **crosstable** plus a Standings table, never a tree. Distinct from **single-/double-elimination** (results-contingent trees) and from the **Season** it borrows the pairing generator from (a Season has cumulative cross-Season **Standings** and a `generate_schedule` over enrolled Teams; a round-robin Tournament is a one-off standalone sandbox container).
_Avoid_: expecting **Advancement** / a tree / a bracket final in a round-robin (the champion is the **Standings** leader, decided after the last game); applying **Series escalation** or the **Bracket seed** node tiebreaker to it (every node is **Bo1**, and standings ties break on team name inside `compute_standings`, not on seed); treating reuse of `generate_schedule` as a violation of the **Bracket**-is-not-a-schedule rule (that rule forbids routing *advancement* through the deterministic path — round-robin has none); confusing the double round-robin's two **legs** of an A-vs-B pairing with the two game **Rounds** inside one **Match** (each leg is a full Match = two Rounds); assuming the round-robin **Standings** leader is always the Tournament champion (true for the standalone **Round robin** format, but in **Round robin → double elimination** the round-robin is only a **Seeding stage** — its standings *seed* the **Finals stage**, and the champion comes from the **Grand final**, not the standings).

**Round robin → double elimination**:
A two-stage **Tournament** **format** (`round_robin_double_elim`) combining a round-robin **Seeding stage** with a **double-elimination** **Finals stage**. Every enrolled **Team** plays a full double round-robin (the **Round robin** format's flat `BracketNode` set, `bracket_type = "round_robin"`) whose final **Standings** rank — *not* its champion — **seeds** the Finals: the top `N_upper` ranked Teams start in the **Winners bracket**, the next `N_lower` start pre-dropped into the **Losers bracket**, and the remaining Teams are **eliminated** after the Seeding stage (they never enter the Finals). `N_upper` is a power of two (4 / 8 / 16) and `N_lower` is either `0` (a plain top-`N_upper` double-elimination) or `N_upper / 2` (every **Losers bracket** round-1 node pairs one pre-seeded LB Team against one **Winners bracket** round-1 **Drop**per). The Finals **Bracket seed** is the round-robin standings rank (rank 1 = top WB seed), never the original talent **Seeding**. Because the Finals participants are *results-contingent* on the Seeding stage, the double-elimination **Bracket** is **not** built at lock time like every other format — it is built lazily the moment the last round-robin node resolves (the **deferred bracket build**, [ADR-0021](docs/adr/0021-double-elimination-bracket.md)); the Tournament stays `active` across the stage transition and the champion is crowned by the **Grand final** / **Bracket reset** exactly as in standalone double-elimination.
_Avoid_: calling the round-robin half a "Tournament" of its own (it is a **Seeding stage** — it crowns no one, it only ranks); seeding the Finals from the **talent Seeding** rather than the round-robin **Standings** rank; expecting the **Losers bracket** to be filled only by **Drops** (in this format `N_lower` Teams are *pre-seeded* into the LB before any Drop); expecting the Finals **Bracket** to exist at lock time (it is **deferred**-built when the Seeding stage ends).

**Swiss**:
A **Tournament** **format** (`swiss`, LG-02c) in which every enrolled **Team** plays a fixed number of **Swiss rounds** — far fewer than a full **Round robin** — being **paired each round against a similarly-ranked Team it has not yet faced**, with the champion the **Standings** leader after the last round (no bracket final). The number of rounds defaults to ⌈log₂(N)⌉ (admin-overridable, clamped to `[1, N−1]`) and is frozen at lock; **N must be even** (an odd field is rejected at lock — there are no **Byes** in Swiss, a deliberate divergence from chess Swiss). Like **Round robin** it is a flat, edge-less **Bracket** of `BracketNode`s (`bracket_type = "swiss"`, `series_length = 1` — every pairing one **Bo1** **Match**, no **Series escalation**, `advances_to = None`), but unlike every other format **its nodes are not all built at lock**: only **round 1** is built at lock (seed-based "fold" pairing — sort by **Bracket seed**, split in half, pair `1 vs N/2+1`, `2 vs N/2+2`, …), and each later round is **deferred**-built the moment the previous round's last node resolves (the recurring analogue of the RR→DE **deferred bracket build**), pairing from the *current* **Standings** by a greedy ranked sweep (walk the ranked list top-down, pair each unpaired Team with the next unpaired Team it has not played; if avoiding a rematch is impossible for the trailing Teams, allow the rematch). There are **no draws** — `break_tie` forces a winner on every **Match** (consistent with **Round robin**), so `league_points` are purely `3·wins`. The champion and the displayed Swiss table are ranked by a **Buchholz**-augmented ladder (below), *not* `compute_standings`' built-in ladder. The Tournament completes (crowns the **Standings** leader) only when the last node of the last Swiss round resolves.
_Avoid_: expecting **Advancement** / a tree / a bracket final (the champion is the **Standings** leader after a fixed number of rounds); building all Swiss rounds at lock (only round 1 is built at lock — later rounds are results-contingent and **deferred**); supporting odd N or **Byes** (Swiss is even-N-only here, rejected at lock); expecting draws (every Match is W/L via `break_tie`, like **Round robin**); ranking the Swiss table by the plain `compute_standings` ladder (Swiss inserts **Buchholz** as the first tiebreak — see below); confusing a **Swiss round** (one full set of simultaneous pairings, a `bracket_round` tier) with a game **Round** (the 15-min `GameRound` unit) or a **Series** (Swiss is always Bo1).

**Buchholz**:
The Swiss strength-of-opponent tiebreaker — a **Team**'s Buchholz score is the **sum of its opponents' final `league_points`** across every **Swiss round** it played (plain Buchholz; not median-Buchholz or Sonneborn-Berger). It rewards a Team that earned its points against a tougher field. Swiss ranking (table + champion) uses the full ladder `league_points desc → Buchholz desc → round_wins desc → total_score desc → team_name asc` — i.e. points first, then Buchholz, then `compute_standings`' own residual tiebreaks for full determinism. Because `compute_standings` is a frozen shared pure module (a **Season** / **Round robin** consume it unchanged), Buchholz is computed in a *separate* Swiss-only re-rank layer that takes the `compute_standings` rows **plus** the played-pairs opponent graph and re-sorts; the base module is never touched.
_Avoid_: applying Buchholz to **Round robin** or **Season** **Standings** (it is Swiss-only — RR/Season keep the plain `compute_standings` ladder); using a median/Sonneborn-Berger variant (plain sum-of-opponents'-points only); expecting Buchholz to see draw points (there are no draws — opponents' `league_points` are always `3·wins`).

**Player pool**:
The set of individual **Players** registered to a **player-pool Tournament** — a Tournament whose participants are *Players*, not pre-built **Teams**. The system assembles the Teams (the pool is registered first, Teams come second), so a player-pool Tournament inverts the LG-02a assumption that a participant *is* a Team. Intake mirrors the Team-intake of the elim/RR formats but at Player granularity: **select existing** Players (the global Players list / free-agent pool), **generate** fresh Players (LG-00 `player_generator`), or **CSV import** a roster of Players (LG-00b reuse). The first player-pool format is **Random Draw**; the **Duos / Trios** sub-group formats (deferred to LG-02x-2) are the others.
_Avoid_: treating a player-pool Tournament's participants as **Teams** (they are **Players** — the Teams are *produced* by the **Draw**, not registered); confusing the Player pool with the per-League **Free Agents Team** (the pool is a registration set for one Tournament, not a standing roster container).

**Drawn-team membership** (cross-tournament Player sharing):
A **Player** may belong to many **drawn Teams** across **different** Tournaments at once, but **never two Teams in the same Tournament**. The single `Player.team` FK is **not** the drawn-team membership mechanism — the durable membership is the **`TournamentPlayerEntry`** (its `unique(tournament, player)` enforces the one-team-per-Tournament rule), and a drawn Team merely *references* its (possibly borrowed) Players through its **slot** FKs. The Draw therefore **does not reassign `Player.team`** — a selected Player keeps their home / free-agent team, and generated / CSV-imported pool Players are created on the **Free Agents Team**. Because of this, the "all slot Players belong to this Team" **Roster** rule is **relaxed for draw Teams only** (the duplicate-Player and Role-distribution rules still apply); `PlayerRoundState` rows reference the real Player, so a draw Tournament's games merge into that Player's unified career stats.
_Avoid_: reassigning `Player.team` onto the drawn Team (it stays the Player's home/free-agent team — sharing across Tournaments depends on not moving it); relying on the `player.team_id == team.pk` roster check for draw Teams (it is deliberately relaxed there); letting one Player land on two drawn Teams of the same Tournament (forbidden — guaranteed by `TournamentPlayerEntry`'s uniqueness and the one-Player-per-Tier-slot Draw).

**Random Draw**:
A **player-pool Tournament** **format** that assembles a **Player pool** into skill-balanced **Teams** by **Tier** and then assigns **Roles dynamically per Round**, run as **round robin → double elimination**. The pool (size a multiple of 6, yielding ≥ 4 Teams) is split into **6 Tiers** by talent; each drawn Team takes exactly one Player per Tier (so every Team has one top-Tier player, one second-Tier player, …) — the **Draw**. The Draw runs in `setup` (reviewable / re-rollable / hand-editable), persists the **drawn Teams**, and `lock_and_build` then builds the RR→DE **Bracket** over them. Distinct from the elim/RR/Swiss formats, whose participants are pre-built Teams with fixed **Rosters**.
_Avoid_: calling the Draw "random" in the shuffle sense (it is **Tier-balanced** — one Player per Tier per Team, deliberately *not* a flat shuffle); assuming a drawn Team's **Roles** are fixed on its **slot** FKs (Roles are re-assigned every **Round** — the stored slots are transient, see **Role assignment mode**); expecting a new Tournament **state** (the Draw lives inside the existing `setup` window).

**Tier**:
One of **6 skill bands** the **Player pool** of a **Random Draw** Tournament is split into (by `overall_rating` descending, chunked into 6 equal bands of `team_count = N/6` Players each). Each **drawn Team** takes exactly one Player per Tier, so the Teams are skill-balanced. A Player's Tier is fixed for the Tournament and drives **per-tier Role assignment** (the **Role assignment mode**).
_Avoid_: confusing a **Tier** (a skill band of the player pool, sized `N/6`) with the 6 **Role** slots of a Team (a Tier maps *to* a role each Round, but is not itself a role); confusing a Tier with a **Bracket seed** (the Tier balances *team composition*; the seed ranks *teams* in the Bracket).

**Role assignment mode**:
A **Random Draw** Tournament's rule for assigning the 6 **Roles** (commander / heavy / scout / scout / medic / ammo) to a drawn Team's 6 **Tier** Players, **re-drawn every Round** (the 2 Rounds of a **Match** get independent assignments; tournament sims are non-deterministic so the Role draw consumes a fresh RNG, no SIM-07/08 contract). Fixed at create time as one of two modes: **random** — each Team independently shuffles its Tier players into the 6 role slots; **per-tier matched** — one Tier→Role mapping is drawn per Round and applied to **both** Teams in the Match, so equal-Tier Players face off in the same Role (skill-matched per-Role matchups).
_Avoid_: storing the per-Round Roles as the drawn Team's standing **Roster** (the slot FKs are overwritten each Round — the durable truth is the (Player, Tier) membership); applying **per-tier matched** mode across Teams that are not the two in the current Match (the mapping is shared only by the Match's two opponents).
